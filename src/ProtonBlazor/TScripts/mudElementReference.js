...
class ProElementReference {
    ...
    saveFocus (element) {
        if (element)
        {
            ...
            element['protonblazor_savedFocus'] = document.activeElement;
        }
    }
    ...
    restoreFocus (element) {
        if (element)
        {
            const previous = element['protonblazor_savedFocus'];
            delete element['protonblazor_savedFocus'];
            if (previous)
                previous.focus();
        }
    }
    ...
    addOnBlurEvent(element, dotNetReference) {
        ...
        element._proBlurHandler = function (e) {
        ...
        element.addEventListener('blur', element._proBlurHandler);
    }
    ...
    removeOnBlurEvent(element) {
        if (!element) return;
        if (element._proBlurHandler) {
            element.removeEventListener('blur', element._proBlurHandler);
            delete element._proBlurHandler;
        }
    }
};
window.mudElementRef = new ProElementReference();
