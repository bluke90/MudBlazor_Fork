...
class ProResizeListener {
    ...
    listenForResize(dotnetRef, options) {
        ...
        this.logger(`[ProtonBlazor] Reporting resize events at rate of: ${this.options.reportRate}ms`);
    ...
    resizeHandler() {
        ...
        try {
            ...
            this.logger("[ProtonBlazor] Error in resizeHandler:", { error });
        }
    }
    ...
    /**
     * Maps viewport width to ProtonBlazor breakpoint index.
     */
    getBreakpoint(width) {
...
window.mudResizeListener = new ProResizeListener();
window.mudResizeListenerFactory = {
    ...
    listenForResize: (dotnetRef, options, id) => {
        ...
        const listener = new ProResizeListener(id);
