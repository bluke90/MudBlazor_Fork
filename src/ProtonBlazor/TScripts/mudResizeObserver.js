...
/**
 * Factory that maps .NET observer IDs to ProResizeObserver instances.
 * Enables independent element-observer lifecycles per service instance.
 */
class ProResizeObserverFactory {
    constructor() {
        this._maps = {};
    }
    ...
    connect(id, dotNetRef, elements, elementIds, options) {
        ...
        if (!existingEntry) {
            const observer = new ProResizeObserver(dotNetRef, options);
...
class ProResizeObserver {
    constructor(dotNetRef, options) {
        ...
        this.logger('[ProtonBlazor | ResizeObserver] Observer initialized');

        this._resizeObserver = new ResizeObserver(entries => {
            const changes = [];
            this.logger('[ProtonBlazor | ResizeObserver] changes detected');
    ...
    resizeHandler(changes) {
        try {
            this.logger("[ProtonBlazor | ResizeObserver] OnSizeChanged handler invoked");
            this._dotNetRef.invokeMethodAsync("OnSizeChanged", changes);
        } catch (error) {
            this.logger("[ProtonBlazor | ResizeObserver] Error in OnSizeChanged handler:", { error });
        }
    }
    ...
    connect(elements, ids) {
        const result = [];
        this.logger('[ProtonBlazor | ResizeObserver] Start observing elements...');
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            this.logger("[ProtonBlazor | ResizeObserver] Start observing element:", { newEntry });
    ...
    disconnect(elementId) {
        this.logger('[ProtonBlazor | ResizeObserver] Try to unobserve element with id', { elementId });
        ...
        if (affectedObservedElement) {
            ...
            this.logger('[ProtonBlazor | ResizeObserver] Element found. Ubobserving size changes of element', { element });
     ...
     */
    cancelListener() {
        this.logger('[ProtonBlazor | ResizeObserver] Closing ResizeObserver. Detaching all observed elements');

        this._resizeObserver.disconnect();
        this._dotNetRef = undefined;
    }
}
window.mudResizeObserver = new ProResizeObserverFactory();
