...
/**
 * SplitPanel resize behavior for the ProSplitPanel component.
...
class ProSplitPanel {
    ...
    static build(containerId, horizontal, resetOnDoubleClick, minPanelSize, firstPanelInitialSize, panelGap) {
        window.splitPanels[containerId] = new ProSplitPanel(containerId, horizontal, resetOnDoubleClick, minPanelSize, firstPanelInitialSize, panelGap);
    }

    constructor(containerId, horizontal, resetOnDoubleClick, minPanelSize, firstPanelInitialSize, panelGap) {
        this.container = document.getElementById(containerId);
        if (!this.container) {
            console.warn(`ProSplitPanel: Container with id '${containerId}' not found.`);
            return;
        }

        const children = this.container.children;
        if (children.length !== 3) {
            console.warn(`ProSplitPanel: Invalid child count '${children.length}'.`);
            return;
        }
...
if (!window.mudSplitPanel) {
    window.mudSplitPanel = ProSplitPanel;
    window.splitPanels = {};
}
