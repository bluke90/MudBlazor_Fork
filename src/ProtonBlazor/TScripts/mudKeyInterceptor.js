...
/**
 * Factory that resolves elements and manages ProKeyInterceptor instances.
 * Exposes connect/update/disconnect entry points for .NET interop.
 */
class ProKeyInterceptorFactory {
    /**
     * Creates (or reuses) a key interceptor for an element and attaches handlers.
     */
    connect(dotNetRef, elementId, options) {
        //console.log('[ProtonBlazor | ProKeyInterceptorFactory] connect ', { dotNetRef, element, options });
        if (!elementId)
        ...
        if (!element.mudKeyInterceptor)
            element.mudKeyInterceptor = new ProKeyInterceptor(dotNetRef, options);
...
window.mudKeyInterceptor = new ProKeyInterceptorFactory();
 ...
 */
class ProKeyInterceptor {
    constructor(dotNetRef, options) {
        this._dotNetRef = dotNetRef;
        this._options = options;
        this.logger = options.enableLogging ? console.log : () => { };
        this.logger('[ProtonBlazor | KeyInterceptor] Interceptor initialized', { options });
    }
    ...
    connect(element) {
        ...
        for (const keyOption of this._options.keys) {
            if (!keyOption?.key) {
                this.logger('[ProtonBlazor | KeyInterceptor] got invalid key options: ', keyOption);
                continue;
            }
            this.setKeyOption(keyOption);
        }
        this.logger('[ProtonBlazor | KeyInterceptor] key options: ', this._keyOptions);
        if (this._regexOptions.length > 0)
            this.logger('[ProtonBlazor | KeyInterceptor] regex options: ', this._regexOptions);
    ...
    updatekey(updatedOption) {
        ...
        option || this.logger('[ProtonBlazor | KeyInterceptor] updating option failed: key not registered');
        this.setKeyOption(updatedOption);
        this.logger('[ProtonBlazor | KeyInterceptor] updated option ', { option, updatedOption });
    }
    ...
    disconnect() {
        if (!this._isConnected)
            return;
        this.logger('[ProtonBlazor | KeyInterceptor] disconnect delegated event handlers');
    ...
    attachHandlers(element) {
        this.logger('[ProtonBlazor | KeyInterceptor] attaching delegated handlers ', { element });
    ...
    detachHandlers(element) {
        this.logger('[ProtonBlazor | KeyInterceptor] detaching delegated handlers ', { element });
        if (!this._delegatedHandlersAttached)
            return;
    ...
    onKeyDown(args) {
        ...
        if (!self.shouldHandleEvent.call(this, args))
            return;
        if (!args.key) {
            self.logger('[ProtonBlazor | KeyInterceptor] key is undefined', args);
            return;
        }

        const key = args.key.toLowerCase();
        self.logger('[ProtonBlazor | KeyInterceptor] down "' + key + '"', args);
        let invoke = false;
        if (self._keyOptions.hasOwnProperty(key)) {
            const keyOptions = self._keyOptions[key];
            self.logger('[ProtonBlazor | KeyInterceptor] options for "' + key + '"', keyOptions);
        ...
        for (const keyOptions of self._regexOptions) {
            ...
            if (keyOptions.regex.test(key)) {
                self.logger('[ProtonBlazor | KeyInterceptor] regex options for "' + key + '"', keyOptions);
                self.processKeyDown(args, keyOptions);
                if (self.shouldInvokeKeyDown(args, keyOptions))
                    invoke = true;
            }
    ...
    onKeyUp(args) {
        ...
        const key = args.key.toLowerCase();
        self.logger('[ProtonBlazor | KeyInterceptor] up "' + key + '"', args);
        let invoke = false;
        if (self._keyOptions.hasOwnProperty(key)) {
            const keyOptions = self._keyOptions[key];
