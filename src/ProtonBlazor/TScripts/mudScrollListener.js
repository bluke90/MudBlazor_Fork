...
class ProScrollListener {
    ...
    _getElementBySelector(selector) {
        ...
        if (!element && selector) {
            console.error(`[ProtonBlazor] ProScrollListener._getElementBySelector: Element not found: ${selector}`);
            return null;
        }
    ...
    scrollHandler(dotnetReference, listenerId) {
        try {
            ...
            console.error('[ProtonBlazor] ProScrollListener.scrollHandler:', {error});
        }
    }
...
if (!window.mudScrollListener) {
    window.mudScrollListener = new ProScrollListener();
}
