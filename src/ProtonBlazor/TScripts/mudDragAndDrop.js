...
window.mudDragAndDrop = {
    ...
    makeDropZonesNotRelative: () => {
        const firstDropItems = Array.from(document.getElementsByClassName('pro-drop-item')).filter(x => x.getAttribute('index') == "-1");
        for (const dropItem of firstDropItems) {
            dropItem.style.position = 'static';
        }

        const dropZones = document.getElementsByClassName('pro-drop-zone');
    ...
    getDropZoneIdentifierOnPosition: (x, y) => {
        ...
        const dropZones = elems.filter(e => e.classList.contains('pro-drop-zone'));
    ...
    getDropIndexOnPosition: (x, y, id) => {
        ...
        const dropItems = elems.filter(e => e.classList.contains('pro-drop-item') && e.id != id);
