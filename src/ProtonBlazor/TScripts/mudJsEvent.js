...
/**
 * Factory that wires element IDs to ProJsEvent instances.
 * Provides connect/subscribe lifecycle entry points for .NET interop.
 */
class ProJsEventFactory {
    /**
     * Creates (or reuses) a JsEvent observer for the element and starts observing it.
     */
    connect(dotNetRef, elementId, options) {
        //console.log('[ProtonBlazor | ProJsEventFactory] connect ', { dotNetRef, elementId, options });
        if (!elementId)
            throw "[ProtonBlazor | JsEvent] elementId: expected element id!";
        const element = document.getElementById(elementId);
        if (!element)
            throw "[ProtonBlazor | JsEvent] no element found for id: " + elementId;
        if (!element.mudJsEvent)
            element.mudJsEvent = new ProJsEvent(dotNetRef, options);
    ...
    subscribe(elementId, eventName) {
        //console.log('[ProtonBlazor | ProJsEventFactory] subscribe ', { elementId, eventName});
        if (!elementId)
            throw "[ProtonBlazor | JsEvent] elementId: expected element id!";
        const element = document.getElementById(elementId);
        if (!element)
            throw "[ProtonBlazor | JsEvent] no element found for id: " +elementId;
        if (!element.mudJsEvent)
            throw "[ProtonBlazor | JsEvent] please connect before subscribing";
...
window.mudJsEvent = new ProJsEventFactory();
...
class ProJsEvent {
    constructor(dotNetRef, options) {
        ...
        this.logger('[ProtonBlazor | JsEvent] Initialized', { options });
    ...
    connect(element) {
        ...
        this.logger('[ProtonBlazor | JsEvent] Start observing DOM of element for changes to child with class ', { element, targetClass });
    ...
    disconnect() {
        if (!this._observer)
            return;
        this.logger('[ProtonBlazor | JsEvent] disconnect mutation observer and event handler ');
    ...
    subscribe(eventName) {
        ...
        const targetClass = this._options.targetClass;
        //this.logger('[ProtonBlazor | JsEvent] Subscribe event ' + eventName, { element, targetClass });
    ...
    unsubscribe(eventName) {
        if (!this._observer)
            return;
        this.logger('[ProtonBlazor | JsEvent] unsubscribe event handler ' + eventName );
    ...
    attachHandlers(child) {
        ...
        //this.logger('[ProtonBlazor | JsEvent] attachHandlers ', this._subscribedEvents, child);
        for (const eventName of Object.getOwnPropertyNames(this._subscribedEvents)) {
            ...
            this.logger('[ProtonBlazor | JsEvent] attaching event ' + eventName, child);
    ...
    detachHandler(child, eventName) {
        this.logger('[ProtonBlazor | JsEvent] detaching handler ' + eventName, child);
    ...
    detachHandlers(child) {
        this.logger('[ProtonBlazor | JsEvent] detaching handlers ', child);
        for (const eventName of Object.getOwnPropertyNames(this._subscribedEvents)) {
            if (!this._subscribedEvents[eventName])
                continue;
            child.removeEventListener(eventName, this.eventHandler);
    ...
    onDomChanged(mutationsList, _) {
        ...
        //self.logger('[ProtonBlazor | JsEvent] onDomChanged: ', { self });
        const targetClass = self._options.targetClass;
        for (const mutation of mutationsList) {
            //self.logger('[ProtonBlazor | JsEvent] Subtree mutation: ', { mutation });
    ...
    eventHandler(e) {
        ...
        self.logger('[ProtonBlazor | JsEvent] "' + eventName + '"', e);
    ...
    onkeyup(self, e) {
        ...
        if (invoke) {
            ...
            //self.logger('[ProtonBlazor | JsEvent] caret pos: ' + caretPosition);
    ...
    //    if (invoke) {
    //        //self.logger('[ProtonBlazor | JsEvent] copy (preventing default and stopping propagation)');
    //        e.preventDefault();
    //        e.stopPropagation();
    ...
    onpaste(self, e) {
        ...
        if (invoke) {
            //self.logger('[ProtonBlazor | JsEvent] paste (preventing default and stopping propagation)');
            e.preventDefault();
            e.stopPropagation();
            const clipboardData = ((e.originalEvent || e).clipboardData || window.clipboardData);
            if (!clipboardData) {
                self.logger('[ProtonBlazor | JsEvent] clipboardData is null', e);
                return;
            }
    ...
    onselect(self, e) {
        ...
        if (invoke) {
            ...
            //self.logger('[ProtonBlazor | JsEvent] select ' + start + "-" + end);
