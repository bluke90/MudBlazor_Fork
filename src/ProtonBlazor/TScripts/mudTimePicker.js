...
/**
 * Pointer interaction model for the analog clock UI in ProTimePicker.
...
window.mudTimePicker = {
    ...
    initPointerEvents: (clock, dotNetHelper) => {
        ...
        const startHandler = (event) => {
            ...
            if (event.target.classList.contains('pro-hour') || event.target.classList.contains('pro-minute')) {
        ...
        const moveHandler = (event) => {
            if (!isPointerDown || (!event.target.classList.contains('pro-hour') && !event.target.classList.contains('pro-minute'))) {
