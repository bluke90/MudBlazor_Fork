...
{
    /// <summary>
    /// Linearly interpolates between two <see cref="ProColor"/> instances.
    /// </summary>
    /// <param name="colorStart">The starting <see cref="ProColor"/> instance.</param>
    /// <param name="colorEnd">The ending <see cref="ProColor"/> instance.</param>
    /// <param name="t">The interpolation factor (0.0 to 1.0).</param>
    /// <returns>A new <see cref="ProColor"/> instance that is the result of the interpolation.</returns>
    public static ProColor Lerp(ProColor colorStart, ProColor colorEnd, float t)
    {
        ...
        // Creating a ProColor from an alpha byte or integer can result in fractional alpha values (e.g., 0.996078431372549), 
        // which makes it difficult to compare two colors accurately in real-world scenarios.
        return new ProColor(r, g, b, alpha: aPercentage);
    ...
    /// <returns>An enumerable collection of <see cref="ProColor"/> representing the gradient palette.</returns>
    ...
    public static IEnumerable<ProColor> GenerateGradientPalette(ProColor startColor, ProColor endColor, int numberOfColors = 5)
    {
    ...
    /// <returns>An enumerable collection of <see cref="ProColor"/> representing the multi-gradient palette.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="numberOfColors"/> is less than or equal to zero.</exception>
    ...
    public static IEnumerable<ProColor> GenerateMultiGradientPalette(IReadOnlyList<ProColor> colors, int numberOfColors = 5)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(numberOfColors);
        if (colors.Count < 2)
        {
    ...
    /// <returns>An enumerable collection of <see cref="ProColor"/> representing the analogous palette.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="numberOfColors"/> is less than or equal to zero.</exception>
    public static IEnumerable<ProColor> GenerateAnalogousPalette(ProColor baseColor, int numberOfColors = 5, double angle = 30)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(numberOfColors);
    ...
    /// <returns>A read-only list of <see cref="ProColor"/> representing the generated palette.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="numberOfColors"/> is less than or equal to zero.</exception>
    ...
    public static IEnumerable<ProColor> GenerateTintShadePalette(ProColor baseColor, int numberOfColors = 5, double tintStep = 0.075, double shadeStep = 0.075)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(numberOfColors);
