...
namespace ProtonBlazor
{
    ...
    public abstract class ProBaseInput<T> : ProFormComponent<T, string>
    {
        ...
        protected internal virtual async Task OnBlurredAsync(FocusEventArgs obj)
        {
            ...
            // all the OnBlur parents (TextField, ProMask, NumericField, DateRange, etc) currently point to this method
        ...
        protected override void OnParametersSet()
        {
            ...
            {
                // ProtonBlazor uses an unconventional SubscribeToParentForm mechanism whose behavior is not fully understandable.
        ...
        protected string? GetHelperId()
        {
            ...
            // error text replaces helper text in ProInputControl, so if the user does not provide a custom helper id, we have no valid helper element
            if (HasErrors)
            {
                return null;
            }
