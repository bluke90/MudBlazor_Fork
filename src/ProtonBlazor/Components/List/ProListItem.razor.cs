...
namespace ProtonBlazor
{
    ...
    /// <summary>
    /// An item within a <see cref="ProList{T}"/> component.
    /// </summary>
    /// <typeparam name="T">The type of item being listed.</typeparam>
    /// <seealso cref="ProList{T}"/>
    /// <seealso cref="ProListSubheader"/>
    public partial class ProListItem<T> : ProComponentBase, IDisposable
    {
        private bool _selected;
        private bool MultiSelection => ProList?.SelectionMode == SelectionMode.MultiSelection;
        ...
        protected string Classname =>
            new CssBuilder("pro-list-item")
                .AddClass("pro-list-item-dense", GetDense())
                .AddClass("pro-list-item-gutters", GetGutters())
                .AddClass("pro-list-item-clickable", GetClickable())
                .AddClass("pro-ripple", Ripple && GetClickable())
                .AddClass($"pro-selected-item pro-{ProList?.Color.ToStringFast(true)}-text", !MultiSelection && _selected && !GetDisabled())
                .AddClass($"pro-{ProList?.Color.ToStringFast(true)}-hover", !MultiSelection && _selected && !GetDisabled())
                .AddClass("pro-list-item-disabled", GetDisabled())
                .AddClass(Class)
                .Build();
        ...
        [CascadingParameter]
        protected ProList<T>? ProList { get; set; }

        private ProList<T>? TopLevelList => ProList?.TopLevelList;
        ...
        /// <summary>
        /// The custom <see cref="ProAvatar" /> to display to the left of <see cref="Text"/>.
        /// </summary>
        /// <remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Defaults to <c>false</c>.  This value can be overridden by <see cref="ProList{T}.Disabled"/>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.List.Behavior)]
        ...
        /// </summary>
        /// <remarks>
        /// Defaults to the value of the parent <see cref="ProList{T}.Gutters"/>. When set, it overrides the list's setting.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.List.Appearance)]
        ...
        private IEqualityComparer<T?> Comparer => ProList?.Comparer ?? EqualityComparer<T?>.Default;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (ProList is not null)
            {
                await ProList.RegisterAsync(this);
            }
        }
        ...
        private bool CanHandleKeys() => !GetDisabled() && ProList is not null && ProList.IsInteractive() && TopLevelList is not null && TopLevelList.IsTabbable(this);

        private Task HandleArrowDownAsync() => ProList!.FocusAdjacentItemAsync(this, 1);

        private Task HandleArrowUpAsync() => ProList!.FocusAdjacentItemAsync(this, -1);

        private Task HandleHomeAsync() => ProList!.FocusBoundaryItemAsync(first: true);

        private Task HandleEndAsync() => ProList!.FocusBoundaryItemAsync(first: false);
        ...
        private bool GetDisabled() => Disabled || ProList?.GetDisabled() == true || TopLevelList?.GetDisabled() == true;

        private bool GetReadOnly() => ProList?.ReadOnly == true || TopLevelList?.GetReadOnly() == true;

        private bool GetDense() => Dense ?? (ProList?.Dense == true);

        private bool GetGutters() => Gutters ?? ProList?.Gutters ?? true;
        ...
        private string? GetTabIndex()
        {
            ...
            return ProList?.IsTabbable(this) == true ? "0" : "-1";
        }
