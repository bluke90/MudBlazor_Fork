...
namespace ProtonBlazor
{
    ...
    /// </summary>
    /// <remarks>
    /// This component contains an optional <see cref="ProListSubheader"/> and one or more <see cref="ProListItem{T}"/>.
    /// </remarks>
    /// <typeparam name="T">The type of item being listed.</typeparam>
    /// <seealso cref="ProListItem{T}"/>
    /// <seealso cref="ProListSubheader"/>
    public partial class ProList<T> : ProComponentBase, IDisposable
    {
        public ProList()
        {
            TopLevelList = this;
        ...
        private readonly List<ProListItem<T>> _items = [];
        private readonly HashSet<ProList<T>> _childLists = new();
        private HashSet<T> _selection = new();
        private ProListItem<T>? _activeItem;
        internal ProList<T> TopLevelList { get; private set; }

        protected string Classname =>
            new CssBuilder("pro-list")
                .AddClass("pro-list-padding", Padding)
                .AddClass(Class)
                .Build();

        [CascadingParameter]
        protected ProList<T>? ParentList { get; set; }
        ...
        internal async Task RegisterAsync(ProListItem<T> item)
        {
        ...
        internal void Unregister(ProListItem<T> item)
        {
        ...
        internal void Register(ProList<T> child)
        {
            _childLists.Add(child);
        }

        internal void Unregister(ProList<T> child)
        {
        ...
        {
            ProListItem<T>? selectedItem = null;
        ...
        internal bool IsTabbable(ProListItem<T> item)
        {
        ...
        internal void SetActiveItem(ProListItem<T> item)
        {
            if (TopLevelList != this)
            {
        ...
        private ProListItem<T>? EnsureActiveItem()
        {
            if (TopLevelList != this)
            {
        ...
        /// </summary>
        /// <remarks>
        /// <see cref="ProList{T}"/> derives its container semantics from its selection behavior.
        /// </remarks>
