...
namespace ProtonBlazor
{
    ...
    public partial class ProButton : ProBaseButton, IDisposable
    {
        protected string Classname => new CssBuilder("pro-button-root pro-button")
            .AddClass($"pro-button-{Variant.ToStringFast(true)}")
            .AddClass($"pro-button-{Variant.ToStringFast(true)}-{Color.ToStringFast(true)}")
            .AddClass($"pro-button-{Variant.ToStringFast(true)}-size-{Size.ToStringFast(true)}")
            .AddClass($"pro-width-full", GetRealFullWith())
            .AddClass($"pro-ripple", Ripple)
            .AddClass($"pro-button-disable-elevation", !DropShadow)
            .AddClass(Class)
            .Build();

        protected string StartIconClass => new CssBuilder("pro-button-icon-start")
            .AddClass($"pro-button-icon-size-{(IconSize ?? Size).ToStringFast(true)}")
            .AddClass(IconClass)
            .Build();

        protected string EndIconClass => new CssBuilder("pro-button-icon-end")
            .AddClass($"pro-button-icon-size-{(IconSize ?? Size).ToStringFast(true)}")
            .AddClass(IconClass)
            .Build();
        ...
        /// </summary>
        [CascadingParameter]
        private ProButtonGroup? ButtonGroup { get; set; }
        ...
        internal bool GetRealFullWith()
        {
            ...
            // See https://github.com/ProtonBlazor/ProtonBlazor/issues/9710
