...
/// <summary>
/// A menu appearing from a <see cref="ProFab"/> that displays a list of items.
/// </summary>
/// <seealso cref="ProFabMenuItem" />
public partial class ProFabMenu : ProFab
{
    private new string Classname => new CssBuilder("pro-fab-menu-container")
        .AddClass("fixed", Fixed)
        .AddClass($"pro-fab-anchor-{Anchor.ToStringFast(true)}", Fixed)
        .AddClass($"align-{AlignItems.ToStringFast(true)}")
        .AddClass(Class)
        .Build();

    private string ClassnameMenu => new CssBuilder("pro-fab-menu")
        .AddClass("pro-fab-menu-open", _openState.Value)
        .AddClass("pro-fab-menu-dampen", DampenItemsBackgroundColor)
        .AddClass($"pro-fab-menu-direction-{Direction.ToStringFast(true)}")
        .AddClass($"align-{AlignItems.ToStringFast(true)}")
        .AddClass($"pro-fab-menu-{Size.ToString().ToLower()}", !string.IsNullOrEmpty(Label))
        .AddClass(MenuClass)
        .Build();

    private string ClassnameFab => new CssBuilder("pro-fab-menu-button")
    ...
    /// <summary>
    /// The <see cref="ProFabMenuItem" /> components within this menu.
    /// </summary>
    /// <remarks>
    /// Note that you can add any component you like as long as it has the <c>pro-fab-menu-item</c> class.
    /// </remarks>
