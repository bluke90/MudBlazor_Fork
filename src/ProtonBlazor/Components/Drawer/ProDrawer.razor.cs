...
namespace ProtonBlazor
{
    ...
    /// </summary>
    /// <seealso cref="ProDrawerContainer"/>
    /// <seealso cref="ProDrawerHeader"/>
    public partial class ProDrawer : ProComponentBase, INavigationEventReceiver, IBrowserViewportObserver, IAsyncDisposable
    {
        ...
        public ProDrawer()
        {
            ...
            registerScope.RegisterParameter<DrawerClipMode>(nameof(ClipMode))
        ...
        protected string Classname =>
            new CssBuilder("pro-drawer")
                .AddClass($"pro-drawer-fixed", IsFixed)
                .AddClass($"pro-drawer-pos-{GetPosition()}")
                .AddClass($"pro-drawer--open", _openState.Value)
                .AddClass($"pro-drawer--closed", !_openState.Value)
                .AddClass($"pro-drawer--initial", _initial)
                .AddClass($"pro-drawer-{Breakpoint.ToStringFast(true)}")
                .AddClass($"pro-drawer-clipped-{ClipMode.ToStringFast(true)}")
                .AddClass($"pro-theme-{Color.ToStringFast(true)}", Color != Color.Default)
                .AddClass($"pro-elevation-{Elevation}")
                .AddClass($"pro-drawer-{Variant.ToStringFast(true)}")
                .AddClass(Class)
                .Build();

        protected string OverlayClass =>
            new CssBuilder("pro-drawer-overlay pro-overlay-drawer")
                .AddClass($"pro-drawer-pos-{GetPosition()}")
                .AddClass($"pro-drawer-overlay--open", _openState.Value)
                .AddClass($"pro-drawer-overlay-{Variant.ToStringFast(true)}")
                .AddClass($"pro-drawer-overlay-{Breakpoint.ToStringFast(true)}")
                .AddClass($"pro-drawer-overlay--initial", _initial)
                .AddClass($"pro-skip-overlay-positioning") // popovers try to position the overlay by zindex, this skips that behavior
                .AddClass($"pro-skip-overlay-section") // drawer overlay remains outside of Section
                .Build();

        protected string Stylename =>
            new StyleBuilder()
                .AddStyle("--pro-drawer-width", Width, !string.IsNullOrWhiteSpace(Width) && (!IsFixed || Variant == DrawerVariant.Temporary))
                .AddStyle("height", Height, !string.IsNullOrWhiteSpace(Height))
                .AddStyle("--pro-drawer-height", string.IsNullOrWhiteSpace(Height) ? _height.ToPx() : Height, Anchor == Anchor.Bottom || Anchor == Anchor.Top)
        ...
        [CascadingParameter]
        private ProDrawerContainer? DrawerContainer { get; set; }
        ...
        /// <summary>
        /// The position of this drawer when opened, relative to a <see cref="ProAppBar"/> when inside a <see cref="ProLayout"/>.
        /// </summary>
        [Parameter, ParameterState(ParameterUsage = ParameterUsageOptions.None)]
        [Category(CategoryTypes.Drawer.Behavior)]
        ...
        {
            _height = (await _contentRef.ProGetBoundingClientRectAsync())?.Height ?? 0;
        }
        ...
        internal bool IsFixed => Fixed && DrawerContainer is ProLayout;
        ...
        async Task IBrowserViewportObserver.NotifyBrowserViewportChangeAsync(BrowserViewportEventArgs browserViewportEventArgs)
        {
            ...
            bool HandleBreakpointNone() => Breakpoint == Breakpoint.None;
            bool HandleBreakpointAlways() => Breakpoint == Breakpoint.Always;
        ...
        {
            // Historically, ProDrawer only functioned with breakpoints like Xs, Sm, Md, Lg, Xl, and Xxl.
