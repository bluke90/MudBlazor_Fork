...
namespace ProtonBlazor
{
    ...
    /// <remarks>
    /// Note that ProMask is recommended to be used in WASM projects only because it has known problems
    /// in BSS, especially with high network latency.
    /// </remarks>
    /// </summary>
    public partial class ProMask : ProBaseInput<string>
    {
        private int _caret;
        ...
        protected string Classname =>
            new CssBuilder("pro-input")
                .AddClass($"pro-input-{Variant.ToStringFast(true)}")
                .AddClass($"pro-input-{Variant.ToStringFast(true)}-with-label", !string.IsNullOrEmpty(Label))
                .AddClass($"pro-input-adorned-{Adornment.ToStringFast(true)}", Adornment != Adornment.None)
                .AddClass($"pro-input-margin-{Margin.ToStringFast(true)}", () => Margin != Margin.None)
                .AddClass("pro-input-underline", () => Underline && Variant != Variant.Outlined)
                .AddClass("pro-shrink", () => !string.IsNullOrEmpty(ReadText) || Adornment == Adornment.Start || !string.IsNullOrWhiteSpace(Placeholder) || ShrinkLabel)
                .AddClass("pro-disabled", GetDisabledState())
                .AddClass("pro-input-error", HasErrors)
                .AddClass("pro-ltr", GetInputType() == InputType.Email || GetInputType() == InputType.Telephone)
                .AddClass($"pro-typography-{Typo.ToStringFast(true)}")
                .AddClass(Class)
                .Build();
        ...
        protected string InputClassname =>
            new CssBuilder("pro-input-slot")
                ...
                .AddClass($"pro-input-root-{Variant.ToStringFast(true)}")
                .AddClass($"pro-input-root-adorned-{Adornment.ToStringFast(true)}", Adornment != Adornment.None)
                .AddClass($"pro-input-root-margin-{Margin.ToStringFast(true)}", () => Margin != Margin.None)
                .AddClass(Class)
                .Build();

        protected string AdornmentClassname =>
            new CssBuilder()
                .AddClass($"pro-input-adornment-{Adornment.ToStringFast(true)}", Adornment != Adornment.None)
                .AddClass($"pro-text", !string.IsNullOrEmpty(AdornmentText))
                .AddClass($"pro-input-root-filled-shrink", Variant == Variant.Filled)
                .AddClass(Class)
                .Build();

        protected string ClearButtonClassname =>
            new CssBuilder("pro-input-clear-button")
                // .AddClass("me-n1", Adornment == Adornment.End && HideSpinButtons == false)
                .AddClass("pro-icon-button-edge-end", Adornment == Adornment.End)
                // .AddClass("me-6", Adornment != Adornment.End && HideSpinButtons == false)
                .AddClass("pro-icon-button-edge-margin-end", Adornment != Adornment.End)
                .Build();
        ...
        /// When set, some properties will be ignored such as <see cref="ProInput{T}.MaxLines"/>, <see cref="ProInput{T}.Sizing"/>, and <see cref="ProInput{T}.HideSpinButtons"/>.
        /// </remarks>
        [Parameter]
        ...
        {
            return _elementReference.ProSelectAsync();
        }
        ...
        {
            return _elementReference.ProSelectRangeAsync(pos1, pos2);
        }
        ...
        private async Task SetCaretPositionAsync(int caret, (int, int)? selection = null)
        {
            ...
            if (selection == null)
            {
                await _elementReference.ProSelectRangeAsync(caret, caret);
            }
            else
            {
                var sel = selection.Value;
                await _elementReference.ProSelectRangeAsync(sel.Item1, sel.Item2);
            }
        }
