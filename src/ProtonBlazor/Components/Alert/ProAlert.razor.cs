...
namespace ProtonBlazor
{
    ...
    public partial class ProAlert : ProComponentBase
    {
        protected string Classname => new CssBuilder("pro-alert")
            .AddClass($"pro-alert-{Variant.ToStringFast(true)}-{Severity.ToStringFast(true)}")
            .AddClass($"pro-dense", Dense)
            .AddClass($"pro-square", Square)
            .AddClass($"pro-elevation-{Elevation}")
            .AddClass(Class)
            .Build();

        protected string ClassPosition => new CssBuilder("pro-alert-position")
        ...
        /// </summary>
        [Parameter]
        public EventCallback<ProAlert> CloseIconClicked { get; set; }
