...
{
    /// <summary>
    /// An expandable branch of a <see cref="ProTreeView{T}"/>.
    /// </summary>
    /// <typeparam name="T">The type of the selectable value held by the item.</typeparam>
    /// <remarks>
    /// Used as the data model of the tree.
    /// </remarks>
    /// <seealso cref="ProTreeView{T}"/>
    /// <seealso cref="ProTreeViewItemToggleButton"/>
    public partial class ProTreeViewItem<T> : ProComponentBase, IDisposable
    {
        ...
        private readonly HashSet<ProTreeViewItem<T>> _childItems = new();

        public ProTreeViewItem()
        {
        ...
        protected string Classname =>
            new CssBuilder("pro-treeview-item")
                .AddClass("pro-treeview-select-none", GetExpandOnDoubleClick)
                .AddClass("pro-treeview-item-disabled", GetDisabled())
                .AddClass(Class)
                .Build();

        protected string ContentClassname =>
            new CssBuilder("pro-treeview-item-content")
                .AddClass("cursor-pointer", !GetDisabled() && (!GetReadOnly() || (GetExpandOnClick() && HasChildren())))
                .AddClass("pro-ripple", GetRipple() && !GetDisabled() && !GetExpandOnDoubleClick() && (!GetReadOnly() || (GetExpandOnClick() && HasChildren())))
                .AddClass("pro-treeview-item-selected", !GetDisabled() && !MultiSelection && _selectedState)
                .Build();

        public string TextClassname =>
            new CssBuilder("pro-treeview-item-label")
                .AddClass(TextClass)
                .Build();

        private bool MultiSelection => ProTreeRoot?.MultiSelection == true;

        [CascadingParameter]
        private ProTreeView<T>? ProTreeRoot { get; set; }

        [CascadingParameter]
        internal ProTreeViewItem<T>? Parent { get; set; }

        // When the item comes from ItemTemplate, this links the component instance to its backing node object.
        [CascadingParameter(Name = ProTreeViewCascadingValues.ItemData)]
        ...
        /// </summary>
        /// <remarks>
        /// Defaults to <c>true</c>. A value of <c>false</c> is typically used for lazy-loaded items via <see cref="ProTreeView{T}.ServerData" />.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.TreeView.Behavior)]
        ...
        /// </summary>
        /// <remarks>
        /// Must be one or more <see cref="ProTreeViewItem{T}"/> components. Only applies when <see cref="Content"/> is not set.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.TreeView.Data)]
        ...
        /// </summary>
        /// <remarks>
        /// When set, completely controls the rendering of child items. For <see cref="ProTreeViewItem{T}"/> children, use <see cref="Items"/> or <see cref="ChildContent"/>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.TreeView.Behavior)]
        ...
        [Parameter]
        [Category(CategoryTypes.TreeView.Behavior)]
        public RenderFragment<ProTreeViewItem<T>>? BodyContent { get; set; }
        ...
        /// </summary>
        /// <remarks>
        /// Defaults to <c>false</c>. Can be set alongside other items if <see cref="ProTreeView{T}.SelectionMode"/> is <see cref="SelectionMode.MultiSelection"/> or <see cref="SelectionMode.ToggleSelection"/>.
        /// </remarks>
        [Parameter, ParameterState]
        ...
        private string CheckedIcon => ProTreeRoot?.CheckedIcon ?? Icons.Material.Filled.CheckBox;

        private string UncheckedIcon => ProTreeRoot?.UncheckedIcon ?? Icons.Material.Filled.CheckBoxOutlineBlank;

        private string IndeterminateIcon => ProTreeRoot?.IndeterminateIcon ?? Icons.Material.Filled.IndeterminateCheckBox;
        ...
        private bool HasChildren()
        {
            return ChildContent != null
                || (ProTreeRoot != null && GetItems().Count != 0)
                || (ProTreeRoot?.ServerData != null && CanExpand && !GetServerDataLoaded() && GetItems().Count == 0);
        }
        ...
        private bool GetDisabled() => Disabled || ProTreeRoot?.Disabled == true;

        private bool GetServerDataLoaded()
        {
            if (CurrentItemData is not null && ProTreeRoot is not null)
            {
                return ProTreeRoot.GetServerDataLoaded(CurrentItemData);
            }
        ...
        private void SetServerDataLoaded(bool isLoaded)
        {
            if (CurrentItemData is not null && ProTreeRoot is not null)
            {
                ProTreeRoot.SetServerDataLoaded(CurrentItemData, isLoaded);
                return;
            }
        ...
        protected override void OnParametersSet()
        {
            ...
            if (Text == null && Value == null && ProTreeRoot?.ServerData != null)
                throw new InvalidOperationException(
                    $"'{nameof(ProTreeView<T>)}.{nameof(ProTreeRoot.ServerData)}' requires '{nameof(ProTreeRoot.ItemTemplate)}.{nameof(ProTreeViewItem<T>)}.{nameof(Value)}' to be supplied.");
        }
        ...
        private async Task OnCheckboxChangedAsync()
        {
            ...
            await ProTreeRoot.OnItemClickAsync(this);
        }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            {
                if (ProTreeRoot is not null)
                {
                    await ProTreeRoot.AddChildAsync(this);
                }
            }
        ...
        private Task OnSelectedParameterChangedAsync(ParameterChangedEventArgs<bool> arg)
        {
            ...
            if (selected)
            {
                return ProTreeRoot.SelectAsync(value);
            }
            return ProTreeRoot.UnselectAsync(value);
        }

        private bool GetReadOnly() => ReadOnly || ProTreeRoot?.ReadOnly == true;

        private bool GetExpandOnClick() => ProTreeRoot?.ExpandOnClick == true;

        private bool GetExpandOnDoubleClick() => ProTreeRoot?.ExpandOnDoubleClick == true;

        private bool GetRipple() => ProTreeRoot?.Ripple == true;

        private bool GetAutoExpand() => ProTreeRoot?.AutoExpand == true;
        ...
        private void AddChild(ProTreeViewItem<T> item) => _childItems.Add(item);

        private void RemoveChild(ProTreeViewItem<T> item) => _childItems.Remove(item);

        internal List<ProTreeViewItem<T>> ChildItems => _childItems.ToList();
        ...
        internal IEnumerable<ProTreeViewItem<T>> GetSelectedItems()
        {
            if (_selectedState)
            {
                yield return this;
        ...
        {
            if (GetItems().Count != 0 || !CanExpand || ProTreeRoot?.ServerData == null)
                return;
            _loading = true;
            StateHasChanged();
            var loaded = false;
            try
            {
                var items = await ProTreeRoot.ServerData(GetValue());
        ...
        public void Dispose()
        {
            ProTreeRoot?.RemoveChild(this);
            Parent?.RemoveChild(this);
        }

        internal List<ProTreeViewItem<T?>> GetChildItemsRecursive(List<ProTreeViewItem<T?>>? list = null)
        {
            list ??= new List<ProTreeViewItem<T?>>();
            foreach (var child in _childItems)
            {
                list.Add(child!);
