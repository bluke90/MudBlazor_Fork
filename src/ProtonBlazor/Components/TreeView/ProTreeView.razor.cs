...
namespace ProtonBlazor
{
    ...
    /// <seealso cref="ProTreeViewItem{T}"/>
    /// <seealso cref="ProTreeViewItemToggleButton"/>
    public partial class ProTreeView<T> : ProComponentBase
    {
        public ProTreeView()
        {
            ProTreeRoot = this;
        ...
        protected string Classname =>
            new CssBuilder("pro-treeview")
                .AddClass("pro-treeview-dense", Dense)
                .AddClass("pro-treeview-hover", !Disabled && Hover && (!ReadOnly || ExpandOnClick))
                .AddClass($"pro-treeview-selected-{Color.ToStringFast(true)}")
                .AddClass($"pro-treeview-checked-{CheckBoxColor.ToStringFast(true)}")
                .AddClass(Class)
                .Build();
        ...
        {
            if (firstRender && ProTreeRoot == this)
            {
        ...
        /// <summary>
        /// The internal filter logic that traverses the tree recursively and applies the <see cref="FilterFunc"/> to every item to set the <see cref="ProTreeViewItem{T}.Visible"/> property
        /// </summary>
        ...
        /// <summary>
        /// Resets the filter, so that all <see cref="ProTreeViewItem{T}.Visible"/> are set to true and the entire tree is visible.
        /// </summary>
        ...
        internal async Task OnItemClickAsync(ProTreeViewItem<T> clickedItem)
        {
            if (ReadOnly)
            {
                return;
            ...
            if (MultiSelection)
            {
                ...
                var allSelected = items.All(x => x.GetState<bool>(nameof(ProTreeViewItem<T>.Selected)));
            ...
            var selected = clickedItem.GetState<bool>(nameof(ProTreeViewItem<T>.Selected));
            if (ToggleSelection)
            {
        ...
        /// </summary>
        private void UpdateParentItem(ProTreeViewItem<T>? parentItem)
        {
        ...
        internal async Task AddChildAsync(ProTreeViewItem<T> item)
        {
            ...
            if (value is not null && item.GetState<bool>(nameof(ProTreeViewItem<T>.Selected)))
            {
        ...
        internal void RemoveChild(ProTreeViewItem<T> item)
        {
        ...
        // TODO: speed this up with caching
        private HashSet<T> GetChildValuesRecursive(IEnumerable<ProTreeViewItem<T>>? children = null, HashSet<T>? values = null)
        {
            values ??= new HashSet<T>(Comparer);
