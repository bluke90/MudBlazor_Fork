...
/// </remarks>
public partial class ProExitPrompt : ProComponentBase, IAsyncDisposable
{
    ...
    /// </summary>
    /// <remarks>
    /// When <c>false</c>, navigation inside the app uses a ProtonBlazor message box with <see cref="Title"/> and <see cref="Text"/>.
    ...
    private string TitleToDisplay => Title ?? Localizer[LanguageResource.ProExitPrompt_Title];

    private string TextToDisplay => Text ?? Localizer[LanguageResource.ProExitPrompt_Text];

    public ProExitPrompt()
    {
    ...
    private async Task<bool> IsNavigationAllowedAsync()
    {
        ...
        // Otherwise, use the ProtonBlazor confirmation dialog and allow navigation only on explicit confirmation.
        return await DialogService.ShowMessageBoxAsync(
            TitleToDisplay,
            TextToDisplay,
            Localizer[LanguageResource.ProExitPrompt_Exit],
            Localizer[LanguageResource.ProExitPrompt_Cancel]
        ) == true;
    }
    ...
    private async Task EnableAsync()
    {
        ...
        if (!success)
        {
            throw new InvalidOperationException($"Failed to enable {nameof(ProExitPrompt)} JS interop for prompt '{_promptId}'.");
        }
    }
