...
namespace ProtonBlazor
{
    ...
    public abstract partial class ProPicker<T> : ProFormComponent<T, string>
    {
        ...
        protected string PickerClassname =>
            new CssBuilder("pro-picker")
                .AddClass("pro-picker-inline", PickerVariant != PickerVariant.Static)
                .AddClass("pro-picker-static", PickerVariant == PickerVariant.Static)
                .AddClass("pro-rounded", PickerVariant == PickerVariant.Static && !_pickerSquare)
                .AddClass($"pro-elevation-{Elevation ?? 0}", PickerVariant != PickerVariant.Inline)
                .AddClass("pro-picker-input-button", !Editable && PickerVariant != PickerVariant.Static)
                .AddClass("pro-picker-input-text", Editable && PickerVariant != PickerVariant.Static)
        ...
        protected string PickerPaperClassname =>
            new CssBuilder("pro-picker")
                .AddClass("pro-picker-paper")
                .AddClass("pro-picker-view", PickerVariant == PickerVariant.Inline)
                .AddClass("pro-picker-open", Open && PickerVariant == PickerVariant.Inline)
                .AddClass("pro-picker-popover-paper", PickerVariant == PickerVariant.Inline)
                .AddClass("pro-dialog", PickerVariant == PickerVariant.Dialog)
                .Build();
        ...
        public RenderFragment<ProPicker<T>>? PickerActions { get; set; }
        ...
        protected internal ProTextField<string>? _inputReference;
        ...
        protected virtual async Task OnOpenedAsync()
        {
            ...
            {
                await _pickerInlineRef.ProChangeCssAsync(PickerInlineClassname);
            }
        ...
        // Since for ParameterState we don't want to read directly from the Text property, but we have other components that inherit from ProPicker
        // In future when all Pickers will use ParameterState, we can remove this.
        protected virtual string? ReadText => Text;

        // A proxy for components that will utilize ParameterState
        // Since for ParameterState we don't want to write directly from the Text property, but we have other components that inherit from ProPicker
        // In future when all Pickers will use ParameterState, we can remove this.
