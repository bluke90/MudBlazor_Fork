...
namespace ProtonBlazor
{
    ...
    public partial class ProFileUpload<T> : ProFormComponent<T, string>
    {
        ...
        protected string Classname =>
            new CssBuilder("pro-file-upload")
                .AddClass(Class)
                .Build();

        protected string DragClass =>
            new CssBuilder("pro-file-upload-dragarea")
                .AddClass("pro-file-upload-dragarea-clickable", !GetDisabledState())
                .AddClass("pro-border-primary", _draggingState.Value)
                .Build();

        protected string InputClasses =>
            new CssBuilder(InputClass)
                .AddClass("pro-file-upload-dragover", DragAndDrop && (Hidden || CustomContent == null))
                .Build();
        ...
        /// </summary>
        /// <remarks>
        /// The context is the current <see cref="ProFileUpload{T}"/> instance and can be used to call <see cref="OpenFilePickerAsync"/>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.FileUpload.Behavior)]
        public RenderFragment<ProFileUpload<T>>? CustomContent { get; set; }
        ...
        /// </remarks>
        [Obsolete("Prefer the InputClass property with CSS https://github.com/ProtonBlazor/ProtonBlazor/issues/12047")]
        [Parameter]
        [Category(CategoryTypes.FileUpload.Appearance)]
        ...
        private async Task ProcessFileChangeAsync(InputFileChangeEventArgs args)
        {
            ...
            if (args.FileCount > MaximumFileCount)
            {
                ...
                _validationErrors.Add(Localizer[LanguageResource.ProFileUpload_MaximumFileCountExceeded, args.FileCount, MaximumFileCount]);
        ...
        private IReadOnlyList<IBrowserFile> ProcessMultipleFiles(IReadOnlyCollection<IBrowserFile> files)
        {
            ...
            foreach (var file in files)
            {
                ...
                {
                    _validationErrors.Add(Localizer[LanguageResource.ProFileUpload_FileSizeError, file.Name, MaxFileSize.Value.ToString()]);
                }
                else
                {
                    validFiles.Add(file);
