...
{
    partial class HeatMap<T> : ProChartBase<T, HeatMapChartOptions>, IDisposable where T : struct, INumber<T>, IMinMaxValue<T>, IFormattable
    {
        ...
        internal List<ProHeatMapCell<T>> _customHeatMapCells = [];
        ...
        /// </summary>
        [CascadingParameter]
        public ProChart<T>? ProChartParent { get; set; }
        ...
        protected override void OnParametersSet()
        {
            ...
            UpdateHeatMapCells(ProHeatMapCells);
        ...
        private void UpdateHeatMapCells(List<ProHeatMapCell<T>> mudHeatMapCellsList)
        {
            ...
            for (var row = 0; row < rows; row++)
            {
                for (var col = 0; col < cols; col++)
                {
                    ...
                    _heatMapCells.Add(new HeatMapCell<T>
                    {
                        ...
                        ProColor = mudHeatMapOverride?.ProColor,
                    });
                    if (value.HasValue)
                    {
        ...
        private void BuildLegends()
        {
            ...
            for (var i = 0; i < colors.Length; i++)
            {
                ...
                _legends.Add((value, colors[i].ToString(ProColorOutputFormats.RGB)));
            }
        }
        ...
        private ProColor[] GetEqualizedColorPalette(int shadeCount)
        {
            var baseColors = _colorPalette.Select(x => new ProColor(x)).ToArray();
            var colorCount = baseColors.Length;

            if (colorCount == 1)
            {
                return ProColor.GenerateTintShadePalette(baseColors[0]).ToArray();
            }

            if (colorCount != 5)
            {
                return ProColor.GenerateMultiGradientPalette(baseColors, shadeCount).ToArray();
            }
        ...
        internal List<ProHeatMapCell<T>> ProHeatMapCells { get; set; } = [];

        internal void AddCell(ProHeatMapCell<T> cell)
        {
            ProHeatMapCells.Add(cell);
