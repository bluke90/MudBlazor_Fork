...
namespace ProtonBlazor
{
    ...
    public partial class ProRadioGroup<T> : ProFormComponent<T, T>, IProRadioGroup
    {
        ...
        protected string Classname =>
            new CssBuilder("pro-input-control-boolean-input")
                .AddClass(Class)
                .Build();

        private string GetInputClass() =>
            new CssBuilder("pro-radio-group")
                .AddClass(InputClass)
                .Build();
        ...
        /// </summary>
        /// <remarks>
        /// Usually a set of <see cref="ProRadio{T}"/> components.
        /// </remarks>
        [Parameter]
        ...
        internal bool GetDisabledState() => Disabled || ParentDisabled; //internal because the ProRadio reads this value directly

        internal bool GetReadOnlyState() => ReadOnly || ParentReadOnly; //internal because the ProRadio reads this value directly
        ...
        public void CheckGenericTypeMatch(object selectItem)
        {
            ...
            if (itemT != typeof(T))
            {
                throw new GenericTypeMismatchException("ProRadioGroup", "ProRadio", typeof(T), itemT);
            }
        }

        internal Task SetSelectedRadioAsync(ProRadio<T> radio)
        {
            Touched = true;
            return SetSelectedRadioAsync(radio, true);
        }

        protected async Task SetSelectedRadioAsync(ProRadio<T>? radio, bool updateOption)
        {
        ...
        internal Task RegisterRadioAsync(ProRadio<T> radio)
        {
            _radios.Add(radio);
        ...
        internal void UnregisterRadio(ProRadio<T> radio)
        {
        ...
        private static T? GetValueOrDefault(ProRadio<T>? radio)
        {
