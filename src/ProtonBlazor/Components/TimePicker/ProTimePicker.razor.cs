...
namespace ProtonBlazor
{
    ...
    /// </summary>
    /// <seealso cref="ProDatePicker"/>
    /// <seealso cref="ProDateRangePicker"/>
    public partial class ProTimePicker : ProPicker<TimeSpan?>
    {
        private OpenTo _currentView;
        private string? _clockElementReferenceId;
        private readonly SetTime _timeSet = new();
        private readonly Lazy<DotNetObjectReference<ProTimePicker>> _dotNetReferenceLazy;
        ...
        public ProTimePicker()
        {
            AdornmentIcon = Icons.Material.Filled.AccessTime;
            _dotNetReferenceLazy = new Lazy<DotNetObjectReference<ProTimePicker>>(CreateDotNetObjectReference);
        ...
        protected string ToolbarClassname =>
            new CssBuilder("pro-picker-timepicker-toolbar")
                .AddClass("pro-picker-timepicker-toolbar-landscape", Orientation == Orientation.Landscape && PickerVariant == PickerVariant.Static)
                .AddClass(Class)
                .Build();

        protected string HoursButtonClassname =>
            new CssBuilder("pro-timepicker-button")
                .AddClass("pro-timepicker-toolbar-text", _currentView == OpenTo.Minutes)
                .Build();

        protected string MinuteButtonClassname =>
            new CssBuilder("pro-timepicker-button")
                .AddClass("pro-timepicker-toolbar-text", _currentView == OpenTo.Hours)
                .Build();

        protected string AmButtonClassname =>
            new CssBuilder("pro-timepicker-button")
                .AddClass("pro-timepicker-toolbar-text", !IsAm) // gray it out.
                .Build();

        protected string PmButtonClassname =>
            new CssBuilder("pro-timepicker-button")
                .AddClass("pro-timepicker-toolbar-text", !IsPm) // gray it out.
                .Build();

        private string HourDialClassname =>
            new CssBuilder("pro-time-picker-hour")
                .AddClass("pro-time-picker-dial")
                .AddClass("pro-time-picker-dial-out", _currentView != OpenTo.Hours)
                .AddClass("pro-time-picker-dial-hidden", _currentView != OpenTo.Hours)
                .Build();

        private string MinuteDialClassname =>
            new CssBuilder("pro-time-picker-minute")
                .AddClass("pro-time-picker-dial")
                .AddClass("pro-time-picker-dial-out", _currentView != OpenTo.Minutes)
                .AddClass("pro-time-picker-dial-hidden", _currentView != OpenTo.Minutes)
                .Build();
        ...
        {
            return $"pro-picker-time-clock-pin pro-{Color.ToStringFast(true)}";
        }

        private string GetClockPointerColor()
        {
            return PointerMoving
                ? $"pro-picker-time-clock-pointer pro-{Color.ToStringFast(true)}"
                : $"pro-picker-time-clock-pointer pro-picker-time-clock-pointer-animation pro-{Color.ToStringFast(true)}";
        }
        ...
        private string GetClockPointerThumbColor()
        {
            var deg = GetDeg();
            return deg % 30 == 0
                ? $"pro-picker-time-clock-pointer-thumb pro-onclock-text pro-onclock-primary pro-{Color.ToStringFast(true)}"
                : $"pro-picker-time-clock-pointer-thumb pro-onclock-minute pro-{Color.ToStringFast(true)}-text";
        }
        ...
        private string GetNumberColor(int value)
        {
            if (_currentView == OpenTo.Hours)
            {
                ...
                if (h == value)
                {
                    return $"pro-clock-number pro-theme-{Color.ToStringFast(true)}";
                }
            }
        ...
        {
            base.OnInitialized();
            AdornmentAriaLabel ??= Localizer[LanguageResource.ProTimePicker_Open];
            UpdateTimeSetFromTime();
            _currentView = OpenTo;
        }
        ...
        private DotNetObjectReference<ProTimePicker> CreateDotNetObjectReference() => DotNetObjectReference.Create(this);
