...
/// <seealso cref="ProChipSet{T}"/>
public partial class ProChip<T> : ProComponentBase, IAsyncDisposable
{
    [Inject]
    ...
    protected string Classname => new CssBuilder("pro-chip")
        .AddClass($"pro-chip-{GetVariant().ToStringFast(true)}")
        .AddClass($"pro-chip-size-{GetSize().ToStringFast(true)}")
        .AddClass($"pro-chip-color-{GetColor().ToStringFast(true)}")
        .AddClass("pro-clickable", IsButton || IsAnchor)
        .AddClass("pro-ripple", IsButton && GetRipple())
        .AddClass("pro-chip-label", GetLabel())
        .AddClass("pro-disabled", GetDisabled())
        .AddClass("pro-chip-selected", SelectedState.Value)
        .AddClass(Class)
        .Build();
    ...
    {
        var chipSetVariant = ChipSet?.Variant ?? ProtonBlazor.Variant.Filled;
        var variant = Variant ?? chipSetVariant;
        return variant switch
        {
            ProtonBlazor.Variant.Text => SelectedState.Value ? ProtonBlazor.Variant.Filled : ProtonBlazor.Variant.Text,
            ProtonBlazor.Variant.Filled => SelectedState.Value ? ProtonBlazor.Variant.Text : ProtonBlazor.Variant.Filled,
            ProtonBlazor.Variant.Outlined => ProtonBlazor.Variant.Outlined,
            _ => ProtonBlazor.Variant.Outlined
        };
    }
    ...
    private Color GetColor()
    {
        ...
        if (SelectedState.Value && selectedColor != ProtonBlazor.Color.Inherit)
        {
            return selectedColor;
        }
        return Color ?? ChipSet?.Color ?? ProtonBlazor.Color.Default;
    }

    private Color GetSelectedColor() => SelectedColor ?? ChipSet?.SelectedColor ?? ProtonBlazor.Color.Inherit;

    private Color GetIconColor() => IconColor ?? ChipSet?.IconColor ?? ProtonBlazor.Color.Inherit;

    private Size GetSize() => Size ?? ChipSet?.Size ?? ProtonBlazor.Size.Medium;
    ...
    [CascadingParameter]
    private ProChipSet<T>? ChipSet { get; set; }
    ...
    /// </summary>
    /// <remarks>
    /// When part of a <see cref="ProChipSet{T}"/>, the <see cref="ProChipSet{T}.SelectedValue"/> is set to this value when the chip is selected.  Once set, the value should not change.
    /// </remarks>
    [Parameter]
    [Category(CategoryTypes.Chip.Behavior)]
    ...
    /// <summary>
    /// Selects this chip by default when part of a <see cref="ProChipSet{T}"/>.
    /// </summary>
    /// <remarks>
    /// Defaults to <c>null</c>.
    /// </remarks>
