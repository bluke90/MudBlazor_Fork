...
namespace ProtonBlazor
{
    ...
    /// <seealso cref="ProToggleItem{T}"/>
    /// <seealso cref="ProRadioGroup{T}"/>
    /// <seealso cref="ProRadio{T}"/>
    /// <seealso cref="ProButtonGroup"/>
    public partial class ProToggleGroup<T> : ProComponentBase
    {
        public ProToggleGroup()
        {
        ...
        private readonly List<ProToggleItem<T>> _items = new();

        protected string Classname => new CssBuilder("pro-toggle-group")
            .AddClass("pro-toggle-group-horizontal", !Vertical)
            .AddClass("pro-toggle-group-vertical", Vertical)
            .AddClass($"pro-toggle-group-size-{Size.ToStringFast(true)}")
            .AddClass("pro-toggle-group-rtl", RightToLeft)
            .AddClass($"pro-toggle-group-{Color.ToStringFast(true)}")
            .AddClass("pro-toggle-group-outlined", Outlined)
        ...
        /// <summary>
        /// Contains the <see cref="ProToggleItem{T}"/> components of this group.
        /// </summary>
        [Parameter]
        [Category(CategoryTypes.List.Behavior)]
        ...
        protected internal void Register(ProToggleItem<T> item)
        {
        ...
        protected internal void Unregister(ProToggleItem<T> item)
        {
        ...
        private void ApplySelectionState(ProToggleItem<T> item)
        {
        ...
        protected internal async Task ToggleItemAsync(ProToggleItem<T> item)
        {
        ...
        protected internal IEnumerable<ProToggleItem<T>> GetItems() => _items;

        protected internal bool IsFirstItem(ProToggleItem<T> item) => item.Equals(_items.FirstOrDefault());

        protected internal bool IsLastItem(ProToggleItem<T> item) => item.Equals(_items.LastOrDefault());
    }
}
