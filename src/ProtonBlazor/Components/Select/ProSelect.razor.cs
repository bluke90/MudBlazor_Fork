...
namespace ProtonBlazor
{
    ...
    public partial class ProSelect<T> : ProBaseInput<T>, IMudSelect, IMudShadowSelect
    {
        ...
        private ProSelectItem<T>? _longestItem;
        private bool _needsHighlightAfterRender;
        private bool _needsFitContentRefresh;
        private ProInput<string> _elementReference = null!;
        ...
        private readonly ProSelectContext<T> _context;
        ...
        public ProSelect()
        {
            _context = new ProSelectContext<T>(this);
        ...
        protected string OuterClassname =>
            new CssBuilder("pro-select")
                .AddClass("pro-width-full", FullWidth)
                .AddClass("pro-width-content", FitContent && !FullWidth)
                .AddClass(OuterClass)
                .Build();
        ...
        /// <summary>
        /// The content within this component, typically a list of <see cref="ProSelectItem{T}"/> components.
        /// </summary>
        [Parameter]
        ...
        /// <summary>
        /// Restricts the selected values to the ones defined in <see cref="ProSelectItem{T}"/> items.
        /// </summary>
        /// <remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Use <see cref="ProSelectItem{T}"/> components to provide more items.
        /// </remarks>
        public IReadOnlyList<ProSelectItem<T>> Items => _context.Items;
        ...
        /// </summary>
        /// <remarks>
        /// <see cref="ProSelect{T}"/> keeps focus on its trigger while the popup is open, so the trigger publishes the combobox relationship to the popup list.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Provides the stable list identifier and list-level semantics owned by <see cref="ProSelect{T}"/>.
        /// </remarks>
        ...
        internal void UpdateFitContent()
        {
            if (FitContent)
            {
                ...
                var longestItem = default(ProSelectItem<T>);
        ...
        private Task HighlightItemAsync(ProSelectItem<T>? item)
        {
        ...
        private ValueTask ScrollToItemAsync(ProSelectItem<T>? item)
        ...
        {
            IReadOnlyCollection<ProSelectItem<T>> selectList = Items;
        ...
        private ProSelectItem<T>? SelectItemBySearch(IReadOnlyCollection<ProSelectItem<T>> items, string inputChar)
        {
            UpdateSearchText(inputChar);

            ProSelectItem<T>? activeItem = null;
            ProSelectItem<T>? previousItem = null;
            ProSelectItem<T>? firstMatch = null;
            ProSelectItem<T>? nextMatch = null;
            ...
            void TrackSpecialItems(ProSelectItem<T> item, ref ProSelectItem<T>? active, ref ProSelectItem<T>? previous)
            {
            ...
            bool IsMatch(ProSelectItem<T> item)
            {
                if (item.Disabled)
                {
                    return false;
            ...
            ProSelectItem<T>? DetermineResult(ProSelectItem<T>? active, ProSelectItem<T>? previous,
                                               ProSelectItem<T>? first, ProSelectItem<T>? next)
            {
                if (first is null)
                {
                    return active;
        ...
        private async Task SelectAndHighlightItemAsync(ProSelectItem<T> item)
        {
            if (!MultiSelection)
            {
                _selectedValues.Clear();
        ...
        /// <summary>
        /// Internal method for ProSelectItem to access the converted string value.
        /// </summary>
