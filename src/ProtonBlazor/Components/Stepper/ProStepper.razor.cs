...
/// </summary>
public partial class ProStepper : ProComponentBase
{
    public ProStepper()
    {
    ...
    private ProStep? _activeStep;
    private readonly ParameterState<int> _activeIndex;
    private readonly List<ProStep> _steps = [];
    private readonly string _componentId = Identifier.Create();

    protected string Classname =>
        new CssBuilder("pro-stepper")
            .AddClass("pro-stepper__horizontal", Vertical == false)
            .AddClass("pro-stepper__vertical", Vertical)
            .AddClass("pro-stepper__center-labels", CenterLabels && !Vertical)
            .AddClass(Class)
            .Build();

    internal string StepClassname =>
        new CssBuilder("pro-stepper-content")
            .AddClass(StepClass)
            .Build();

    protected string NavClassname =>
        new CssBuilder("pro-stepper-nav")
            .AddClass("pro-stepper-nav-scrollable", ScrollableNavigation)
            .AddClass(NavClass)
            .Build();

    internal string GetStepButtonId(ProStep step) => $"stepper-{_componentId}-tab-{step.FieldId}";

    internal string GetStepPanelId(ProStep step) => $"stepper-{_componentId}-tabpanel-{step.FieldId}";

    internal string? GetStepPanelIdIfRendered(ProStep step)
    {
    ...
    /// </summary>
    /// <remarks>
    /// Must be a set of <see cref="ProStep"/> components.  
    /// </remarks>
    [Parameter]
    [Category(CategoryTypes.List.Appearance)]
    ...
    /// </summary>
    /// <remarks>
    /// Defaults to <c>null</c>.  The current <see cref="ProStep"/> is passed as context for this render fragment.
    /// </remarks>
    [Parameter]
    [Category(CategoryTypes.List.Appearance)]
    ...
    [Parameter]
    [Category(CategoryTypes.List.Appearance)]
    public RenderFragment<ProStepper>? ActionContent { get; set; }

    internal async Task AddStepAsync(ProStep step)
    {
        _steps.Add(step);
    ...
    internal async Task RemoveStepAsync(ProStep step)
    {
        _steps.Remove(step);
        await ConsolidateActiveIndexAsync();
    }
    ...
    private async Task UpdateStepAsync(ProStep? step, MouseEventArgs ev, StepAction stepAction, bool ignoreDisabledState = false)
    {
    ...
    private ProStep? GetPreviousStep(int index)
    {
        ProStep? step = null;
    ...
    private ProStep? GetNextStep(int index)
    {
        ProStep? step = null;
        if (index < -1)
            index = -1;
    ...
    private Task OnStepClickAsync(ProStep step, MouseEventArgs e)
    {
