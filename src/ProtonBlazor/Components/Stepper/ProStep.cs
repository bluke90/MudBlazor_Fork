...
/// <summary>
/// A individual step as part of a <see cref="ProStepper"/>.
/// </summary>
public class ProStep : ProComponentBase, IStepContext, IAsyncDisposable
{
    public ProStep()
    {
    ...
    internal string LabelClassname =>
        new CssBuilder("pro-step-label")
            .AddClass("pro-step-label-active", IsActive)
            .Build();

    internal string LabelIconClassname =>
        new CssBuilder("pro-step-label-icon")
            .AddClass($"pro-{(CompletedStepColor.HasValue ? CompletedStepColor.Value.ToStringFast(true) : Parent?.CompletedStepColor.ToStringFast(true))}", CompletedState && !HasErrorState && Parent?.CompletedStepColor != Color.Default && (Parent?.ActiveStep != this || (Parent?.IsCompleted == true && Parent?.NonLinear == false)))
            .AddClass($"pro-{(ErrorStepColor.HasValue ? ErrorStepColor.Value.ToStringFast(true) : Parent?.ErrorStepColor.ToStringFast(true))}", HasErrorState)
            .AddClass($"pro-{(SkippedStepColor.HasValue ? SkippedStepColor.Value.ToStringFast(true) : Parent?.SkippedStepColor.ToStringFast(true))}", SkippedState)
            .AddClass($"pro-{Parent?.CurrentStepColor.ToStringFast(true)}", Parent?.ActiveStep == this && !(Parent?.IsCompleted == true && Parent?.NonLinear == false))
            .Build();

    internal string LabelContentClassname =>
        new CssBuilder("pro-step-label-content")
            .AddClass($"pro-{(ErrorStepColor.HasValue ? ErrorStepColor.Value.ToStringFast(true) : Parent?.ErrorStepColor.ToStringFast(true))}-text", HasErrorState)
            .Build();
    ...
    [CascadingParameter]
    internal ProStepper? Parent { get; set; }
    ...
    /// Use the <see cref="ProStepContext"/> cascading parameter to access information about the current step inside the template.
    /// </remarks>
    [Parameter]
