...
namespace ProtonBlazor
{
    ...
    /// </summary>
    /// <seealso cref="ProTabPanel"/>
    public partial class ProTabs : ProComponentBase, IAsyncDisposable
    {
        internal List<ProTabPanel> _panels;
        ...
        private ProDropContainer<ProTabPanel>? _dropContainer;
        ...
        /// <summary>
        /// Unique identifier for this ProTabs component instance.
        ...
        /// </summary>
        /// <remarks>
        /// Controlled via the <see cref="ProRTLProvider"/> component.
        /// </remarks>
        ...
        /// The dropped item is provided in the <see cref="ProItemDropInfo{T}"/> and will have already been moved to its new position.
        /// </summary>
        [Parameter]
        public EventCallback<ProItemDropInfo<ProTabPanel>> OnItemDropped { get; set; }
        ...
        /// </summary>
        /// <remarks>
        /// Typically a set of <see cref="ProTabPanel"/> components.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.Tabs.Behavior)]
        ...
        [Parameter]
        [Category(CategoryTypes.Tabs.Behavior)]
        public RenderFragment<ProTabPanel>? PrePanelContent { get; set; }
        ...
        /// </summary>
        public ProTabPanel? ActivePanel => _activePanelIndexState.Value >= 0 && _activePanelIndexState.Value < _panels.Count ?
        ...
        /// </summary>
        /// <remarks>
        /// Tab panels are controlled by either adding more <see cref="ProTabPanel"/> components in the Razor page, or by using the <see cref="ProDynamicTabs"/> component instead.
        /// </remarks>
        public IReadOnlyList<ProTabPanel> Panels { get; private set; }
        ...
        [Parameter]
        [Category(CategoryTypes.Tabs.Behavior)]
        public RenderFragment<ProTabs>? Header { get; set; }
        ...
        [Parameter]
        [Category(CategoryTypes.Tabs.Behavior)]
        public RenderFragment<ProTabPanel>? TabPanelHeader { get; set; }
        ...
        /// <summary>
        /// Sort tab labels lexicographically by <see cref="ProTabPanel.Text"/> or <see cref="ProTabPanel.SortKey"/>. Ignored if <see cref="SortComparer" /> is set.
        /// </summary>
        /// <remarks>
        ...
        [Parameter]
        [Category(CategoryTypes.Tabs.Appearance)]
        public IComparer<ProTabPanel>? SortComparer { get; set; }
        ...
        internal async Task AddPanelAsync(ProTabPanel tabPanel)
        {
        ...
        internal async Task RemovePanel(ProTabPanel tabPanel)
        {
            if (_isDisposed)
                return;
        ...
        /// <param name="ignoreDisabledState">When <c>true</c>, the panel will be activated even if it is disabled.</param>
        public async Task ActivatePanelAsync(ProTabPanel? panel, bool ignoreDisabledState = false)
        {
            if (panel == null)
            {
                await _activePanelIndexState.SetValueAsync(-1);
        ...
        private async Task ActivatePanelClickAsync(ProTabPanel panel, MouseEventArgs ev, bool ignoreDisabledState = false)
        {
        ...
        private int GetTabSortExpression(ProTabPanel a, ProTabPanel b)
        {
        ...
        private static string? GetTabSortKey(ProTabPanel panel) => panel.SortKey ?? panel.Text;
        #endregion

        #region Style and classes

        protected string TabsClassnames =>
            new CssBuilder("pro-tabs")
                .AddClass($"pro-tabs-rounded", ApplyEffectsToContainer && Rounded)
                .AddClass($"pro-paper-outlined", ApplyEffectsToContainer && Outlined)
                .AddClass($"pro-elevation-{Elevation}", ApplyEffectsToContainer && Elevation != 0)
                .AddClass($"pro-tabs-reverse", Position == Position.Bottom)
                .AddClass($"pro-tabs-vertical", _isVerticalTabs)
                .AddClass($"pro-tabs-vertical-reverse", (Position == Position.Right && !RightToLeft) || ((Position == Position.Left) && RightToLeft) || Position == Position.End)
                .AddClass(InternalClassName)
                .AddClass(Class)
                .Build();

        protected string TabBarClassnames =>
            new CssBuilder("pro-tabs-tabbar")
                .AddClass($"pro-tabs-rounded", !ApplyEffectsToContainer && Rounded)
                .AddClass($"pro-tabs-vertical", _isVerticalTabs)
                .AddClass($"pro-tabs-tabbar-{Color.ToStringFast(true)}", Color != Color.Default)
                .AddClass($"pro-tabs-border-{ConvertPosition(Position).ToStringFast(true)}", Border)
                .AddClass($"pro-paper-outlined", !ApplyEffectsToContainer && Outlined)
                .AddClass($"pro-elevation-{Elevation}", !ApplyEffectsToContainer && Elevation != 0)
        ...
        protected string WrapperClassnames =>
            new CssBuilder("pro-tabs-tabbar-wrapper")
                .AddClass($"pro-tabs-centered", Centered)
                .AddClass($"pro-tabs-vertical", _isVerticalTabs)
                .Build();
        ...
        protected string PanelsClassnames =>
            new CssBuilder("pro-tabs-panels")
                .AddClass($"pro-tabs-vertical", _isVerticalTabs)
                .AddClass(TabPanelsClass)
                .Build();

        protected string SliderClass =>
            new CssBuilder("pro-tab-slider")
                .AddClass($"pro-{SliderColor.ToStringFast(true)}", SliderColor != Color.Inherit)
                .AddClass($"pro-tab-slider-horizontal", Position is Position.Top or Position.Bottom)
                .AddClass($"pro-tab-slider-vertical", _isVerticalTabs)
                .AddClass($"pro-tab-slider-horizontal-reverse", Position == Position.Bottom)
                .AddClass($"pro-tab-slider-vertical-reverse", Position == Position.Right || (Position == Position.Start && RightToLeft) || (Position == Position.End && !RightToLeft))
                .Build();

        protected string DropZoneClassnames =>
            new CssBuilder("pro-tabs-dropzone")
                .AddClass("d-flex", !_isVerticalTabs)
                .AddClass("pro-tabs-dropzone-horizontal", !_isVerticalTabs)
                .AddClass("pro-tabs-dropzone-vertical", _isVerticalTabs)
                .AddClass($"pro-tabs-vertical", _isVerticalTabs)
        ...
        private string GetTabClass(ProTabPanel panel)
        {
            var tabClass = new CssBuilder("pro-tab")
              .AddClass($"pro-tab-active", when: () => panel == ActivePanel)
              .AddClass($"pro-disabled", IsPanelDisabled(panel))
        ...
        private string GetTabStyle(ProTabPanel panel)
        {
        ...
        private Color GetPanelIconColor(ProTabPanel panel)
        {
        ...
        private bool IsPanelDisabled(ProTabPanel panel) => Disabled || panel.Disabled;
        ...
        /// <summary>
        /// this sets _tabBarContentSize to the total calculated width of the pro-tabs-tabbar-content
        /// </summary>
        ...
        private async Task GetAllReferenceSizes()
        {
            ...
            var tasks = panelRefs.Select(async panelRef => (panelRef, rect: await panelRef.ProGetBoundingClientRectAsync())).ToList();
        ...
        {
            // _tabSizes get current values using ProGetBoundingClientRectAsync
        ...
        /// </remarks>
        private double GetLengthOfPanelItems(ProTabPanel panel, bool inclusive = false)
        {
            var value = 0.0;
        ...
        private double GetPanelLength(ProTabPanel? panel) => panel == null ? 0.0 : GetRelevantSize(panel.PanelRef);

        private bool IsFirstVisiblePanel(ProTabPanel? activePanel)
        {
            foreach (var panel in _panels)
            {
        ...
        /// <summary>
        /// Scrolls a <see cref="ProTabPanel" /> to the center of the tab content viewport. Will not scroll beyond the bounds of tabs.
        /// </summary>
        private void ScrollToItem(ProTabPanel panel, bool isLast = false)
        {
            ...
            if (isLast)
            {
                // scroll so the right edge of the last tab is flush to the right edge of the pro-tabs-content (visible tab area)
                position = maxScroll;
            }
            else
            {
        ...
        internal async Task ItemUpdated(ProItemDropInfo<ProTabPanel> dropItem)
        {
        ...
        /// </summary>
        protected virtual async Task HandleTabKeyDownAsync(KeyboardEventArgs e, ProTabPanel panel)
        {
            switch (e.Key)
            {
                case "Enter":
        ...
        /// </summary>
        private async Task MoveFocusToPreviousTab(ProTabPanel currentPanel)
        {
        ...
        /// </summary>
        private async Task MoveFocusToNextTab(ProTabPanel currentPanel)
        {
            var enabledPanels = _panels.Where(p => !IsPanelDisabled(p)).ToList();
            if (enabledPanels.Count <= 1) return;
        ...
        /// </summary>
        private static async Task FocusPanel(ProTabPanel panel)
        {
        ...
        /// </summary>
        internal string GetTabId(ProTabPanel panel)
        {
        ...
        /// </summary>
        internal string GetTabPanelId(ProTabPanel panel)
        {
        ...
        {
            if (_isVerticalTabs)
                return Localizer[LanguageResource.ProTabs_ScrollUp];

            return RightToLeft
                ? Localizer[LanguageResource.ProTabs_ScrollRight]
                : Localizer[LanguageResource.ProTabs_ScrollLeft];
        }
        ...
        {
            if (_isVerticalTabs)
                return Localizer[LanguageResource.ProTabs_ScrollDown];
