...
/// </summary>
public partial class ProOverlay : ProComponentBase, IPointerEventsNoneObserver, IAsyncDisposable
{
    ...
    protected string Classname =>
        new CssBuilder("pro-overlay")
            .AddClass("pro-overlay-absolute", Absolute)
            .AddClass(Class)
            .Build();

    protected string ScrimClassname =>
        new CssBuilder("pro-overlay-scrim")
            .AddClass("pro-overlay-dark", DarkBackground)
            .AddClass("pro-overlay-light", LightBackground)
            .Build();
    ...
    /// <summary>
    /// Determines whether the overlay should be rendered outside of the section. If it's false, the overlay will be rendered with the ProPopOverProvider.
    /// If it's true it will be rendered as is where is (v7 and previous behavior)
    /// </summary>
    /// <remarks>
    /// If the user sets Absolute to true, the user intends for it to be part of his markup and not rendered by the ProPopoverProvider
    /// Dialog's need the separation of the overlay for display purposes
    /// If the user provides a child content, the user intends for it to be part of his markup and not rendered by the ProPopoverProvider
    /// </remarks>
    internal bool RenderOutsideOfSection =>
        Absolute ||
        (Class?.Contains("pro-skip-overlay-section") ?? false) ||
        ChildContent != null;
