...
{
    /// <summary>
    /// Collects and validates user input, monitoring and validating every input derived from ProFormComponent within it.
    /// </summary>
    public partial class ProForm : ProComponentBase, IDisposable, IForm
    {
        ...
        protected string Classname =>
            new CssBuilder("pro-form")
                .AddClass($"gap-{Spacing}", Spacing >= 0)
                .AddClass(Class)
                .Build();

        [CascadingParameter]
        private ProForm? ParentMudForm { get; set; }
        ...
        /// </summary>
        /// <remarks>
        /// Defaults to <c>300</c> (300 milliseconds).  This delay can improve rendering performance for larger forms with inputs which set <see cref="ProBaseInput{T}.Immediate"/> to <c>true</c>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.Form.Behavior)]
        ...
        /// </summary>
        /// <remarks>
        /// Properties of this model are typically linked to form input components via their <see cref="ProFormComponent{T, U}.For"/>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.Form.ValidatedData)]
        ...
        protected HashSet<ProForm> ChildForms { get; } = [];
