...
namespace ProtonBlazor
{
    ...
    /// </summary>
    /// <seealso cref="ProMenuItem" />
    public partial class ProMenu : ProComponentBase, IDisposable
    {
        private readonly ParameterState<bool> _openState;
        private readonly List<ProMenu> _subMenus = [];
        ...
        private ProButton? _buttonActivator;
        private ProMenuItem? _menuItemActivator;
        private ProIconButton? _iconButtonActivator;
        ...
        protected string Classname =>
            new CssBuilder("pro-menu")
                .AddClass("pro-menu-button-hidden", GetActivatorHidden())
                .AddClass(Class)
                .Build();
        ...
        protected string PopoverClassname =>
            new CssBuilder()
                ...
                .AddClass("pro-popover-nested", ParentMenu is not null)
                .AddClass("pro-popover-position-override", PositionAtCursor)
                .Build();
        ...
        protected string ActivatorClassname =>
            new CssBuilder("pro-menu-activator")
        ...
        /// </summary>
        /// <remarks>
        /// When set, this menu will display a <see cref="ProIconButton" />.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.Menu.Behavior)]
        ...
        /// <summary>
        /// The icon displayed before the text when <see cref="Icon"/> is not set. Also used for nested <see cref="ProMenu"/>.
        /// </summary>
        /// <remarks>
        /// Defaults to <c>null</c>.
        /// </remarks>
        ...
        /// <summary>
        /// The color of this menu's button when <see cref="Icon"/> is not set, or the color of the <see cref="ProIconButton" /> when <see cref="Icon"/> is set.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// The size of this menu's button when <see cref="Icon"/> is not set, or the size of the <see cref="ProIconButton" /> when <see cref="Icon"/> is set.
        /// </summary>
        /// <remarks>
        ...
        /// &lt;ProMenu&gt;
        ///     &lt;ActivatorContent&gt;
        ///         &lt;ProButton OnClick="@context.ToggleAsync"&gt;Open Menu&lt;/ProButton&gt;
        ///     &lt;/ActivatorContent&gt;
        ///     &lt;ChildContent&gt;
        ///         &lt;ProMenuItem&gt;Item 1&lt;/ProMenuItem&gt;
        ///     &lt;/ChildContent&gt;
        /// &lt;/ProMenu&gt;
        /// </code>
        /// </remarks>
        ...
        /// <summary>
        /// The <see cref="ProMenuItem" /> components within this menu.
        /// </summary>
        [Parameter]
        [Category(CategoryTypes.Menu.PopupBehavior)]
        ...
        [CascadingParameter]
        protected ProMenu? ParentMenu { get; set; }
        ...
        /// </summary>
        /// <param name="child">The child <see cref="ProMenu"/> to register.</param>
        protected void RegisterChild(ProMenu child)
        {
            _subMenus.Add(child);
        }
        ...
        /// </summary>
        /// <param name="child">The child <see cref="ProMenu"/> to unregister.</param>
        protected void UnregisterChild(ProMenu child)
        {
        ...
        protected override void OnInitialized()
        {
            ...
            if (ParentMenu != null)
            {
                ParentMenu.RegisterItem(this); // Pass the ProMenu directly
            }
        }
        ...
        private async Task PointerEnterAsync(PointerEventArgs args)
        {
            ...
            if (ProGlobal.MenuDefaults.HoverDelay > 0)
            {
                _hoverCts = new();

                try
                {
                    await Task.Delay(TimeSpan.FromMilliseconds(ProGlobal.MenuDefaults.HoverDelay), TimeProvider, _hoverCts.Token);
                }
        ...
        private async Task PointerLeaveAsync(PointerEventArgs args)
        {
            ...
            if (ProGlobal.MenuDefaults.HoverDelay > 0)
            {
                ...
                try
                {
                    await Task.Delay(TimeSpan.FromMilliseconds(ProGlobal.MenuDefaults.HoverDelay), TimeProvider, _leaveCts.Token);
                }
                catch (TaskCanceledException)
                {
        ...
        /// </remarks>
        protected bool HasPointerOver(ProMenu menu)
        {
        ...
        {
            var items = _menuItems.Where(x => x is ProMenuItem).ToList();
            if (items.Count == 0)
                return;
        ...
        private async Task HandleActivationKeyAsync(KeyboardEventArgs e)
        {
            ...
            if (_focusedIndex >= 0 && _focusedIndex < _menuItems.Count)
            {
                ...
                switch (currentItem)
                {
                    ...
                    case ProMenu menu:
                        // For ProMenu items, always open the submenu
        ...
        internal async Task FocusItemAsync(int index)
        {
            if (index >= 0 && index < _menuItems.Count)
            {
                ...
                {
                    ProMenuItem menuItem => menuItem.ElementReference,
                    ProMenu menu => menu._menuItemActivator?.ElementReference ?? default,
                    _ => default
                };
        ...
        /// </summary>
        private ProMenu? FindSubmenuForItem(ProMenuItem menuItem)
        {
