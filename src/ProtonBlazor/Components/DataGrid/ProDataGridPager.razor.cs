...
{
    /// <summary>
    /// Represents a pager for navigating pages of a <see cref="ProDataGrid{T}"/>.
    /// </summary>
    /// <typeparam name="T">The kind of data displayed in the grid.</typeparam>
    /// <seealso cref="ProDataGrid{T}"/>
    public partial class ProDataGridPager<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] T> : ProComponentBase, IDisposable
    {
        /// <summary>
        /// The grid which contains this pager.
        /// </summary>
        [CascadingParameter]
        public ProDataGrid<T>? DataGrid { get; set; }
        ...
        private string Info
        {
            get
            {
                ...
                {
                    return Localizer[LanguageResource.ProDataGridPager_InfoFormat, firstItem.ToString("N0", culture), lastItem.ToString("N0", culture), allItems.ToString("N0", culture)];
                }
