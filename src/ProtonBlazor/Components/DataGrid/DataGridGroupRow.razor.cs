...
{
    public partial class DataGridGroupRow<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] T> : ProComponentBase
    {
        internal bool _expanded;

        protected string GroupClassname => new CssBuilder("pro-table-cell")
            .AddClass("pro-datagrid-group")
            .AddClass($"pro-row-group-indented-{(GroupDefinition.Indentation ? Math.Min(GroupDefinition.Level, 5) : 0)}")
