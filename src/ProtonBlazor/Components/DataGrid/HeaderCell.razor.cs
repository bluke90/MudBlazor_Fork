...
{
    /// <summary>
    /// Represents a cell displayed at the top of a <see cref="ProDataGrid{T}"/> column.
    /// </summary>
    /// <typeparam name="T">The kind of item managed by the grid.</typeparam>
    /// <seealso cref="ProDataGrid{T}"/>
    public partial class HeaderCell<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] T> : ProComponentBase, IDisposable
    {
        ...
        /// <summary>
        /// The <see cref="ProDataGrid{T}"/> which contains this header cell.
        /// </summary>
        [CascadingParameter]
        ...
        private string ResizerClass =>
            new CssBuilder()
                .AddClass("pro-resizing", when: _isResizing)
                .AddClass("pro-resizer")
                .Build();
        ...
        internal bool IncludeHierarchyToggle => Column?.HeaderClass?.Contains("pro-header-togglehierarchy") ?? false;
        ...
        internal string sortIconClass
        {
            get
            {
                ...
                {
                    SortDirection.Descending => "sort-direction-icon pro-direction-desc",
                    SortDirection.Ascending => "sort-direction-icon pro-direction-asc",
        ...
        {
            var boundingRect = await _headerElement.ProGetBoundingClientRectAsync();
