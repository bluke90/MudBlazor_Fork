...
namespace ProtonBlazor
{
    internal class SnackBarMessageState
    {
        ...
        public string SnackbarClass
        {
            get
            {
                var baseTypeClass = $"pro-alert-{Options.SnackbarVariant.ToStringFast(true)}-{Options.Severity.ToStringFast(true)}";

                if (Options.SnackbarVariant != Variant.Filled)
                {
                    baseTypeClass += Options.BackgroundBlurred ? " pro-snackbar-blurred" : " pro-snackbar-surface";
                }

                var result = $"pro-snackbar {baseTypeClass} {Options.SnackbarTypeClass}";
