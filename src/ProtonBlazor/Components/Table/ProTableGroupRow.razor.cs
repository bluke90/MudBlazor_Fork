...
namespace ProtonBlazor
{
    ...
    /// <summary>
    /// A grouping of values for a column in a <see cref="ProTable{T}"/>.
    /// </summary>
    /// <typeparam name="T">The type of item being grouped.</typeparam>
    public partial class ProTableGroupRow<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] T> : ProComponentBase, IDisposable
    {
        private bool? _checked = false;
        private IGrouping<object, T>? _items = null;
        private IEnumerable<IGrouping<object, T>>? _innerGroupItems = null;

        protected string HeaderClassname => new CssBuilder("pro-table-row")
            .AddClass(HeaderClass)
            .AddClass($"pro-table-row-group-indented-{GroupDefinition?.Level - 1}",
                (GroupDefinition?.Indentation ?? false) && GroupDefinition?.Level > 1)
            .Build();

        protected string FooterClassname => new CssBuilder("pro-table-row")
        ...
        /// <param name="notify">When <c>true</c>, and <see cref="Checkable"/> is <c>true</c>, the <see cref="ProTable{T}.OnGroupHeaderCheckboxClicked(bool, IEnumerable{T})"/> event will occur.</param>
        ...
        private ProTable<T>? Table
        {
            get => (ProTable<T>?)((TableContext<T>?)Context)?.Table;
        }
    }
}
