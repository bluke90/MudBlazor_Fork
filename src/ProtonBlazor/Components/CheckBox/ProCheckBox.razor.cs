...
namespace ProtonBlazor
{
    ...
    public partial class ProCheckBox<T> : ProBooleanInput<T>
    {
        ...
        protected override string LabelClassname => new CssBuilder("pro-checkbox")
            .AddClass($"pro-disabled", GetDisabledState())
            .AddClass($"pro-readonly", GetReadOnlyState())
        ...
        protected override string IconClassname => new CssBuilder("pro-button-root pro-icon-button")
            .AddClass($"pro-{Color.ToStringFast(true)}-text hover:pro-{Color.ToStringFast(true)}-hover", (!GetReadOnlyState() && !GetDisabledState() && UncheckedColor == null) || (UncheckedColor != null && BoolValue == true))
            .AddClass($"pro-{UncheckedColor?.ToStringFast(true)}-text hover:pro-{UncheckedColor?.ToStringFast(true)}-hover", !GetReadOnlyState() && !GetDisabledState() && UncheckedColor != null && BoolValue == false)
            .AddClass($"pro-checkbox-dense", Dense)
            .AddClass($"pro-ripple pro-ripple-checkbox", Ripple && !GetReadOnlyState() && !GetDisabledState())
            .AddClass($"pro-disabled", GetDisabledState())
            .AddClass($"pro-readonly", GetReadOnlyState())
            .AddClass($"pro-checkbox-true", BoolValue == true)
            .AddClass($"pro-checkbox-false", BoolValue == false)
            .AddClass($"pro-checkbox-null", BoolValue is null)
            .Build();
