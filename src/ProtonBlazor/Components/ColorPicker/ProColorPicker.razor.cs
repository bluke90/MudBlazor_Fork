...
namespace ProtonBlazor
{
    ...
    /// </summary>
    public partial class ProColorPicker : ProPicker<ProColor>
    {
        ...
        private ProColor? _lastColor;
        private ProColor? _baseColor;
        ...
        private readonly IReadOnlyList<ProColor> _gridList = new ProColor[]
        {
        ...
        private readonly ProColor _defaultColor = "#594ae2";

        [Inject]
        private TimeProvider TimeProvider { get; set; } = null!;

        public ProColorPicker()
        {
            ...
            _valueState = registerScope.RegisterParameter<ProColor?>(nameof(Value))
                ...
                .WithComparer(ProColor.ProColorComparer.RgbaAndHsl);
        ...
        private Task OnValueChangeHandlerAsync(ParameterChangedEventArgs<ProColor?> args)
        {
        ...
        /// <summary>
        /// The currently selected color as a <see cref="ProColor"/>.
        /// </summary>
        /// <remarks>
        /// You can use properties in <see cref="ProColor"/> to get color channel values such as <c>RGB</c>, <c>HSL</c>, <c>HEX</c> and more.  When this value changes, the <see cref="ValueChanged"/> event occurs.
        /// </remarks>
        [Parameter, ParameterState]
        ...
        [Parameter]
        [Category(CategoryTypes.FormComponent.PickerBehavior)]
        public IEnumerable<ProColor> Palette { get; set; } = new ProColor[]
        ...
        protected override void OnInitialized()
        {
            ...
            AdornmentAriaLabel ??= Localizer[Resources.LanguageResource.ProColorPicker_Open];
        }
        ...
        private async Task SelectPaletteColorAsync(ProColor color)
        {
        ...
        /// <inheritdoc />
        protected override IConverter<ProColor?, string?> GetDefaultConverter()
        {
            return new DefaultConverter<ProColor>
            {
                Culture = GetCulture,
                Format = GetFormat
            };
        ...
        private async Task SetColorAsync(ProColor? newColor, bool forceUpdate = false)
        {
        ...
        protected internal override ProColor? ReadValue => _valueState.Value;

        protected override Task SetValueCoreAsync(ProColor? value) => SetColorAsync(value);
        ...
        private static ProColor UpdateBaseColor(ProColor newColor)
        {
            ...
            var newBaseColor = new ProColor(r(value), g(value), b(value), 255);
            return newBaseColor;
        }
        ...
        private async Task UpdateColorBaseOnSelectionAsync()
        {
            ...
            var newColor = new ProColor((byte)r, (byte)g, (byte)b, _valueState.Value ?? _baseColor);
            await SetColorAsync(newColor);

            _skipFeedback = false;
        }

        private static (double x, double y) UpdateColorSelectorBasedOnRgb(ProColor newColor)
        {
        ...
        /// </summary>
        private ProColor ValueOrDefault => _valueState.Value ?? _lastColor ?? _defaultColor;
        ...
        private async Task SetInputStringAsync(string? input)
        {
            ...
            else if (ProColor.TryParse(input, out var result))
            {
        ...
        private string? GetColorTextValue(ProColor? color) => !_alphaState.Value || _colorPickerViewState.Value is ColorPickerView.Palette or ColorPickerView.GridCompact
            ? color?.ToString(ProColorOutputFormats.Hex)
            : color?.ToString(ProColorOutputFormats.HexA);
        ...
        private EventCallback<MouseEventArgs> GetSelectPaletteColorCallback(ProColor color) => new EventCallbackFactory().Create(this, (MouseEventArgs _) => SelectPaletteColorAsync(color));

        private Color GetButtonColor(ColorPickerView view) => _colorPickerViewState.Value == view ? Color.Primary : Color.Inherit;

        private string GetColorDotClass(ProColor color) => new CssBuilder("pro-picker-color-dot").AddClass("selected", color == _valueState.Value).ToString();

        private string AlphaSliderStyle => new StyleBuilder()
            .AddStyle($"background-image: linear-gradient(to {(RightToLeft ? "left" : "right")}, transparent, {ValueOrDefault.ToString(ProColorOutputFormats.RGB)})")
            .Build();
    }
}
