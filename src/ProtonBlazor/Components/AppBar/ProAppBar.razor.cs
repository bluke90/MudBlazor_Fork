...
/// </summary>
/// <seealso cref="ProContextualActionBar"/>
public partial class ProAppBar : ProComponentBase
{
    internal static SectionOutlet ContextualActionBar { get; } = new();

    protected string Classname =>
        new CssBuilder("pro-appbar")
            .AddClass($"pro-appbar-dense", Dense)
            .AddClass($"pro-appbar-fixed-top", Fixed && !Bottom)
            .AddClass($"pro-appbar-fixed-bottom", Fixed && Bottom)
    ...
    protected string ToolBarClassname =>
        new CssBuilder("pro-toolbar-appbar")
    ...
    /// </summary>
    /// <remarks>
    /// Defaults to <c>false</c>. When <c>true</c>, can be overridden by <see cref="ProContextualActionBar"/>
    /// </remarks>
    [Parameter]
    [Category(CategoryTypes.AppBar.Behavior)]
