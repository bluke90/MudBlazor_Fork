...
/// <summary>
/// A portion of a <see cref="ProGrid"/>.
/// </summary>
/// <seealso cref="ProGrid"/>
public partial class ProItem : ProComponentBase
{
    protected string Classname =>
        new CssBuilder("pro-grid-item")
            .AddClass($"pro-grid-item-xs-{xs}", xs != 0)
            .AddClass($"pro-grid-item-sm-{sm}", sm != 0)
            .AddClass($"pro-grid-item-md-{md}", md != 0)
            .AddClass($"pro-grid-item-lg-{lg}", lg != 0)
            .AddClass($"pro-grid-item-xl-{xl}", xl != 0)
            .AddClass($"pro-grid-item-xxl-{xxl}", xxl != 0)
            .AddClass(Class)
            .Build();

    [CascadingParameter]
    private ProGrid? Parent { get; set; }
