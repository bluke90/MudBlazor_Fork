...
namespace ProtonBlazor
{
    ...
    /// </summary>
    public partial class ProTooltip : ProComponentBase
    {
        ...
        protected string ContainerClass => new CssBuilder("pro-tooltip-root")
            .AddClass("pro-tooltip-inline", Inline)
            .AddClass(RootClass)
            .Build();

        protected string Classname => new CssBuilder("pro-tooltip")
            .AddClass("d-flex")
            .AddClass("pro-tooltip-default", Color == Color.Default)
            .AddClass($"pro-tooltip-{ConvertPlacement().ToStringFast(true)}")
            .AddClass("pro-tooltip-arrow", Arrow)
            .AddClass($"pro-border-{Color.ToStringFast(true)}", Arrow && Color != Color.Default)
        ...
        /// </summary>
        /// <remarks>
        /// Defaults to 251ms in <see cref="ProGlobal.TooltipDefaults.Duration"/>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.Tooltip.Appearance)]
        public double Duration { get; set; } = ProGlobal.TooltipDefaults.Duration.TotalMilliseconds;

        /// <summary>
        /// The amount of time, in milliseconds, to wait from opening the popover before performing the transition. 
        /// </summary>
        /// <remarks>
        /// Defaults to 0ms in <see cref="ProGlobal.TooltipDefaults.Delay"/>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.Tooltip.Appearance)]
        public double Delay { get; set; } = ProGlobal.TooltipDefaults.Delay.TotalMilliseconds;
