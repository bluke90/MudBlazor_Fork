...
namespace ProtonBlazor
{
    ...
    public partial class ProSwitch<T> : ProBooleanInput<T>
    {
        ...
        protected override string LabelClassname => new CssBuilder("pro-switch")
        ...
        protected string SwitchClassname => new CssBuilder("pro-button-root pro-icon-button pro-switch-base")
            .AddClass($"pro-ripple pro-ripple-switch", Ripple && !GetReadOnlyState() && !GetDisabledState())
            .AddClass($"pro-{Color.ToStringFast(true)}-text hover:pro-{Color.ToStringFast(true)}-hover", !GetReadOnlyState() && !GetDisabledState() && BoolValue == true)
            .AddClass($"pro-{UncheckedColor.ToStringFast(true)}-text hover:pro-{UncheckedColor.ToStringFast(true)}-hover", !GetReadOnlyState() && !GetDisabledState() && BoolValue == false)
            .AddClass($"pro-switch-disabled", GetDisabledState())
            .AddClass($"pro-readonly", GetReadOnlyState())
            .AddClass($"pro-checked", BoolValue)
            .AddClass($"pro-switch-base-{Size.ToStringFast(true)}")
            .Build();

        protected string TrackClassname => new CssBuilder("pro-switch-track")
            .AddClass($"pro-{Color.ToStringFast(true)}", BoolValue == true)
            .AddClass($"pro-{UncheckedColor.ToStringFast(true)}", BoolValue == false)
            .Build();

        protected string ThumbClassname => new CssBuilder($"pro-switch-thumb-{Size.ToStringFast(true)}")
            .AddClass("d-flex align-center justify-center")
            .Build();

        protected string SpanClassname => new CssBuilder("pro-switch-span")
            .AddClass($"pro-switch-span-{Size.ToStringFast(true)}")
            .Build();
