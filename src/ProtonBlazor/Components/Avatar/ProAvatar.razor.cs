...
namespace ProtonBlazor
{
    ...
    /// </summary>
    /// <seealso cref="ProAvatarGroup" />
    partial class ProAvatar : ProComponentBase, IDisposable
    {
        [CascadingParameter]
        protected ProAvatarGroup? AvatarGroup { get; set; }

        protected string Classname => new CssBuilder("pro-avatar")
            .AddClass($"pro-avatar-{Size.ToStringFast(true)}")
            .AddClass($"pro-avatar-rounded", Rounded)
            .AddClass($"pro-avatar-square", Square)
            .AddClass($"pro-avatar-{Variant.ToStringFast(true)}")
            .AddClass($"pro-avatar-{Variant.ToStringFast(true)}-{Color.ToStringFast(true)}")
