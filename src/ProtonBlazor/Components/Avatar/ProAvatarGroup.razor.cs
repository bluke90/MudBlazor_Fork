...
{
    /// <summary>
    /// Represents a grouping of multiple <see cref="ProAvatar"/> components.
    /// </summary>
    /// <seealso cref="ProAvatar" />
    partial class ProAvatarGroup : ProComponentBase
    {
        private bool _childrenNeedUpdates = false;

        protected string Classname => new CssBuilder("pro-avatar-group")
            .AddClass($"pro-avatar-group-outlined", Outlined)
            .AddClass($"pro-avatar-group-outlined-{OutlineColor.ToStringFast(true)}", Outlined)
            .AddClass(Class)
            .Build();

        protected string MaxAvatarClassname => new CssBuilder("pro-avatar-group-max-avatar")
        ...
        internal List<ProAvatar> _avatars = new();

        public ProAvatarGroup()
        {
        ...
        internal void AddAvatar(ProAvatar avatar)
        {
            _avatars.Add(avatar);
            StateHasChanged();
        }

        internal void RemoveAvatar(ProAvatar avatar)
        {
        ...
        internal StyleBuilder GetAvatarZindex(ProAvatar avatar) => new StyleBuilder()
            .AddStyle("z-index", $"{_avatars.Count - _avatars.IndexOf(avatar)}");

        internal bool MaxGroupReached(ProAvatar avatar)
        {
