...
namespace ProtonBlazor
{
    ...
    public partial class ProDateRangePicker : ProBaseDatePicker
    {
        ...
        /// </summary>
        /// <remarks>
        /// By default, it will always ignore disabled dates. This parameter will take effect when <see cref="ProBaseDatePicker.IsDateDisabledFunc"/> is set.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.FormComponent.Validation)]
        ...
        private ProRangeInput<string> _rangeInput = null!;
        ...
        protected override string GetDayClasses(int month, DateTime day)
        {
            ...
            if ((_firstDate?.Date < day && _secondDate?.Date > day) || CheckDateRange(day, compareStart: isLessThan, compareEnd: isGreaterThan))
            {
                return b
                    .AddClass("pro-range")
                    .AddClass("pro-range-between")
                    .AddClass($"pro-current pro-{Color.ToStringFast(true)}-text pro-button-outlined pro-button-outlined-{Color.ToStringFast(true)}", day == today)
                    .Build();
            }

            if (_firstDate?.Date == day && _secondDate?.Date == day)
            {
                return b.AddClass("pro-selected")
            ...
            if (_firstDate?.Date == day || CheckDateRange(day, compareStart: isEqualTo, compareEnd: isNotEqualTo))
            {
                return b.AddClass("pro-selected")
                    .AddClass("pro-range")
                    .AddClass("pro-range-start-selected")
                    .AddClass("pro-range-selection", _firstDate != null)
                    .AddClass($"pro-theme-{Color.ToStringFast(true)}")
                    .Build();
            }
            ...
            if ((_firstDate is { } && _secondDate?.Date == day) || CheckDateRange(day, compareStart: isNotEqualTo, compareEnd: isEqualTo))
            {
                return b.AddClass("pro-selected")
                    .AddClass("pro-range")
                    .AddClass("pro-range-end-selected")
                    .AddClass($"pro-theme-{Color.ToStringFast(true)}")
                    .Build();
            }
            ...
            {
                return b.AddClass("pro-selected").AddClass($"pro-theme-{Color.ToStringFast(true)}").Build();
            }

            if (_firstDate?.Date < day)
            {
                return b.AddClass("pro-range", _secondDate is null && day != today)
                    .AddClass("pro-range-selection")
                    .AddClass($"pro-range-selection-{Color.ToStringFast(true)}", _firstDate is not null)
                    .AddClass($"pro-current pro-{Color.ToStringFast(true)}-text pro-button-outlined pro-button-outlined-{Color.ToStringFast(true)}", day == today)
                    .Build();
            }
            ...
            if (day == today)
            {
                return b.AddClass("pro-current")
                    .AddClass($"pro-button-outlined pro-button-outlined-{Color.ToStringFast(true)}")
                    .AddClass($"pro-{Color.ToStringFast(true)}-text")
                    .Build();
            }
