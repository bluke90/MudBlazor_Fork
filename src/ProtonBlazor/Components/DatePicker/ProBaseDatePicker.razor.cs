...
namespace ProtonBlazor
{
    ...
    /// </summary>
    public abstract partial class ProBaseDatePicker : ProPicker<DateTime?>
    {
        private readonly string _proPickerCalendarContentElementId;
        private readonly ParameterState<string?> _dateFormatState;

        protected ProBaseDatePicker()
        {
            _proPickerCalendarContentElementId = Identifier.Create();
        ...
        private string? GetYearClasses(int year)
        {
            ...
            if (year == calendar.GetYear(selectedYear))
                return $"pro-picker-year-selected pro-{Color.ToStringFast(true)}-text";
            return null;
        }

        private string GetCalendarHeaderClasses(int month)
        {
            return new CssBuilder("pro-picker-calendar-header")
                .AddClass($"pro-picker-calendar-header-{month + 1}")
                .AddClass($"pro-picker-calendar-header-last", month == DisplayMonths - 1)
                .Build();
        }
        ...
        private string? GetMonthClasses(DateTime month)
        {
            ...
            if (calendar.GetMonth(month) == calendar.GetMonth(selectedMonth) && !IsMonthDisabled(selectedMonth))
                return $"pro-picker-month-selected pro-{Color.ToStringFast(true)}-text";
        ...
        {
            base.OnInitialized();
            AdornmentAriaLabel ??= Localizer[Resources.LanguageResource.ProBaseDatePicker_Open];
            CurrentView = OpenTo;
        ...
        {
            return JsApiService.UpdateStyleProperty(_proPickerCalendarContentElementId, "--selected-day", tempId);
        }
    }
}
