...
using ProtonBlazor.State.Invocation;
...
/// ProtonBlazor's ParameterState framework. For details and usage please read CONTRIBUTING.md
/// </summary>
/// <remarks>
/// You don't need to create this object directly.
/// Instead, use the "ProComponentBase.RegisterParameter" method from within the component's constructor.
/// </remarks>
...
internal class ParameterStateInternal<T> : ParameterState<T>, IParameterComponentLifeCycle, IEquatable<ParameterStateInternal<T>>
{
    ...
    /// </para>
    /// <code>
    ///      var panels = Context.RenderComponent&lt;ProExpansionPanels&gt;();
    ///      var panel = new ProExpansionPanel();
    ///      panels.Instance.AddPanelAsync(panel);
    /// </code>
    /// <para>
    /// In this scenario, ProExpansionPanel is created outside Blazor's lifecycle, so
    ...
    public override Task SetValueAsync(T value)
    {
        ...
        if (!_comparer.Equals(_value, value))
        {
            ...
            {
                // https://github.com/ProtonBlazor/ProtonBlazor/pull/12241
    ...
    ///  <summary>
    ///  Creates a <see cref="ParameterState{T}"/> object which automatically manages parameter value changes as part of ProtonBlazor's ParameterState framework.
    /// <para />
    ///  <b>NB!</b> Usually you don't need to call this directly. Instead, use the RegisterParameter method (<see cref="ProComponentBase"/>) from within the
