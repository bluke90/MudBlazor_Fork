...
/// </remarks>
internal class PopoverService : IPopoverService, IBatchTimerHandler<ProPopoverHolder>
{
    private bool _disposed;
    private bool _isInitializing;
    private readonly PopoverJsInterop _popoverJsInterop;
    private readonly CancellationToken _cancellationToken;
    private readonly Dictionary<Guid, ProPopoverHolder> _holders;
    private readonly CancellationTokenSource _cancellationTokenSource;
    private readonly BatchPeriodicQueue<ProPopoverHolder> _batchExecutor;
    ...
    public PopoverService(ILogger<PopoverService> logger, IJSRuntime jsInterop, TimeProvider timeProvider, IOptions<PopoverOptions>? options = null)
    {
        ...
        _holders = new Dictionary<Guid, ProPopoverHolder>();
        ...
        _batchExecutor = new BatchPeriodicQueue<ProPopoverHolder>(this, PopoverOptions.QueueDelay, timeProvider);
    ...
    public async Task CreatePopoverAsync(IPopover popover)
    {
        ...
        if (PopoverOptions.CheckForPopoverProvider)
        {
            if (ObserversCount == 0)
            {
                throw new InvalidOperationException($"Missing <{nameof(ProPopoverProvider)} />, please add it to your layout. See https://protonblazor.com/getting-started/installation#manual-install-add-components");
            }
        }

        var holder = new ProPopoverHolder(popover.Id, _timeProvider)
    ...
    /// <inheritdoc />
    public virtual Task OnBatchTimerElapsedAsync(IReadOnlyCollection<ProPopoverHolder> items, CancellationToken stoppingToken = default)
    {
        ...
        // In the future, there might be a requirement to split the jobs and introduce a change where instead of using IReadOnlyCollection<ProPopoverHolder>,
    ...
    {
        var holdersListCopy = new List<ProPopoverHolder>(_holders.Values);
        _holders.Clear();
    ...
    private async Task<bool> DestroyPopoverByIdAsync(Guid id, bool queueForDisconnect = true)
    {
        ...
        // as it is no longer being rendered by ProPopoverProvider since it has been removed from the ActivePopovers collection.
    ...
    private async Task DetachRangeAsync(IReadOnlyCollection<ProPopoverHolder> holders)
    {
        if (_disposed)
        {
            return;
    ...
    private async Task InitializePopoverIfNeededAsync(ProPopoverHolder holder)
    {
