...
/// </summary>
/// <remarks>
/// This is the standard interceptor used by ProtonBlazor. It keeps the internal English resources as a reliable fallback while allowing apps to override or extend translations.
/// </remarks>
public class DefaultLocalizationInterceptor : AbstractLocalizationInterceptor
{
    ...
    /// <param name="mudLocalizer">The optional custom <see cref="ProLocalizer"/>.</param>
    public DefaultLocalizationInterceptor(ILoggerFactory loggerFactory, ProLocalizer? mudLocalizer = null)
    ...
    public override LocalizedString Handle(string key, params object[] arguments)
    {
        if (!IgnoreDefaultEnglish)
        {
            ...
            if (ProLocalizer is null || currentCulture.Equals("en", StringComparison.InvariantCultureIgnoreCase))
            {
    ...
    /// <remarks>
    /// This method is called when the default English translation is ignored or unavailable, and a custom ProLocalizer service implementation is registered.
    /// It attempts to use user-provided languages, falling back to the internal English translation if ProLocalizer is missing or no resource is found.
    /// </remarks>
    protected virtual LocalizedString TranslationWithFallback(string key, params object[] arguments)
    {
        ...
        if (ProLocalizer is null)
        {
            return anyArguments ? Localizer[key, arguments] : Localizer[key];
        }

        // If CurrentUICulture is not english and a custom ProLocalizer service implementation is registered, try to use user provided languages.
        // If no translation was found, fallback to the internal English translation
        var res = ProLocalizer[key, arguments]; //Handles both scenarios with empty or non-empty arguments.
