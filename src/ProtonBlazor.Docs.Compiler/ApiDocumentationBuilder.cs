...
/// <remarks>
/// <para>
/// This class documents the ProtonBlazor assembly, including all public types, properties, methods, events, and fields.  Inherited
/// members are supported, as well as "see cref" links.  Once all documentation has been loaded, several types are made available 
/// to the <c>ProtonBlazor.Docs</c> such as <see cref="DocumentedType"/>, <see cref="DocumentedMethod"/>, <see cref="DocumentedProperty"/>,
...
public class ApiDocumentationBuilder
{
    ...
    /// <summary>
    /// Serializes all documentation to the ProtonBlazor.Docs "Generated" folder.
    /// </summary>
    ...
    /// <summary>
    /// Finds <see cref="ProGlobal"/> settings related to all types.
    /// </summary>
    public void AddGlobalsToDocument()
    {
        // Find all the "ProGlobal" properties
        var globalProperties = Properties.Where(property => property.Key.StartsWith("ProtonBlazor.ProGlobal+")).ToList();
        foreach (var globalProperty in globalProperties)
        {
            /* ProGlobal properties thankfully mention the component they are for, by way of a
            ...
            if (start != -1)
            {
                // Yes.   Move up to the type  (i.e. "ProtonBlazor.___")
                start += 13;
    ...
    /// <summary>
    /// Resolves XML references like "T:ProtonBlazor.ProAlert" into a <see cref="DocumentedType"/> or <see cref="DocumentedMember"/>.
    /// </summary>
    public void ResolveSeeAlsoLinks()
    {
        ...
        foreach (var type in Types.Where(type => type.Value.Links.Count > 0))
        {
            ...
            {
                // Is this a CREF link?  (e.g. "T:ProtonBlazor.ProAlert")
