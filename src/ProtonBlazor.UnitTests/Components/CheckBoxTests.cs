...
using ProtonBlazor.UnitTests.TestComponents.CheckBox;
...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class CheckBoxTests : BunitTest
    {
        ...
        public async Task CheckBox_Test_BooleanStateSelectors()
        {
            ...
            // pro-checkbox-true, pro-checkbox-false, pro-checkbox-null applied to the span
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.Value, false))
                .Find(".pro-checkbox .pro-checkbox-false").Should().NotBe(null);
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.Value, true))
                .Find(".pro-checkbox span").ClassList.Should().Contain("pro-checkbox-true");
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.Value, true))
                .Find(".pro-checkbox span").ClassList.Should().NotContain("pro-checkbox-false");
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.Value, true))
                .Find(".pro-checkbox span").ClassList.Should().NotContain("pro-checkbox-null");
            var comp = Context.Render<ProCheckBox<bool?>>(self => self
                .Add(x => x.Value, null)
                .Add(x => x.TriState, true));
            comp.Find(".pro-checkbox span").ClassList.Should().Contain("pro-checkbox-null");
            await comp.Find("input").ChangeAsync(true);
            comp.Find(".pro-checkbox span").ClassList.Should().Contain("pro-checkbox-true");
            await comp.Find("input").ChangeAsync(false);
            comp.Find(".pro-checkbox span").ClassList.Should().Contain("pro-checkbox-false");
        ...
        {
            var comp = Context.Render<ProCheckBox<bool>>();
        ...
        {
            var comp = Context.Render<ProCheckBox<bool>>(parameters => parameters.Add(x => x.Value, true));
            // select elements needed for the test
            var box = comp.Instance;
            // check initial state
            box.ReadValue.Should().Be(true);
        ...
        public async Task CheckBoxTest3()
        {
            ...
            // select elements needed for the test
            var boxes = comp.FindComponents<ProCheckBox<bool>>();
            // check initial state
        ...
        public void CheckBoxTest4()
        {
            ...
            // check dense
            comp.FindAll("span").ToArray()[0].ClassList.Should().Contain("pro-checkbox-dense");
            comp.FindAll("span").ToArray()[1].ClassList.Should().NotContain("pro-checkbox-dense");
            comp.FindAll("span").ToArray()[2].ClassList.Should().NotContain("pro-checkbox-dense");
            comp.FindAll("span").ToArray()[3].ClassList.Should().NotContain("pro-checkbox-dense");
            // check size
            comp.FindAll("svg").ToArray()[0].ClassList.Should().Contain("pro-icon-size-medium");
            comp.FindAll("svg").ToArray()[1].ClassList.Should().Contain("pro-icon-size-small");
            comp.FindAll("svg").ToArray()[2].ClassList.Should().Contain("pro-icon-size-medium");
            comp.FindAll("svg").ToArray()[3].ClassList.Should().Contain("pro-icon-size-large");
        }
        ...
        {
            var comp = Context.Render<ProCheckBox<bool?>>(parameters => parameters.Add(x => x.TriState, true));
            // print the generated html
            // select elements needed for the test
            var box = comp.Instance;
            // check initial state
        ...
        {
            using var comp = Context.Render<ProCheckBox<bool>>();
        ...
        {
            var comp = Context.Render<ProCheckBox<bool>>(x => x.Add(c => c.Color, color).Add(b => b.UncheckedColor, uncheckedcolor));

            var box = comp.Instance;

            // check initial state
            box.ReadValue.Should().Be(false);
            comp.Find(".pro-button-root.pro-icon-button").ClassList.Should().ContainInOrder(new[] { $"pro-{uncheckedcolor.ToStringFast(true)}-text", $"hover:pro-{uncheckedcolor.ToStringFast(true)}-hover" });

            // click and check if it has new color
            await comp.Find("input").ChangeAsync(true);
            box.ReadValue.Should().Be(true);
            comp.Find(".pro-button-root.pro-icon-button").ClassList.Should().ContainInOrder(new[] { $"pro-{color.ToStringFast(true)}-text", $"hover:pro-{color.ToStringFast(true)}-hover" });
        }
        ...
        public void CheckBoxDisabled()
        {
            ...
            comp.FindAll("label.pro-checkbox")[3].ClassList.Should().Contain("pro-disabled"); // 4rd checkbox
        }
        ...
        public void CheckBoxLabelPlacement()
        {
            ...
            comp.FindAll("label.pro-checkbox")[2].ClassList.Should().Contain("pro-input-content-placement-start"); // 3rd checkbox: Placement.Start
        }
        ...
        public void CheckBoxLabel()
        {
            ...
            var comp = Context.Render<ProCheckBox<bool>>(x => x.Add(f => f.For, () => value.Boolean));
            comp.Instance.Label.Should().Be("Boolean LabelAttribute"); //label should be set by the attribute

            var comp2 = Context.Render<ProCheckBox<bool>>(x => x.Add(f => f.For, () => value.Boolean).Add(l => l.Label, "Label Parameter"));
        ...
        {
            var comp = Context.Render<ProCheckBox<bool>>(parameters => parameters
        ...
        {
            var comp = Context.Render<ProCheckBox<bool>>(parameters => parameters.AddUnmatched("tabindex", "-1"));
        ...
        {
            var comp = Context.Render<ProCheckBox<bool>>(parameters => parameters.Add(x => x.Disabled, true));
        ...
        {
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.ReadOnly, false)).Find("span").ClassList.Should().Contain("hover:pro-default-hover");
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.ReadOnly, true)).Find("span").ClassList.Should().NotContain("hover:pro-default-hover");
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.ReadOnly, true).Add(x => x.Disabled, false)).Find("span").ClassList.Should().NotContain("hover:pro-default-hover");
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.Disabled, false)).Find("span").ClassList.Should().Contain("hover:pro-default-hover");
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.Disabled, true).Add(x => x.ReadOnly, false)).Find("span").ClassList.Should().NotContain("hover:pro-default-hover");
            Context.Render<ProCheckBox<bool>>(self => self.Add(x => x.Disabled, true).Add(x => x.ReadOnly, true)).Find("span").ClassList.Should().NotContain("hover:pro-default-hover");
        }
        ...
        public void CheckBox_AriaLabel_OverRides()
        {
            ...
            var checkboxes = comp.FindAll(".pro-input-control.pro-input-control-boolean-input");

            // verify checkbox one maintains it's original structure, no aria class used, label with a span element
            checkboxes[0].GetElementsByClassName("pro-sr-only").Length.Should().Be(0);
            var element0 = comp.Find(".cb1 label.pro-checkbox span.pro-typography");
            ...
            checkboxes[1].GetElementsByClassName("pro-sr-only").Length.Should().Be(1);
            var element1 = comp.Find(".cb2 label.pro-checkbox span.pro-typography");
            element1.HasAttribute("aria-hidden").Should().BeTrue();
            var input1 = comp.Find(".cb2 label.pro-checkbox input");
            var input1ForId = input1.GetAttribute("aria-labelledby");
            comp.Find($".cb2 label.pro-checkbox #{input1ForId}").Should().NotBeNull();

            // checkbox three should have original structure intact, no aria class used, label with a span element for child content
            checkboxes[2].GetElementsByClassName("pro-sr-only").Length.Should().Be(0);
            var element2 = comp.Find(".cb3 label.pro-checkbox span.pro-typography");
            ...
            checkboxes[3].GetElementsByClassName("pro-sr-only").Length.Should().Be(1);
            var element3 = comp.Find(".cb4 label.pro-checkbox span.pro-typography");
            element3.HasAttribute("aria-hidden").Should().BeTrue();
            var input3 = comp.Find(".cb4 label.pro-checkbox input");
            var input3ForId = input3.GetAttribute("aria-labelledby");
            comp.Find($".cb4 label.pro-checkbox #{input3ForId}").Should().NotBeNull();

            // checkbox five has no label, no child content, just arialabel
            checkboxes[4].GetElementsByClassName("pro-sr-only").Length.Should().Be(1);
            comp.FindAll(".cb5 label.pro-checkbox span.pro-typography").Count.Should().Be(0);
            var input4 = comp.Find(".cb5 label.pro-checkbox input");
            var input4ForId = input4.GetAttribute("aria-labelledby");
            comp.Find($".cb5 label.pro-checkbox #{input4ForId}").Should().NotBeNull();
        }
    }
}
