...
using Bunit;
using ProtonBlazor.UnitTests.TestComponents.AppBar;
...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class AppBarTests : BunitTest
    {
        ...
        {
            var comp = Context.Render<ProAppBar>(parameters => parameters.Add(x => x.ToolBarClass, "test-class"));
        ...
        {
            var bar = Context.Render<ProAppBar>();
            bar.Markup
               .Should()
               .StartWith("<header")
               .And
               .Contain("pro-appbar-fixed-top");
        }
        ...
        {
            var bar = Context.Render<ProAppBar>(parameters => parameters.Add(x => x.Bottom, false));
            bar.Markup
               .Should()
               .StartWith("<header")
               .And
        ...
        {
            var bar = Context.Render<ProAppBar>(parameters => parameters.Add(x => x.Bottom, true));
            bar.Markup
               .Should()
               .StartWith("<footer")
               .And
               .Contain("pro-appbar-fixed-bottom");
        }
        ...
        {
            var comp = Context.Render<ProAppBar>();
            comp.FindComponent<ProToolBar>().Instance.WrapContent.Should().Be(false);
        }
        ...
        public async Task AppBarWithContextualSetTrue()
        {
            ...
            var bar = comp.FindComponent<ProAppBar>();

            bar.Markup.Should().Contain("regular-app-bar").And.Contain("pro-theme-primary");

            await comp.Find(".pro-switch-input").ChangeAsync(true);

            bar.Markup.Should().Contain("contextual-app-bar").And.Contain("pro-theme-tertiary");
        }
