...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class ColorPickerTests : BunitTest
    {
        ...
        private const double _maxYForColorPanel = 250.0;
        private const double _defaultXForColorPanel = 209.84;
        private const double _defaultYForColorPanel = 28.43;

        private static readonly ProColor _defaultColor = "#594ae2";
        private const string _hueSliderCssSelector = ".pro-slider.pro-picker-color-slider.hue input";
        private const string _alphaSliderCssSelector = ".pro-picker-color-slider.alpha input";
        private const string _colorDotCssSelector = ".pro-picker-color-fill";
        private const string _toolbarCssSelector = ".pro-toolbar";
        private const string _proColorPickerCssSelector = ".pro-picker-color-picker";
        private const string _slidersControlCssSelector = ".pro-picker-color-sliders";
        private const string _colorInputCssSelector = ".pro-picker-color-inputs";
        private const string _colorInputModeSwitchCssSelector = ".pro-picker-control-switch";
        private const string _alphaInputCssSelector = ".input-field-alpha";
        private const string CssSelector = ".pro-picker-color-overlay-black .pro-picker-color-overlay";
        private const string _proToolbarButtonsCssSelector = ".pro-toolbar button";

        private static readonly ProColor[] _proGridDefaultColors = new ProColor[]
        {
        ...
        private static readonly ProColor[] _proGridPaletteDefaultColors = new ProColor[]
        {
        ...
        /// </summary>
        /// <remarks>
        /// These tests care about synchronization bugs, not just whether <see cref="ProColorPicker"/> stores the right value.
        ...
        /// </remarks>
        private async Task CheckColorRelatedValues(IRenderedComponent<SimpleColorPickerTest> comp, double expectedX, double expectedY, ProColor expectedColor, ColorPickerMode mode, bool checkInstanceValue = true, bool isRtl = false)
        {
            ...
            await comp.WaitForAssertionAsync(() => AssertDisplayedChannelValues(comp, expectedColor, mode));

            var selector = comp.Find(".pro-picker-color-selector");
            ...
            var hueSlideValue = comp.FindAll(".pro-picker-color-slider.hue input");
            ...
            if (!isRtl)
            {
                alphaSliderStyleAttribute.Should().Be($"background-image: linear-gradient(to right, transparent, {expectedColor.ToString(ProColorOutputFormats.RGB)});");
            }
            else
            {
                alphaSliderStyleAttribute.Should().Be($"background-image: linear-gradient(to left, transparent, {expectedColor.ToString(ProColorOutputFormats.RGB)});");
            }
        }

        /// <summary>
        /// Gets the visible channel inputs for the rendered picker and asserts the expected count.
        /// </summary>
        private static IHtmlInputElement[] GetColorInputs(IRenderedComponent<SimpleColorPickerTest> rendered, int expectedCount = 4)
        {
            var inputs = rendered.FindAll(".pro-picker-color-inputs input");
        ...
        /// <summary>
        /// Gets a visible channel input by index from the rendered picker.
        /// </summary>
        private static IHtmlInputElement GetColorInput(IRenderedComponent<SimpleColorPickerTest> rendered, int index, int expectedCount = 4) => GetColorInputs(rendered, expectedCount)[index];
        ...
        {
            var inputs = scope.QuerySelectorAll(".pro-picker-color-inputs input");
        ...
        /// <summary>
        /// Asserts that the visible channel inputs match the supplied color for the selected mode.
        /// </summary>
        private static void AssertDisplayedChannelValues(IRenderedComponent<SimpleColorPickerTest> rendered, ProColor expectedColor, ColorPickerMode mode)
            => AssertDisplayedChannelValuesCore(GetColorInputs(rendered, mode == ColorPickerMode.HEX ? 1 : 4), expectedColor, mode);

        /// <summary>
        /// Asserts that a picker's channel inputs match the supplied color for the selected mode.
        /// </summary>
        private static void AssertDisplayedChannelValuesCore(IHtmlInputElement[] castedInputs, ProColor expectedColor, ColorPickerMode mode)
        {
        ...
        /// </remarks>
        private static void CheckColorRelatedValues(IElement scope, double expectedX, double expectedY, ProColor expectedColor, ColorPickerMode mode, bool isRtl = false)
        {
            if (mode is ColorPickerMode.RGB or ColorPickerMode.HSL)
            {
                var castedInputs = GetColorInputs(scope);
            ...
            {
                var castedInputs = GetColorInputs(scope, 1);
                castedInputs[0].Value.Should().Be(expectedColor.Value);
            }

            var selector = scope.QuerySelector(".pro-picker-color-selector");
            selector.Should().NotBeNull();

            var selectorStyleAttribute = selector!.GetAttribute("style");
            selectorStyleAttribute.Should().Be($"transform: translate({expectedX.ToString(CultureInfo.InvariantCulture)}px, {expectedY.ToString(CultureInfo.InvariantCulture)}px);");

            var hueSlideValue = scope.QuerySelectorAll(".pro-picker-color-slider.hue input");
            hueSlideValue.Should().ContainSingle();
            hueSlideValue[0].Should().BeAssignableTo<IHtmlInputElement>();

            ((IHtmlInputElement)hueSlideValue[0]).Value.Should().Be(((int)expectedColor.H).ToString());

            var alphaSlider = scope.QuerySelectorAll(_alphaSliderCssSelector);
            alphaSlider.Should().ContainSingle();
            alphaSlider[0].Should().BeAssignableTo<IHtmlInputElement>();
        ...
        /// </remarks>
        private static (double x, double y) GetExpectedSelectorPosition(ProColor color)
        {
            ...
            var dominantColorPart = index switch
            {
                0 => "rb",
                1 => "gb",
                2 => "gr",
            ...
            {
                "rb" => (primary: color.R, secondary: color.B),
                "rg" => (primary: color.R, secondary: color.G),
                "gb" => (primary: color.G, secondary: color.B),
                "gr" => (primary: color.G, secondary: color.R),
                "br" => (primary: color.B, secondary: color.R),
                "bg" => (primary: color.B, secondary: color.G),
                _ => (primary: 255, secondary: 255)
            };

            var primaryDiff = 255 - colorValues.primary;
            var primaryDiffDelta = colorValues.primary / 255.0;
            var selectorY = MathExtensions.Map(0, 255, 0, _maxYForColorPanel, primaryDiff);
            var secondaryColorX = colorValues.secondary * (1.0 / primaryDiffDelta);
            var relation = (255 - secondaryColorX) / 255.0;
            var selectorX = relation * _maxXForColorPanel;
        ...
        /// </remarks>
        private async Task AssertInitializedColorStateAsync(ProColor expectedColor, ColorPickerMode mode)
        {
            var comp = Context.Render<SimpleColorPickerTest>(p =>
            {
                p.Add(x => x.ColorValue, expectedColor);
        ...
        {
            var comp = Context.Render<ProColorPicker>();
        ...
        public void Default()
        {
            ...
            comp.Instance.Palette.Should().BeEquivalentTo(_proGridPaletteDefaultColors);
        ...
        {
            var expectedColor = new ProColor(colorHex);
            await AssertInitializedColorStateAsync(expectedColor, mode);
        }
        ...
        {
            var initialColor = new ProColor("#ff000088");
            var expectedColor = new ProColor("#00ff");
            var comp = Context.Render<SimpleColorPickerTest>(p =>
            {
            ...
            await comp.SetParametersAndRenderAsync(p => p.Add(x => x.ColorValue, expectedColor));
        ...
        public async Task InitializationSnippet_ShouldInitializeHslControls(string testId, string colorHex)
        {
            ...
            await comp.Find($"[data-testid='{testId}']").QuerySelector(".pro-picker-control-switch button")!.ClickAsync();
        ...
        public async Task SetColorInput(string colorHexString, double selectorXPosition, double selectorYPosition)
        {
            ...
            var inputs = comp.FindAll(".pro-picker-color-inputs input");
        ...
        public async Task PointerMove()
        {
            ...
            var expectedColor = new ProColor(74, 70, 112, 255);
        ...
        public async Task Click_ColorPanel()
        {
            ...
            ProColor color = "#232232ff";
            var expectedColor = new ProColor(color.R, color.G, color.B, _defaultColor);
        ...
        public async Task Click_ModeButton()
        {
            ...
            var modeButton = comp.Find(".pro-picker-control-switch button");
        ...
        public async Task ColorPalette_Interaction()
        {
            ...
            // no collection
            Assert.Throws<ElementNotFoundException>(() => comp.Find(".pro-picker-color-collection"));
            await colorDot.ClickAsync();

            // collection found
            var colorsToSelectPanel = comp.Find(".pro-picker-color-collection");
            colorsToSelectPanel.Children.Should().HaveCount(5);

            var expectedColors = new ProColor[] { "#ff4081ff", "#2196f3ff", "#00c853ff", "#ff9800ff", "#f44336ff" };

            for (var i = 0; i < 5; i++)
            {
                var styleAttribute = colorsToSelectPanel.Children[i].GetAttribute("style");
                styleAttribute.Should().Be($"background: {expectedColors[i].ToString(ProColorOutputFormats.RGBA)};");
            }
        ...
        {
            var expectedColors = new ProColor[] { "#23af3daa", "#56a23dff", "#56a85dff" };
        ...
        public async Task Toggle_ColorField()
        {
            ...
            _ = comp.Find(_proColorPickerCssSelector);

            await comp.SetParametersAndRenderAsync(p => p.Add(x => x.ShowColorField, false));

            Assert.Throws<ElementNotFoundException>(() => comp.Find(_proColorPickerCssSelector));
        ...
        {
            var comp = Context.Render<ProColorPicker>(p =>
            {
            ...
            style.Should().Be($"background: {_defaultColor.ToString(ProColorOutputFormats.RGBA)};");
        }
        ...
        public void Tooltips_Disabled_ShouldSuppressTooltipContent()
        {
            ...
            var tooltipRoots = comp.FindAll(".pro-tooltip-root");
        ...
        public void Tooltips_Enabled_ShouldDisplayLocalizedContent()
        {
            ...
            {
                localizer[LanguageResource.ProColorPicker_SpectrumView].Value,
                localizer[LanguageResource.ProColorPicker_GridView].Value,
                localizer[LanguageResource.ProColorPicker_PaletteView].Value,
                localizer[LanguageResource.ProColorPicker_ModeSwitch].Value,
            };
        ...
        {
            var color = new ProColor(12, 220, 124, 120);
            var expectedColor = new ProColor(12, 220, 124, 120);
            ...
            comp.Instance.TextValue.Should().Be(expectedColor.ToString(ProColorOutputFormats.Hex));

            await comp.SetParametersAndRenderAsync(p => p.Add(x => x.ShowAlpha, true));

            _ = comp.Find(_alphaInputCssSelector);
            comp.Instance.ColorValue.Should().Be(expectedColor);
            comp.Instance.TextValue.Should().Be(expectedColor.ToString(ProColorOutputFormats.HexA));
        }
        ...
        public async Task Toggle_Alpha_HexInputMode()
        {
            ...
            var inputs = comp.FindAll(".pro-picker-color-inputfield input");
            ...
            await comp.SetParametersAndRenderAsync(p => p.Add(x => x.ShowAlpha, true));
            comp.Instance.ColorValue.Should().Be(expectedColor);
            inputs = comp.FindAll(".pro-picker-color-inputfield input");
            inputs.Should().ContainSingle();
            inputs.Should().AllBeAssignableTo<IHtmlInputElement>();
        ...
        public async Task ToggleViewMode()
        {
            ...
            IReadOnlyList<IElement> Buttons() => comp.FindAll(_proToolbarButtonsCssSelector);

            Dictionary<int, (ColorPickerView, string)> buttonMapper = new()
            {
                { 2, (ColorPickerView.Palette, ".pro-picker-color-view-collection") },
                { 1, (ColorPickerView.Grid, ".pro-picker-color-grid") },
                { 0, (ColorPickerView.Spectrum, ".pro-picker-color-overlay") },
            };
        ...
        public async Task CloseButtonInToolbarVisible(PickerVariant variant, bool expectedVisibility)
        {
            ...
            {
                Assert.Throws<ElementNotFoundException>(() => comp.Find(".pro-close-picker-button"));
            }
            else
            {
                _ = comp.Find(".pro-close-picker-button");
            }
        }
        ...
        public void ColorPickerView_Selection(ColorPickerView view, int index)
        {
            ...
            var toolbarButtons = comp.FindAll(".pro-toolbar .pro-icon-button button");
            for (var i = 0; i < toolbarButtons.Count; i++)
            {
                if (i == index)
                {
                    toolbarButtons[i].ClassList.Should().Contain(".pro-icon-button-color-primary");
                }
                else
                {
                    toolbarButtons[i].ClassList.Should().NotContain(".pro-icon-button-color-primary");
                }
            }
        }
        ...
        public void PaletteView()
        {
            var comp = Context.Render<SimpleColorPickerTest>(p =>
            {
                ...
                p.Add(x => x.Palette, _proGridPaletteDefaultColors);
            });

            var expectedColors = _proGridPaletteDefaultColors;

            var collectionView = comp.Find(".pro-picker-color-view-collection");
            ...
            for (var i = 0; i < collectionView.Children.Length; i++)
            {
                ...
                colorElement.ClassList.Should().Contain("pro-picker-color-dot");

                var style = colorElement.GetAttribute("style");
                style.Should().Be($"background: {expectedColor.ToString(ProColorOutputFormats.RGBA)};");
            }
        }
        ...
        public async Task PaletteView_ChooseColor()
        {
            ...
            for (var i = 0; i < collectionView.Children.Length; i++)
            {
                ...
                await comp.WaitForAssertionAsync(() => comp.Instance.ColorValue.Should().Be(expectedColor));
                comp.Find(".pro-picker-color-view-collection").Children[i].ClassList.Should().BeEquivalentTo("pro-picker-color-dot", "selected");
        ...
        public void GridView()
        {
            ...
            var expectedColors = _proGridDefaultColors;

            var collectionView = comp.Find(".pro-picker-color-grid");
        ...
        public async Task GridView_ChooseColor()
        {
            ...
            for (var i = 0; i < collectionView.Children.Length; i++)
            {
                ...
                comp.Find(".pro-picker-color-grid").Children[i].ClassList.Should().BeEquivalentTo("pro-picker-color-dot", "selected");
            }
        }
        ...
        public void GridCompactView()
        {
            ...
            var content = comp.Find(".pro-picker-content.pro-picker-color-content");
            content.Children.Should().ContainSingle();

            var gridView = comp.Find(".pro-picker-color-grid");
        ...
        public async Task GridCompact_CloseOnSelect(PickerVariant variant)
        {
            ...
            await (await comp.WaitForElementAsync(".pro-picker-color-grid")).Children[0].ClickAsync();
            await comp.WaitForAssertionAsync(() => comp.Instance.ColorValue.Should().Be(expectedColors[0]));

            Assert.Throws<ElementNotFoundException>(() => comp.Find(".pro-picker-container"));
        }
        ...
        public async Task Palette_CloseOnSelect(PickerVariant variant)
        {
            ...
            await (await comp.WaitForElementAsync(".pro-picker-color-view-collection")).Children[0].ClickAsync();
            await comp.WaitForAssertionAsync(() => comp.Instance.ColorValue.Should().Be(expectedColors[0]));
        ...
        public async Task NoControls_CloseOnSelect(PickerVariant variant, ColorPickerView view)
        {
            ...
            var expectedColors = view == ColorPickerView.Grid ? _proGridDefaultColors : _proGridPaletteDefaultColors;

            IElement item;
            if (view == ColorPickerView.Grid)
            {
                item = comp.Find(".pro-picker-color-grid").Children[0];
            }
            else
            {
                item = comp.Find("[class=\"pro-picker-color-overlay\"][id]");
            }
        ...
        public void TextOutput_Alpha(ColorPickerView view, bool disableAlpha, string expectedOutput)
        {
            var comp = Context.Render<SimpleColorPickerTest>(p =>
            {
                ...
                p.Add(x => x.ColorValue, new ProColor(120, 121, 122, 1.0));
            });
        ...
        public async Task SetNullColor_NothingChanged()
        {
            ...
            comp.FindComponent<ProColorPicker>().Instance.ReadValue.Should().Be(null);
        }
        ...
        public async Task SetHLS_NotChangeRBG_ButCallbackFired()
        {
            ...
            {
                p.Add(x => x.ColorPickerMode, ColorPickerMode.HSL);
                p.Add(x => x.ColorValue, new ProColor(245, 0.35, 0.95, 1.0));
            });

            var sColor = GetColorInput(comp, 1);

            var colorValue = comp.Instance.ColorValue.ToString(ProColorOutputFormats.HexA);
            ...
            var colorValueAfterChange = comp.Instance.ColorValue.ToString(ProColorOutputFormats.HexA);
        ...
        public async Task Spectrum_DragPointer(bool disableDragEffect)
        {
            ...
            var expectedColor1 = new ProColor(35, 34, 50, _defaultColor);
            ...
            var expectedColor2 = new ProColor(74, 70, 112, 255);
        ...
        public async Task CheckPickerPopover()
        {
            ...
            var providerNode = comp.Find(".pro-popover-provider");
            providerNode.Children.Length.Should().BeGreaterThanOrEqualTo(2);

            var popoverNode = providerNode.Children
                .FirstOrDefault(child => child.ClassList.Contains("pro-picker-popover"));
            ...
            [
                ...
                "pro-popover-fixed",
                "pro-popover-open",
                "pro-popover-top-left",
                "pro-popover-anchor-bottom-left",
                "pro-popover-overflow-flip-always",
                "pro-picker-popover",
                "pro-elevation-8",
            ]);
        }

        //https://github.com/ProtonBlazor/ProtonBlazor/issues/4899
        [Test]
        public async Task DistinguishBetweenInternalAndExternalView()
        {
            ...
            var inputField = comp.Find(".pro-input-slot");
            await inputField.ClickAsync();

            //asset that the picker is open in grid mode
            var grid = comp.Find(".pro-picker-color-grid");

            //find spectrum button and click
            var spectrumButton = comp.FindAll(_proToolbarButtonsCssSelector)[1];
            ...
            _ = comp.Find(".pro-picker-color-overlay");
            Assert.Throws<ElementNotFoundException>(() => comp.Find(".pro-picker-color-grid"));
            ...
            Assert.Throws<ElementNotFoundException>(() => comp.Find(".pro-picker-color-overlay"));
            _ = comp.Find(".pro-picker-color-grid");
        }
        ...
        {
            var comp = Context.Render<ProColorPicker>(parameters => parameters
                .Add(x => x.Editable, true));

            await comp.Find("input").ChangeAsync("#180f6fff");
            comp.Instance.GetState(x => x.Text).Should().Be("#180f6fff");
            comp.Instance.GetState(x => x.Value).Should().Be(new ProColor("#180f6fff"));
        }
        ...
        {
            var comp = Context.Render<ProColorPicker>(parameters
        ...
        public void OptionalRGBColorPicker_Should_NotHaveRequiredAttributeAndAriaRequiredShouldBeFalse()
        {
            ...
            comp.FindAll("input:not(.pro-slider-input)").Should().AllSatisfy(input =>
            {
        ...
        {
            var comp = Context.Render<ProColorPicker>(parameters => parameters
                .Add(p => p.Value, new ProColor("#180f6fff"))
