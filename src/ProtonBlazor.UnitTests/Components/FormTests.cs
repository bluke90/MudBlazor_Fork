...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class FormTests : BunitTest
    {
        ...
        public async Task FormIsValid()
        {
            ...
            var textFieldcomp = comp.FindComponent<ProTextField<string>>();
        ...
        public async Task FormIsValidTest3()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var textFields = comp.FindComponents<ProTextField<string>>();
            ...
            comp.FindComponents<ProSwitch<bool>>()[0].Instance.ReadValue.Should().Be(false);
            comp.FindComponents<ProSwitch<bool>>()[1].Instance.ReadValue.Should().Be(false);
            ...
            form.IsValid.Should().Be(true);
            comp.FindComponents<ProSwitch<bool>>()[0].Instance.ReadValue.Should().Be(true);
            comp.FindComponents<ProSwitch<bool>>()[1].Instance.ReadValue.Should().Be(true);
        }
        ...
        public async Task FormIsValidTest4()
        {
            ...
            await comp.WaitForAssertionAsync(() => comp.FindComponent<ProSwitch<bool>>().Instance.ReadValue.Should().Be(true));
        }
        ...
        public async Task FormIsTouched()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var textFieldcomp = comp.FindComponent<ProTextField<string>>();
            var dateComp = comp.FindComponent<ProDatePicker>();
        ...
        public async Task FormIsTouchedAndNestedFormIsNotTouchedWhenParentFormFieldIsTouched()
        {
            ...
            var formsComp = comp.FindComponents<ProForm>();
            var textCompFields = comp.FindComponents<ProTextField<string>>();
        ...
        public async Task FormIsUnTouchedWhenNestedFormTouched()
        {
            ...
            var formsComp = comp.FindComponents<ProForm>();
            var textCompFields = comp.FindComponents<ProTextField<string>>();
            var dateCompFields = comp.FindComponents<ProDatePicker>();
            var form = formsComp[0].Instance;
            var nestedForm = formsComp[1].Instance;
        ...
        public async Task EditFormOnFieldChanged_BlurWithoutValueChange_ShouldNotNotify()
        {
            ...
            var chips = comp.FindAll("span.pro-chip-content");
            chips.Count.Should().Be(3);

            await comp.FindAll("input")[0].BlurAsync();

            chips = comp.FindAll("span.pro-chip-content");
        ...
        public async Task EditFormOnFieldChanged()
        {
            ...
            await comp.FindAll("input")[0].BlurAsync();
            comp.FindComponents<ProTextField<string>>()[0].Instance.ReadText.Should().Be("asdf");
            comp.FindAll("span.pro-chip-content")[0].TextContent.Trim().Should().Be("Field1 changed");
            comp.FindAll("span.pro-chip-content")[1].TextContent.Trim().Should().EndWith("not changed");
            comp.FindAll("span.pro-chip-content")[2].TextContent.Trim().Should().EndWith("not changed");
            await comp.FindAll("input")[1].ChangeAsync(new ChangeEventArgs() { Value = "yxcv" });
            await comp.FindAll("input")[1].BlurAsync();
            comp.FindComponents<ProTextField<string>>()[1].Instance.ReadText.Should().Be("yxcv");
            comp.FindAll("span.pro-chip-content")[0].TextContent.Trim().Should().Be("Field1 changed");
            comp.FindAll("span.pro-chip-content")[1].TextContent.Trim().Should().EndWith("not changed", "Because it has no For, so the change can not be forwarded to the edit context for lack of a FieldIdentifier");
            comp.FindAll("span.pro-chip-content")[2].TextContent.Trim().Should().EndWith("not changed");
            await comp.FindAll("input")[2].ChangeAsync(new ChangeEventArgs() { Value = "qwer" });
            await comp.FindAll("input")[2].BlurAsync();
            comp.FindComponents<ProTextField<string>>()[2].Instance.ReadText.Should().Be("qwer");
            comp.FindAll("span.pro-chip-content")[0].TextContent.Trim().Should().Be("Field1 changed");
            comp.FindAll("span.pro-chip-content")[1].TextContent.Trim().Should().EndWith("not changed");
            comp.FindAll("span.pro-chip-content")[2].TextContent.Trim().Should().EndWith("Field3 changed");
        }
        ...
        public async Task FormWithCheckbox()
        {
            ...
            await comp.FindAll("input")[2].BlurAsync();
            foreach (var tf in comp.FindComponents<ProTextField<string>>())
            {
                tf.Instance.ReadText.Should().NotBeNullOrEmpty();
            }

            comp.FindComponent<ProTextField<int>>().Instance.ReadValue.Should().Be(17);
            // then click the checkbox
            comp.FindComponent<ProCheckBox<bool>>().Instance.ReadValue.Should().Be(true);
            await comp.FindAll("input")[3].ChangeAsync(false); // it was on before
            comp.FindComponent<ProCheckBox<bool>>().Instance.ReadValue.Should().Be(false);
        ...
        public async Task Form_Should_BecomeInValidWhenAConversionErrorOccurs()
        {
            ...
            var textfield = comp.FindComponent<ProTextField<int>>();
        ...
        /// <summary>
        /// Testing the functionality of the ProForm example from the docs.
        /// </summary>
        [Test]
        public async Task ProFormExample()
        {
            var comp = Context.Render<FormValidationTest4>();
            var form = comp.FindComponent<ProForm>().Instance;
            await comp.FindComponent<ProForm>().SetParametersAndRenderAsync(parameters => parameters.Add(x => x.ValidationDelay, 0));
            ...
            var checkbox = comp.FindComponent<ProCheckBox<bool>>();
        ...
        public async Task FormWithRadioGroupIsValid()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var radioGroupcomp = comp.FindComponent<ProRadioGroup<string>>();
        ...
        public async Task FormWithColorPicker()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var colorPickerComp = comp.FindComponent<ProColorPicker>();
            var colorPicker = comp.FindComponent<ProColorPicker>().Instance;
        ...
        public async Task Form_Should_Validate_ColorPicker_When_ColorSelectedViaInputs()
        {
            ...
            await colorPickerComp.SetParametersAndRenderAsync(parameters => parameters.Add(x => x.Validation, new Func<ProColor, string>(color => color != null && color.Value == forbiddenColor.Value ? $"{forbiddenColor.Value} is not allowed" : null)));
        ...
        public async Task Form_Should_ValidateColorPickerTest_When_ColorSelectedViaPicker()
        {
            ...
            await comp.InvokeAsync(() => comp.Find("div.pro-picker-color-dot-current").Click());
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-picker-color-collection").Count.Should().Be(1));

            // set valid color
            await comp.InvokeAsync(() => comp.FindAll("div.pro-picker-color-collection>div.pro-picker-color-dot").Skip(1).First().Click());
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-picker-color-collection").Count.Should().Be(0));
            form.IsTouched.Should().BeTrue();
            form.IsValid.Should().BeTrue();
            form.Errors.Length.Should().Be(0);
            colorPicker.GetState(x => x.Error).Should().BeFalse();
            ...
            // set invalid color
            await comp.InvokeAsync(() => comp.FindAll("div.pro-picker-color-collection>div.pro-picker-color-dot").First().Click());
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-picker-color-collection").Count.Should().Be(0));
            form.IsTouched.Should().BeTrue();
            form.IsValid.Should().BeFalse();
            form.Errors.Length.Should().Be(1);
        ...
        public async Task FormWithDatePicker()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var dateComp = comp.FindComponent<ProDatePicker>();
            var datepicker = comp.FindComponent<ProDatePicker>().Instance;
        ...
        public async Task Form_Should_Validate_DateRangePicker_When_DateRangeSelectedViaInputs()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var dateRangeComp = comp.FindComponent<ProDateRangePicker>();
            var dateRangePicker = comp.FindComponent<ProDateRangePicker>().Instance;
        ...
        public async Task Form_Should_Validate_DateRangePicker_When_DateRangeSelectedViaPicker()
        {
            ...
            await comp.InvokeAsync(() => comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).Click());
            await comp.InvokeAsync(() => comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("11")).Click());
        ...
        public async Task FormWithTimePicker()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var timePickerComp = comp.FindComponent<ProTimePicker>();
            var timePicker = comp.FindComponent<ProTimePicker>().Instance;
            // check initial state: form should not be valid because datepicker is required
            form.IsValid.Should().Be(false);
        ...
        public async Task Form_Should_ValidateTimePicker()
        {
            var comp = Context.Render<FormWithTimePickerTest>();
            var form = comp.FindComponent<ProForm>().Instance;
            var timeComp = comp.FindComponent<ProTimePicker>();
            var timePicker = comp.FindComponent<ProTimePicker>().Instance;
        ...
        public async Task Form_Should_Validate_FileUpload_When_FileAdded()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var fileUploadComp = comp.FindComponent<ProFileUpload<IBrowserFile>>();
            var fileUploadInstance = comp.FindComponent<ProFileUpload<IBrowserFile>>().Instance;
        ...
        /// </summary>
        /// <see cref="https://github.com/ProtonBlazor/ProtonBlazor/issues/1229"/>
        [Test]
        ...
        /// </summary>
        [Test]
        public async Task ProForm_MustNot_ValidateOnInitialRender()
        {
            var comp = Context.Render<FormValidationTest4>();
        ...
        /// <summary>
        /// Testing the functionality of the ProForm example from the docs.
        /// Root ProForm is valid and nested ProForm is invalid
        /// </summary>
        [Test]
        public async Task ProFormExample_FillInValuesRootForm()
        {
            ...
            var textfields = comp.FindComponents<ProTextField<string>>();
            var numericFields = comp.FindComponents<ProNumericField<decimal>>();
        ...
        /// <summary>
        /// Testing the functionality of the ProForm example from the docs.
        /// Root ProForm is invalid and nested ProForm is valid
        /// </summary>
        [Test]
        public async Task ProFormExample_FillInValuesNestedForm()
        {
            var comp = Context.Render<FormValidationTest5>();
        ...
        /// <summary>
        /// Testing the functionality of the ProForm example from the docs.
        /// Both root ProForm and nested ProForm are valid
        /// </summary>
        [Test]
        public async Task ProFormExample_FillInValues()
        {
            var comp = Context.Render<FormValidationTest5>();
            await comp.FindAll("input")[0].InputAsync("Rick Sanchez");
            await comp.FindAll("input")[0].BlurAsync();
        ...
        /// <summary>
        /// Testing error handling of ProFormComponent.ValidateModelWithFullPathOfMember
        /// Validation func throws an error, the error should contain the exception message
        /// </summary>
        [Test]
        public async Task ProFormComponent_ValidationWithModel_UnexpectedErrorInValidationFunc3()
        {
            var comp = Context.Render<FormWithSingleTextField>();
            var form = comp.FindComponent<ProForm>();
            var model = new { data = "asdf" };
            await form.SetParametersAndRenderAsync(parameters => parameters.Add(x => x.Model, model));
            var tf = comp.FindComponent<ProTextField<string>>();
        ...
        /// </summary>
        [Test]
        public async Task ProFormComponent_ValidationWithModelWithNoFor_ShouldShow_ExpectedError()
        {
            var comp = Context.Render<FormWithSingleTextField>();
            var form = comp.FindComponent<ProForm>();
            var model = new { data = "asdf" };
            ...
            await comp.InvokeAsync(tf.Instance.ValidateAsync);
            tf.Instance.GetState(x => x.Error).Should().Be(true);
            tf.Instance.GetState(x => x.ErrorText).Should().Be("For is null, please set parameter For on the form input component of type ProTextField`1");
        }
        ...
        /// </summary>
        [Test]
        public async Task ProFormComponent_AsyncValidationWithModelWithNoFor_ShouldShow_ExpectedError()
        {
            var comp = Context.Render<FormWithSingleTextField>();
            var form = comp.FindComponent<ProForm>();
            var model = new { data = "asdf" };
        ...
        /// <summary>
        /// Testing validation with ProFormComponent.ValidateModelWithFullPathOfMember
        /// </summary>
        [Test]
        public async Task ProFormComponent_ValidationWithModel_UnexpectedErrorInValidationFunc5()
        {
            var comp = Context.Render<FormWithSingleTextField>();
            var form = comp.FindComponent<ProForm>();
            var model = new { data = "asdf" };
        ...
        public async Task FormReset_Should_ClearTextField()
        {
            ...
            var form = comp.FindComponent<ProForm>();
            var textFieldComp = comp.FindComponents<ProTextField<string>>()[1]; //the picker includes a ProTextField, so the ProTextField we want is the second in the DOM
            var textField = textFieldComp.Instance;
        ...
        public async Task FormReset_Should_ClearNumericField()
        {
            var comp = Context.Render<FormResetTest>();
            var form = comp.FindComponent<ProForm>().Instance;
            var numericFieldComp = comp.FindComponent<ProNumericField<int?>>();
        ...
        public async Task FormReset_Should_ClearDatePicker()
        {
            var comp = Context.Render<FormResetTest>();
            var form = comp.FindComponent<ProForm>().Instance;
            var datePickerComp = comp.FindComponent<ProDatePicker>();
        ...
        public async Task FormReset_Should_ClearDateRangePicker()
        {
            var comp = Context.Render<FormResetTest>();
            var form = comp.FindComponent<ProForm>().Instance;
            var dateRangePickerComp = comp.FindComponent<ProDateRangePicker>();
        ...
        /// </summary>
        [Test]
        public void ProForm_Should_RegisterOnlyTopSubscribeToParentFormFormControls()
        {
            var comp = Context.Render<FormTopLevelControlsTest>();
            var form = comp.FindComponent<ProFormTestable>().Instance;
        ...
        /// </summary>
        [Test]
        public void ProForm_Validation_Should_OverrideFieldValidation()
        {
            var comp = Context.Render<FormValidationOverrideFieldValidationTest>();
            var textFields = comp.FindComponents<ProTextField<string>>();
            var numericFields = comp.FindComponents<ProNumericField<int>>();
        ...
        public void FieldValidationWithoutRequiredForm_ShouldNot_Validate()
        {
            ...
            Assert.Throws<ElementNotFoundException>(() => comp.Find(".pro-input-error"));
        }
        ...
        public async Task FieldChangedEventShouldTrigger()
        {
            ...
            var textField = comp.FindComponent<ProTextField<string>>().Instance;
            var numeric = comp.FindComponent<ProNumericField<int>>().Instance;
            var radioGroup = comp.FindComponent<ProRadioGroup<string>>().Instance;
            ...
            var mudFile = comp.FindComponent<ProFileUpload<IBrowserFile>>().Instance;
        ...
        public async Task FieldChangedEventShouldTriggerPicker()
        {
            ...
            var datePicker = comp.FindComponent<ProDatePicker>();
            var timePicker = comp.FindComponent<ProTimePicker>();
            var colorPicker = comp.FindComponent<ProColorPicker>();
            ...
            comp.Instance.FormFieldChangedEventArgs.NewValue.Should().Be(new ProColor("#180f6fff"));
        ...
        public void FormAutoValidationSet()
        {
            ...
            var textComps = comp.FindComponents<ProTextField<string>>();
            var dateComps = comp.FindComponents<ProDatePicker>();
        ...
        public async Task FormReadonly()
        {
            ...
            var textField = comp.FindComponents<ProTextField<string>>()[0];
            var maskedTextField = comp.FindComponents<ProTextField<string>>()[1];
            var checkBox = comp.FindComponent<ProCheckBox<bool>>();
            var radioGroup = comp.FindComponent<ProRadioGroup<string>>();
            var switch_ = comp.FindComponent<ProSwitch<bool>>();
            var select = comp.FindComponent<ProSelect<string>>(); //at present, we can't test if select is readonly based on attribute or classname. A future PR should enable this.
            var colorPicker = comp.FindComponent<ProColorPicker>();
            var datePicker = comp.FindComponent<ProDatePicker>();
            var dateRangePicker = comp.FindComponent<ProDateRangePicker>();
            var timePicker = comp.FindComponent<ProTimePicker>();
            var autocomplete = comp.FindComponent<ProAutocomplete<string>>();
            var numericField = comp.FindComponent<ProNumericField<int>>();
            var fileUpload = comp.FindComponent<ProFileUpload<IBrowserFile>>();
            ...
            checkBox.Find("label").ClassList.Should().NotContain("pro-readonly");
            radioGroup.Find("label").ClassList.Should().NotContain("pro-readonly");
            switch_.Find("label").ClassList.Should().NotContain("pro-readonly");
            ...
            checkBox.Find("label").ClassList.Should().Contain("pro-readonly");
            radioGroup.Find("label").ClassList.Should().Contain("pro-readonly");
            switch_.Find("label").ClassList.Should().Contain("pro-readonly");
        ...
        public async Task FormDisabled()
        {
            ...
            var radioGroup = comp.FindComponent<ProRadioGroup<string>>();
            var switch_ = comp.FindComponent<ProSwitch<bool>>();
            var select = comp.FindComponent<ProSelect<string>>();
            var colorPicker = comp.FindComponent<ProColorPicker>();
            var datePicker = comp.FindComponent<ProDatePicker>();
            var dateRangePicker = comp.FindComponent<ProDateRangePicker>();
            var timePicker = comp.FindComponent<ProTimePicker>();
        ...
        /// <summary>
        /// Ensures the child ProForm correctly inherits ReadOnly and applies it to its children
        /// </summary>
        [Test]
        public async Task FormNestedReadOnly()
        {
            var comp = Context.Render<FormNestedReadOnlyDisabledTest>();
            comp.FindAll(".pro-checkbox.pro-readonly").Count.Should().Be(0);

            await comp.SetParametersAndRenderAsync(parameters => parameters.Add(p => p.ReadOnly, true));
            comp.FindAll(".pro-checkbox.pro-readonly").Count.Should().Be(1);
        ...
        /// <summary>
        /// Ensures the child ProForm correctly inherits Disabled and applies it to its children
        /// </summary>
        [Test]
        public async Task FormNestedDisabled()
        {
            var comp = Context.Render<FormNestedReadOnlyDisabledTest>();
            comp.FindAll(".pro-checkbox.pro-disabled").Count.Should().Be(0);

            await comp.SetParametersAndRenderAsync(parameters => parameters.Add(p => p.Disabled, true));
            comp.FindAll(".pro-checkbox.pro-disabled").Count.Should().Be(1);
        ...
        public async Task FormWithChildForm()
        {
            ...
            var childFormSwitch = comp.Find(".pro-switch-input");
            var parentForm = comp.FindComponent<ProForm>().Instance;
            var parentTextFieldCmp = comp.FindComponent<ProTextField<string>>();
            ...
            var forms = comp.FindComponents<ProForm>();
            ...
            forms = comp.FindComponents<ProForm>();
        ...
        public async Task FormWithCheckBox_When_CheckBoxTickedUsingMouse()
        {
            ...
            var form = comp.FindComponent<ProForm>().Instance;
            var checkBox = comp.FindComponent<ProCheckBox<bool>>().Instance;
        ...
        {
            var comp = Context.Render<ProForm>();

            for (var i = 0; i <= 20; i++)
            {
                await comp.SetParametersAndRenderAsync(parameters => parameters.Add(x => x.Spacing, i));
                comp.Find("form.pro-form").ClassList.Should().Contain($"gap-{i}");
            }
        }
        ...
        public async Task ChildForm_TouchChangedPropagate()
        {
            ...
            forms.Count.Should().Be(2);
            var childForm = forms[1];
            var childTextFieldCmp = childForm.FindComponent<ProTextField<string>>();
            childForm.Instance.IsValid.Should().BeFalse();
            parentForm.IsValid.Should().Be(false);
        ...
        /// <summary>
        /// Regression test for: https://github.com/ProtonBlazor/ProtonBlazor/issues/12012.
