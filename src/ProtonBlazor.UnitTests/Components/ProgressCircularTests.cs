...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class ProgressCircularTests : BunitTest
    {
        ...
        {
            var circular = new ProProgressCircular();
        ...
        public void DefaultStructure(bool indeterminate)
        {
            ...
            var comp = Context.Render<ProProgressCircular>(x =>
                {
            ...
            circleContainer.ClassList.Should().Contain("pro-progress-circular-svg");
            circleContainer.ChildElementCount.Should().Be(1);

            var circleElement = circleContainer.Children[0];
            circleElement.ClassList.Should().Contain("pro-progress-circular-circle");
        ...
        {
            var comp = Context.Render<ProProgressCircular>(x => x.Add(y => y.Rounded, rounded));

            var container = comp.Find(".pro-progress-circular-circle");

            if (rounded)
            {
                container.ClassList.Should().Contain("pro-progress-circular-circle-rounded");
            }
            else
            {
                container.ClassList.Should().NotContain("pro-progress-circular-circle-rounded");
            }
        }
        ...
        {
            var comp = Context.Render<ProProgressCircular>(x => x.Add(y => y.Indeterminate, indeterminate));

            var container = comp.Find(".pro-progress-circular");

            if (indeterminate)
            {
                container.ClassList.Should().Contain("pro-progress-indeterminate");
                container.ClassList.Should().NotContain("pro-progress-static");
            }
            else
            {
                container.ClassList.Should().Contain("pro-progress-static");
                container.ClassList.Should().NotContain("pro-progress-indeterminate");
            }

            var circleContainer = comp.Find(".pro-progress-circular-circle");

            if (indeterminate)
            {
                circleContainer.ClassList.Should().Contain("pro-progress-indeterminate");
                circleContainer.ClassList.Should().NotContain("pro-progress-static");
            }
            else
            {
                circleContainer.ClassList.Should().Contain("pro-progress-static");
                circleContainer.ClassList.Should().NotContain("pro-progress-indeterminate");
            }
        }
        ...
        {
            var comp = Context.Render<ProProgressCircular>(x => x.Add(y => y.Size, size));

            var container = comp.Find(".pro-progress-circular");

            container.ClassList.Should().Contain($"pro-progress-{expectedString}");
        }
        ...
        {
            var comp = Context.Render<ProProgressCircular>(x => x.Add(y => y.Color, color));

            var container = comp.Find(".pro-progress-circular");

            container.ClassList.Should().Contain($"pro-{expectedString}-text");
        }
        ...
        public void ProProgressCircular_ShouldHaveCorrectViewBox_BasedOnStrokeWidth(int strokeWidth, string expectedViewBox)
        {
            var comp = Context.Render<ProProgressCircular>(parameters => parameters
            ...
            var compIndeterminate = Context.Render<ProProgressCircular>(parameters => parameters
                .Add(p => p.StrokeWidth, strokeWidth)
