...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class Pagination : BunitTest
    {
        ...
        public async Task PaginationControlButtonClick(Page controlButton, int numberOfClicks, int initiallySelectedPage, int expectedSelectedPage, bool expectedDisabled)
        {
            ...
            var pagination = comp.FindComponent<ProPagination>().Instance;
            ...
            comp.Find("#pro-pagination-test-selected").TextContent.Should()
        ...
        {
            var comp = Context.Render<PaginationButtonTest>();
            var buttons = comp.FindAll(".pro-pagination-item button");
        ...
        public async Task PaginationControlButtonEventCallback(Page controlButton, int expectedButtonClickedValue)
        {
            ...
            await FindControlButton(comp, controlButton).ClickAsync();

            //Expected values
            comp.Find("#pro-pagination-test-button-clicked").TextContent.Should()
        ...
        public void HidePageButton()
        {
            ...
            comp.FindAll(".pro-pagination-item button").Count.Should().Be(2);
        }
            ...
            int expectedSelectedPage)
        {
            ...
            await comp.FindAll(".pro-pagination-item button")[clickIndexPage + 2].ClickAsync();
        ...
        public async Task PaginationCountWithoutEllipsis(int count, int middleCount, int boundaryCount)
        {
            ...
            var pagination = comp.FindComponent<ProPagination>().Instance;
            await comp.Find(".pro-pagination-test-middle-count input").ChangeAsync(middleCount.ToString());
            await comp.Find(".pro-pagination-test-boundary-count input").ChangeAsync(boundaryCount.ToString());
            ...
            {
                await comp.Find(".pro-pagination-test-count input").ChangeAsync(i.ToString());
                var buttons = comp.FindAll(".pro-pagination-item");
                //Expected values
            ...
            int boundaryCount, string[] expectedValues)
        {
            ...
            var pagination = comp.FindComponent<ProPagination>().Instance;
            //set count variables
            await comp.Find(".pro-pagination-test-count input").ChangeAsync(count.ToString());
            await comp.Find(".pro-pagination-test-middle-count input").ChangeAsync(middleCount.ToString());
            await comp.Find(".pro-pagination-test-boundary-count input").ChangeAsync(boundaryCount.ToString());
            ...
            //Expected values
            var items = comp.FindAll(".pro-pagination-item");
        ...
        public void PaginationStyles()
        {
            ...
            var buttons = comp.FindAll(".pro-pagination-item button");
            var pagination = comp.Find("ul.pro-pagination");
            var paginationItems = comp.FindAll("pro-pagination-item");

            //test if previous and next buttons are hidden
            buttons.Count.Should().Be(8);

            //test if variant is filled
            pagination.ClassName.Should().Contain("pro-pagination-filled");

            //test if color is secondary
            buttons[0].ClassName.Should().Contain("pro-button-filled-secondary");

            //test if items are rectangular
            foreach (var item in paginationItems)
            {
                item.ClassName.Should().Contain("pro-pagination-item-rectangular");
            }

            //test if size is large
            pagination.ClassName.Should().Contain("pro-pagination-large");

            //test if elevation is disabled
            pagination.ClassName.Should().Contain("pro-pagination-disable-elevation");
            ...
            //test if rtl is used
            pagination.ClassName.Should().Contain("pro-pagination-rtl");
        }
    }
}
