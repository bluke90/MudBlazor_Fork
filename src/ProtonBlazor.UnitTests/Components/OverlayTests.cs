...
using Moq;
using ProtonBlazor.UnitTests.TestComponents.Overlay;
...
public class OverlayTests : BunitTest
{
    ...
    public void ShouldNotRenderByDefault()
    {
        ...
        var comp = Context.Render<ProOverlay>();
        comp.Markup.Should().BeEmpty();
        providerComp.FindAll("div.pro-overlay").Count.Should().Be(0);
    }
    ...
    {
        var providerComp = Context.Render<ProPopoverProvider>();
        var comp = Context.Render<ProOverlay>(parameters => parameters
            .Add(p => p.Visible, true)
        );

        providerComp.FindAll("div.pro-overlay").Count.Should().Be(1);
    }
    ...
    public async Task AutoClose_OnClick(bool autoClose)
    {
        ...
        await providerComp.Find("div.pro-overlay").ClickAsync(new());
    ...
    public void ShouldApplyCorrectZIndex()
    {
        ...
        providerComp.Find("div.pro-overlay").Attributes["style"].Value.Should().Contain("z-index:10");
    }
    ...
    public void ShouldApplyBackgroundColor(bool darkBackground, bool lightBackground)
    {
        ...
        if (darkBackground || lightBackground)
        {
            if (darkBackground)
            {
                providerComp.Find("div.pro-overlay-scrim").ClassList.Should().Contain("pro-overlay-dark");
            }

            if (lightBackground)
            {
                providerComp.Find("div.pro-overlay-scrim").ClassList.Should().Contain("pro-overlay-light");
            }
        }
        else
        {
            providerComp.FindAll("div.pro-overlay-scrim").Count.Should().Be(0);
        }
    }
    ...
    public void ShouldApplyAbsoluteClass(bool absolute)
    {
        ...
        if (absolute)
        {
            comp.Find("div.pro-overlay").ClassList.Should().Contain("pro-overlay-absolute");
        }
        else
        {
            providerComp.Find("div.pro-overlay").ClassList.Should().NotContain("pro-overlay-absolute");
        }
    }
    ...
    public void ShouldApplyCorrectPointerEvents(bool modal)
    {
        ...
        if (modal)
        {
            providerComp.Find("div.pro-overlay").Attributes["style"].Value.Should().NotContain("pointer-events:none");
        }
        else
        {
            providerComp.Find("div.pro-overlay").Attributes["style"].Value.Should().Contain("pointer-events:none");
        }
    }
    ...
    public void ShouldHaveId()
    {
        ...
        providerComp.Find("div.pro-overlay").Attributes["id"].Value.Should().NotBeNullOrEmpty();
    }

    [Test]
    [TestCase(true, "", false, 0)] // Absolute is true
    [TestCase(false, "pro-skip-overlay-section", false, 1)] // Dialog
    ...
    public void ShouldRender_SectionLocation(bool absolute, string expectedClass, bool hasChildContent, int testNum)
    {
        ...
        var providerComp = Context.Render<ProPopoverProvider>();
        IRenderedComponent<ProOverlay> comp;
        if (hasChildContent)
        {
            comp = Context.Render<ProOverlay>(parameters => parameters
            .Add(p => p.Visible, true)
        ...
        var countInProvider = providerComp.FindAll("div.pro-overlay");
        var countInComp = comp.FindAll("div.pro-overlay");
    ...
    public void Overlay_ShouldHaveElementId_AndMatchRenderedDivId()
    {
        ...
        var overlayDiv = providerComp.Find("div.pro-overlay");
    ...
    public async Task Overlay_StartClosed()
    {
        ...
        var dialog = Context.Render<ProDialogProvider>();
        var comp = Context.Render<OverlayScrollLockedTest>();
        // click the button opening dialog
        var button = comp.Find("button");
        await button.ClickAsync();
        // verify dialog is open
        await comp.WaitForAssertionAsync(() => dialog.FindComponent<ProOverlay>().Should().NotBeNull());
