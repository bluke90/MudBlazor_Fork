...
{
    [Test]
    public async Task ProAxisChartBase_MatchBoundsToSize_ShouldMatchMeasuredSizeExactly()
    {
        ...
        ProAxisChartBase<double, BarChartOptions> axisChartBase = barComponent.Instance;
        ...
        {
            var svg = comp.Find("svg.pro-chart-bar");
            svg.GetAttribute("viewBox").Should().Be($"0 0 {MeasuredWidth} {MeasuredHeight}");
        });
    }

    [Test]
    public async Task ProAxisChartBase_LineChart_MatchBoundsToSize_ShouldMatchMeasuredSizeExactly()
    {
        var chartSeries = new List<ChartSeries<double>> { new() { Name = "Series 1", Data = new double[] { 10, 20 } }, };
        var comp = Context.Render<ProChart<double>>(parameters => parameters
        ...
        ProAxisChartBase<double, LineChartOptions> axisChartBase = lineComponent.Instance;
        ...
        await comp.WaitForAssertionAsync(() =>
        {
            var svg = comp.Find("svg.pro-chart-line");
            svg.GetAttribute("viewBox").Should().Be($"0 0 {MeasuredWidth} {MeasuredHeight}");
        });
    }

    [Test]
    public void ProAxisChartBase_YAxisTitle_ShouldAllocateSpaceAndPositionCorrectly()
    {
        var chartSeries = new List<ChartSeries<double>> { new() { Name = "Series 1", Data = new double[] { 10, 20 } } };

        // Render without title
        var compWithoutTitle = Context.Render<ProChart<double>>(parameters => parameters
            .Add(p => p.ChartType, ChartType.Bar)
            .Add(p => p.ChartSeries, chartSeries));

        var gridWithoutTitle = compWithoutTitle.Find("g.pro-charts-gridlines-yaxis path");
        ...
        // Render with title
        var compWithTitle = Context.Render<ProChart<double>>(parameters => parameters
            .Add(p => p.ChartType, ChartType.Bar)
            .Add(p => p.ChartSeries, chartSeries)
            .Add(p => p.ChartOptions, new BarChartOptions { YAxisTitle = "Title" }));

        var gridWithTitle = compWithTitle.Find("g.pro-charts-gridlines-yaxis path");
    ...
    [Test]
    public async Task ProChart_MatchBoundsToSize_NoFixedParent_ShouldUseFallbackHeight()
    {
    ...
    [Test]
    public async Task ProChart_MatchBoundsToSize_WithFixedParent_ShouldNotUseFallbackHeight()
    {
        var series = new List<ChartSeries<double>> { new() { Data = new double[] { 10, 20 } } };
