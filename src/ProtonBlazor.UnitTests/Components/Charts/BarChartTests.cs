...
using ProtonBlazor.UnitTests.Components;
...
namespace ProtonBlazor.UnitTests.Charts
{
    public class BarChartTests : BunitTest
    {
        ...
        public void BarChartXAxisLabelRotation90UsesRotatedLabelSpacing()
        {
            ...
            var unrotated = Context.Render<ProChart<double>>(parameters => parameters
                .Add(p => p.ChartType, ChartType.Bar)
                .Add(p => p.Height, "350px")
                .Add(p => p.Width, "100%")
                .Add(p => p.ChartSeries, chartSeries)
            ...
            var rotated = Context.Render<ProChart<double>>(parameters => parameters
                .Add(p => p.ChartType, ChartType.Bar)
                .Add(p => p.Height, "350px")
                .Add(p => p.Width, "100%")
                .Add(p => p.ChartSeries, chartSeries)
            ...
            var unrotatedXAxisLabel = unrotated.Find("g.pro-charts-xaxis text");
            var rotatedXAxisLabel = rotated.Find("g.pro-charts-xaxis text");
            ...
            var unrotatedPlotBottom = GetYCoordinate(unrotated.Find("g.pro-charts-gridlines-yaxis path").GetAttribute("d")!);
            var rotatedPlotBottom = GetYCoordinate(rotated.Find("g.pro-charts-gridlines-yaxis path").GetAttribute("d")!);
        ...
        public async Task BarChart_CanHideSeries()
        {
            ...
            comp.FindAll($"path.pro-chart-bar{series3}").Count.Should().Be(0, "Series 3 should have 0 bars initially");

            // Hide Series 1
            comp.FindAll($"path.pro-chart-bar{series1}").Count.Should().Be(chartSeries[0].Data.Values.Count, "Series 1 bars should be visible");
            ...
            comp.FindAll($"path.pro-chart-bar{series1}").Count.Should().Be(0, "Series 1 bars should be hidden after unchecking");
            ...
            comp.FindAll($"path.pro-chart-bar{series1}").Count.Should().Be(chartSeries[0].Data.Values.Count, "Series 1 bars should be visible again after re-checking");

            // Now check Series 2 (which should have been visible from the start)
            comp.FindAll($"path.pro-chart-bar{series2}").Count.Should().Be(chartSeries[1].Data.Values.Count, "Series 2 should have bars from the start");
            ...
            comp.FindAll($"path.pro-chart-bar{series2}").Count.Should().Be(0, "Series 2 bars should be hidden");
            comp.FindAll($"path.pro-chart-bar{series1}").Count.Should().Be(chartSeries[0].Data.Values.Count, "Series 1 bars should still be visible"); // Ensure other series not affected
            ...
            comp.FindAll($"path.pro-chart-bar{series3}").Count.Should().Be(chartSeries[2].Data.Values.Count, "Series 3 bars should be visible");
        }
    }
}
