...
public class SplineInterpolationTests : BunitTest
{
    ...
    public void SplineInterpolation_ClampToZero_ShouldIncludeZeroOnYAxis_ForPositiveOnlyData()
    {
        ...
        var unclampedComp = Context.Render<ProChart<double>>(parameters => parameters
        ...
        var unclampedYAxisLabels = unclampedComp.FindAll("g.pro-charts-yaxis text").Select(e => e.TextContent.Trim()).ToList();
        unclampedYAxisLabels.Should().NotContain(label => label == "0");

        var clampedComp = Context.Render<ProChart<double>>(parameters => parameters
            .Add(p => p.ChartType, ChartType.Line)
            .Add(p => p.ChartSeries, chartSeries)
            .Add(p => p.ChartLabels, chartLabels)
            .Add(p => p.ChartOptions, new LineChartOptions
        ...
        var clampedYAxisLabels = clampedComp.FindAll("g.pro-charts-yaxis text").Select(e => e.TextContent.Trim()).ToList();
