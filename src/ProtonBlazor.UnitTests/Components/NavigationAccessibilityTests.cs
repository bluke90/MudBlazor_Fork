...
public class NavigationAccessibilityTests : BunitTest
{
    ...
    public void ActiveNavLinkInActiveNavGroup_Should_BeFocusable()
    {
        ...
        comp.FindAll("#second-level-navgroup a.pro-nav-link:not(.pro-nav-link-disabled)").Should().HaveCount(1);
        comp.FindAll("#second-level-navgroup a.pro-nav-link:not(.pro-nav-link-disabled)")
            .Should()
            .AllSatisfy(navLink =>
                navLink
                    .GetAttribute("tabindex")
    ...
    public void DisabledNavLinksInActiveNavGroup_Should_NotBeFocusable()
    {
        ...
        comp.FindAll("#second-level-navgroup a.pro-nav-link.pro-nav-link-disabled").Should().HaveCount(2);
        comp.FindAll("#second-level-navgroup a.pro-nav-link.pro-nav-link-disabled")
            .Should()
            .AllSatisfy(navLink =>
                navLink
                    .GetAttribute("tabindex")
    ...
    /// <summary>
    /// ProCollapse components within collapsed NavGroups should be aria-hidden.
    /// </summary>
    [Test]
    public void ProCollapseWithinCollapsedNavGroup_Should_BeAriaHidden()
    {
        var comp = Context.Render<NavigationAccessibilityTest>(parameters =>
            parameters.Add(p => p.TopLevelExpanded, false));

        comp.FindAll(".pro-collapse-container").Should().HaveCountGreaterThan(0).And.AllSatisfy(node => node.GetAttribute("aria-hidden").Should().Be("true"));
    }

    /// <summary>
    /// ProCollapse components within expanded NavGroups should not be aria-hidden.
    /// </summary>
    [Test]
    public void ProCollapseWithinExpandedNavGroup_Should_NotBeAriaHidden()
    {
        var comp = Context.Render<NavigationAccessibilityTest>();

        comp.FindAll("#second-level-navgroup .pro-collapse-container").Should().HaveCountGreaterThan(0).And.AllSatisfy(node => node.GetAttribute("aria-hidden").Should().Be("false"));
    }
    ...
    {
        var comp = Context.Render<NavigationAccessibilityTest>();
        var navGroups = comp.FindComponents<ProNavGroup>();
    ...
    public void NavGroupButtons_Should_HaveValidAriaControlsValue_And_NavMenus_Should_HaveAnId()
    {
        var comp = Context.Render<NavigationAccessibilityTest>();
        var navGroups = comp.FindComponents<ProNavGroup>();
        var navMenus = comp.FindComponents<ProNavMenu>();
