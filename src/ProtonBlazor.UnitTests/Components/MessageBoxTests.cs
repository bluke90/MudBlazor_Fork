...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class MessageBoxTests : BunitTest
    {
        ...
        public async Task MessageBox_Should_ReturnTrue(int clickButtonIndex, bool? expectedResult)
        {
            ...
            comp.Find("div.pro-message-box").Should().NotBe(null);
            comp.Find("div.pro-dialog-container").Should().NotBe(null);
            comp.Find("div.pro-dialog-title").TrimmedText().Should().Contain("Boom!");
            comp.Find("div.pro-dialog-content").TrimmedText().Should().Contain("pickle");

            // Assert there are exactly 3 buttons
            var buttons = comp.FindAll(".pro-dialog-actions button");
            ...
            buttons[0].ClassList.Should().Contain("pro-message-box__cancel-button");
            buttons[1].TrimmedText().Should().Be("Whatever"); // Second button (No)
            buttons[1].ClassList.Should().Contain("pro-message-box__no-button");
            buttons[2].TrimmedText().Should().Be("Great");    // Third button (Yes)
            buttons[2].ClassList.Should().Contain("pro-message-box__yes-button");

            // close message box by clicking on Great.
            await comp.FindAll(".pro-dialog-actions button")[clickButtonIndex].ClickAsync();
            comp.Markup.Trim().Should().BeEmpty();
        ...
        public async Task MessageBox_CloseOnEscapeKey_NoOptions_NoMudDefaults()
        {
            ...
            await comp.InvokeAsync(async () =>
            {
                ...
                dialogReference = await service.ShowAsync<ProMessageBox>(messageBoxOptions.Title, parameters);
                dialogResult = dialogReference.Result;
            });
            dialogReference.Should().NotBeNull();
            // this component has an instance of ProDialog as a cascading parameter allowing us to access HandleKeyDown
            var dialog = (ProMessageBox)dialogReference.Dialog!;
        ...
        public async Task MessageBox_CloseOnEscapeKey_WithOptions_NoMudDefaults()
        {
            ...
            await comp.InvokeAsync(async () =>
            {
                ...
                dialogReference = await service.ShowAsync<ProMessageBox>(messageBoxOptions.Title, parameters, dialogOptions);
                dialogResult = dialogReference.Result;
            });
            dialogReference.Should().NotBeNull();
            // this component has an instance of ProDialog as a cascading parameter allowing us to access HandleKeyDown
        ...
        {
            var keyInterceptorService = Context.AddKeyInterceptorService();
            var comp = Context.Render<ProDialogProvider>(builder =>
            {
        ...
        {
            var provider = Context.Render<ProDialogProvider>(builder => builder.Add(x => x.BackgroundClass, "global-background"));
            var service = Context.Services.GetService<IDialogService>() as DialogService;
            ...
            provider.Find("div.pro-overlay-dialog").ClassList.Should().Contain("global-background");
            await provider.Find(".pro-message-box__yes-button").ClickAsync();
        ...
        public async Task MessageBox_Should_PreferExplicitBackgroundClass_OverGlobalBackgroundClass()
        {
            ...
            var overlayClasses = provider.Find("div.pro-overlay-dialog").ClassList;
        ...
        {
            var service = Context.Services.GetService<IDialogService>() as DialogService;
            var provider = Context.Render<ProDialogProvider>(builder =>
            {
            ...
            var dialogMessageBox = provider.FindComponent<ProMessageBox>();
            dialogMessageBox.Instance.IsButtonOrderReversed.Should().BeTrue();

            var buttons = provider.FindAll(".pro-dialog-actions button");
            buttons.Count.Should().Be(3);
            buttons[0].TrimmedText().Should().Be("Yes");
            buttons[0].ClassList.Should().Contain("pro-message-box__yes-button");
            buttons[1].TrimmedText().Should().Be("No");
            buttons[1].ClassList.Should().Contain("pro-message-box__no-button");
            buttons[2].TrimmedText().Should().Be("Cancel");
            buttons[2].ClassList.Should().Contain("pro-message-box__cancel-button");
        ...
        public async Task InlineMessageBox_ShouldNot_RenderReverseButtonOrder()
        {
            ...
            var inlineMessageBox = Context.Render<ProMessageBox>(parameters => parameters
                .Add(p => p.YesText, "Yes")
                .Add(p => p.NoText, "No")
