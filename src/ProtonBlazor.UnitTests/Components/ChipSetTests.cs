...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class ChipSetTests : BunitTest
    {
        ...
        public async Task ChipSet_SingleSelection()
        {
            ...
            comp.Find("div.selected-values").TrimmedText().Should().Be("Nothing selected");
            // select milk
            await comp.FindAll("button.pro-chip")[0].ClickAsync();
        ...
        public async Task ChipSet_MultiSelection()
        {
            ...
            // select red wine
            await comp.FindAll("button.pro-chip")[6].ClickAsync();
            comp.Find("div.selected-value").TrimmedText().Should().Be("");
        ...
        public async Task ChipSet_MultiSelection_DefaultChipsShouldBeInitiallySelected()
        {
            ...
            // select eggs
            await comp.FindAll("button.pro-chip")[1].ClickAsync();
        ...
        public async Task ChipSet_MultiSelection_DefaultChipsShouldOverrideInitiallySelected()
        {
            ...
            comp.Find(".selected-values").TrimmedText().Should().Be("Eggs, Milk");
            // select soap
            await comp.FindAll("button.pro-chip")[2].ClickAsync();
        ...
        public async Task ChipSet_ReadOnly()
        {
            ...
            // no chip should have pro-clickable or pro-ripple classes
            var chipset = comp.FindComponent<ProChipSet<string>>();
            comp.FindAll("div.pro-clickable").Count.Should().Be(0);
            comp.FindAll("div.pro-ripple").Count.Should().Be(0);

            //Click test
            comp.FindAll(".pro-chip")[0].TagName.Should().Be("DIV");

            //Should not throw an error because it won't click
            await comp.FindAll("button.pro-chip-close-button")[0].ClickAsync();
        ...
        public async Task ChipSet_SelectedValues_TwoWayBinding()
        {
            ...
            comp.FindComponents<ProChip<int>>()[0].Find(".pro-chip").ClassList.Should().Contain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[1].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[2].Find(".pro-chip").ClassList.Should().Contain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[3].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");

            // change selection and check state of both sets
            await comp.FindAll("button.pro-chip")[0].ClickAsync();
            comp.Find("p.set").TrimmedText().Should().Be("Selection:");
            comp.FindComponents<ProChip<int>>()[0].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[1].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[2].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[3].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");
            await comp.FindAll("button.pro-chip")[1].ClickAsync();
            comp.Find("p.set").TrimmedText().Should().Be("Selection: 2");
            comp.FindComponents<ProChip<int>>()[0].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[1].Find(".pro-chip").ClassList.Should().Contain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[2].Find(".pro-chip").ClassList.Should().NotContain("pro-chip-selected");
            comp.FindComponents<ProChip<int>>()[3].Find(".pro-chip").ClassList.Should().Contain("pro-chip-selected");
            await comp.FindAll("button.pro-chip")[2].ClickAsync();
        ...
        public async Task ChipSetComparer()
        {
            ...
            await comp.FindComponent<ProButton>().Find("button").ClickAsync();
        ...
        public void Chip_GetValue_ShouldReturnTextIfValueIsNullAndT_IsString()
        {
            ...
            Context.Render<ProChip<string>>(p => p
                .Add(x => x.Text, "はい")
            ...
            Context.Render<ProChip<int?>>(p => p
                .Add(x => x.Text, "Zero")
        ...
        {
            var comp = Context.Render<ProChipSet<string>>(self => self
                .Add(x => x.CheckMark, true)
                .Add(x => x.SelectedValue, "x")
                .AddChildContent<ProChip<string>>(chip => chip.Add(x => x.Value, "x"))
            ...
            comp.Instance.CheckMark.Should().Be(false);
            // for coverage
            new ProChip<int>().ShowCheckMark.Should().Be(false);
            var chip = Context.Render<ProChip<string>>(chip => chip
                .Add(x => x.CheckedIcon, Icons.Material.Filled.Cake)
                .Add(x => x.CloseIcon, Icons.Material.Filled.Plagiarism)
                .Add(x => x.Ripple, false)
                .Add(x => x.IconColor, Color.Error)
            ...
            Context.Render<ProChip<int>>(self => self.Add(x => x.Variant, (Variant)69)).Instance.GetVariant().Should().Be(Variant.Outlined); // falls back to outlined
        }
        ...
        public async Task ChipSet_RemoveChip_Logic()
        {
            ...
            var comp = Context.Render<ProChipSet<string>>(self => self
                ...
                .AddChildContent<ProChip<string>>(chip => chip.Add(x => x.Value, "x"))
                .AddChildContent<ProChip<string>>(chip => chip.Add(x => x.Value, "y"))
                .AddChildContent<ProChip<string>>(chip => chip.Add(x => x.Value, "z"))
            );
            await comp.InvokeAsync(() => comp.Instance.RemoveAsync(comp.FindComponent<ProChip<string>>().Instance));
            string.Join(", ", selectedValues).Should().Be("y, z");
            // removing a foreign chip doesn't do anything
            await comp.Instance.RemoveAsync(Context.Render<ProChip<string>>(chip => chip.Add(x => x.Value, "y")).Instance);
            string.Join(", ", selectedValues).Should().Be("y, z");
            // removing from a disposed chipset doesn't raise events, so in this case the selection stays the same
            var chipY = comp.FindComponent<ProChip<string>>().Instance;
        ...
        public async Task ChipSet_With_NonValueTypes_DoesntCrash()
        {
            ...
            var comp = Context.Render<ProChipSet<object>>(self => self
                .Add(x => x.SelectionMode, SelectionMode.MultiSelection)
                .Bind(x => x.SelectedValues, selectedValues, x => selectedValues = x)
                .AddChildContent<ProChip<object>>(chip => chip.Add(x => x.Value, a))
                .AddChildContent<ProChip<object>>(chip => chip.Add(x => x.Value, b))
                .AddChildContent<ProChip<object>>(chip => chip.Add(x => x.Value, c))
            );
            await comp.FindAll("button.pro-chip")[1].ClickAsync();
        ...
        public async Task Chip_TwoWayBinding_ShouldUpdateSelection()
        {
            ...
            // initial state
            comp.FindAll(".pro-chip")[0].ClassList.Should().NotContain("pro-chip-selected");
            comp.FindAll(".pro-chip")[2].ClassList.Should().NotContain("pro-chip-selected");
            comp.FindAll(".pro-checkbox .pro-icon-button")[0].ClassList.Should().Contain("pro-checkbox-false");
            comp.FindAll(".pro-checkbox .pro-icon-button")[2].ClassList.Should().Contain("pro-checkbox-false");

            // click Vodka chip
            await comp.FindAll("button.pro-chip")[0].ClickAsync();
            comp.Find("div.selection").TrimmedText().Should().Be("Vodka");
            comp.FindAll(".pro-chip")[0].ClassList.Should().Contain("pro-chip-selected");
            comp.FindAll(".pro-chip")[2].ClassList.Should().NotContain("pro-chip-selected");
            comp.FindAll(".pro-checkbox .pro-icon-button")[0].ClassList.Should().Contain("pro-checkbox-true");
            comp.FindAll(".pro-checkbox .pro-icon-button")[2].ClassList.Should().Contain("pro-checkbox-false");

            // click Olive checkbox
            await comp.FindAll("input.pro-checkbox-input")[2].ChangeAsync(true);
            comp.Find("div.selection").TrimmedText().Should().Be("Olive, Vodka");
            comp.FindAll(".pro-chip")[0].ClassList.Should().Contain("pro-chip-selected");
            comp.FindAll(".pro-chip")[2].ClassList.Should().Contain("pro-chip-selected");
            comp.FindAll(".pro-checkbox .pro-icon-button")[0].ClassList.Should().Contain("pro-checkbox-true");
            comp.FindAll(".pro-checkbox .pro-icon-button")[2].ClassList.Should().Contain("pro-checkbox-true");

            // click Vodka checkbox
            await comp.FindAll("input.pro-checkbox-input")[0].ChangeAsync(false);
        ...
        public async Task Should_provide_accessible_keyboard_navigation()
        {
            ...
            comp.FindComponent<ProChipSet<string>>().Instance.SelectedValues.Should().BeNullOrEmpty();
            comp.FindComponents<ProChip<string>>().Should().HaveCount(2);

            // pressing a chip using Space or Enter should toggle their state
            comp.Find("#chip-1").KeyDown(" ");
            //comp.Find("#chip-2").KeyDown("Enter");
            await comp.Find("#chip-2").ClickAsync(); // https://github.com/ProtonBlazor/ProtonBlazor/pull/10488#issuecomment-2558409773
            comp.FindComponent<ProChipSet<string>>().Instance.SelectedValues.Should().HaveCount(2);
        ...
        public async Task Should_not_accept_keyboard_inputs_when_disabled_or_readonly()
        {
            ...
            comp.Find("#chip-1").KeyDown(" ");
            comp.Find("#chip-2").KeyDown("Enter");
            comp.FindComponent<ProChipSet<string>>().Instance.SelectedValues.Should().HaveCount(0);
