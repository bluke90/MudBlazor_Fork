...
public class SplitPanelTests : BunitTest
{
    ...
    public void RendersCorrectly()
    {
        ...
        comp.FindAll(".pro-split-panel").Count.Should().Be(1);
    ...
    {
        var comp = Context.Render<ProSplitPanel>(parameters => parameters
        ...
        comp.FindAll(".pro-split-panel.flex-column.absolute").Count.Should().Be(1);

        var childPanels = comp.FindAll(".child-panel");
        foreach (var panel in childPanels)
        {
            panel.ClassList.Should().Contain("pro-elevation-1");
