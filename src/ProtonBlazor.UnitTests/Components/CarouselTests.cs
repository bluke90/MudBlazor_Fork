...
using ProtonBlazor.UnitTests.TestComponents.Carousel;
...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class CarouselTests : BunitTest
    {
        ...
        public async Task CarouselTest1()
        {
            ...
            var carouselComponent = comp.FindComponent<ProCarousel<object>>();
            var carousel = carouselComponent.Instance;
            // validating some renders
            carousel.Should().NotBeNull();
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-carousel-item").Count.Should().Be(1));
            ...
            comp.FindAll("button.pro-icon-button").Count.Should().Be(5); //left + right + 3 pages
            ...
            last = carousel.SelectedContainer;
            var swipe = comp.Find("div.pro-carousel-swipe");
        ...
        {
            var comp = Context.Render<ProCarousel<object>>();
            // print the generated html
            comp.FindAll("button.pro-icon-button").Count.Should().Be(2); //left + right
            // adding some pages
            comp.Instance.Items.Add(Context.Render<ProCarouselItem>().Instance);
            comp.Instance.Items.Add(Context.Render<ProCarouselItem>().Instance);
            comp.Instance.Items.Add(Context.Render<ProCarouselItem>().Instance);
            comp.Render();
            // playing with params
            comp.FindAll("button.pro-icon-button").Count.Should().Be(5); //left + right + 3 items
            await comp.SetParametersAndRenderAsync(parameters => parameters.Add(p => p.ShowArrows, false));
            comp.FindAll("button.pro-icon-button").Count.Should().Be(3);
            await comp.SetParametersAndRenderAsync(parameters => parameters.Add(p => p.ShowBullets, false));
            comp.FindAll("button.pro-icon-button").Count.Should().Be(0);
            await comp.SetParametersAndRenderAsync(parameters => parameters.Add(p => p.ShowArrows, true));
            comp.FindAll("button.pro-icon-button").Count.Should().Be(2);
            await comp.SetParametersAndRenderAsync(parameters => parameters.Add(p => p.ShowBullets, true));
            comp.FindAll("button.pro-icon-button").Count.Should().Be(5);
        ...
        public void Carousel_SelectedIndexTransition()
        {
            ...
            comp.Render();
            comp.Find(".pro-carousel-transition-slide-next-enter.fake-class-item1");
            comp.FindAll(".pro-carousel-item").Should().HaveCount(1);

            // Slide next
            comp.Instance.SelectedIndex = 2;
            comp.Render();
            comp.Find(".pro-carousel-transition-slide-next-exit.fake-class-item1");
            comp.Find(".pro-carousel-transition-slide-next-enter.fake-class-item3");

            // Slide prev
            comp.Instance.SelectedIndex = 0;
            comp.Render();
            comp.Find(".pro-carousel-transition-slide-prev-exit.fake-class-item3");
            comp.Find(".pro-carousel-transition-slide-prev-enter.fake-class-item1");
        ...
        public async Task Carousel_DisableSwipeGesture()
        {
            ...
            var mudSwipeArea = comp.FindComponent<ProSwipeArea>().Instance;
        ...
        public void Carousel_DataBinding()
        {
            ...
            // print the generated html
            //// select elements needed for the test
            var carousel = comp.FindComponent<ProCarousel<string>>().Instance;
            //// validating some renders
            carousel.Should().NotBeNull();
            carousel.MoveTo(0);
        ...
        public async Task Carousel_AddBullets()
        {
            ...
            // adding one page
            await comp.InvokeAsync(() => comp.Instance.AddItem(Context.Render<ProCarouselItem>().Instance));

            // check the new button amount
            comp.FindAll("button.pro-icon-button").Count.Should().Be(3); //left + right + 1 item
            ...
            comp.FindAll("button.pro-icon-button").Count.Should().Be(6); //left + right + 4 items
        }
        ...
        public async Task Carousel_ArrowsColor()
        {
            ...
            comp.FindAll("button.pro-inherit-text").Count.Should().Be(2);
            comp.FindAll("button.pro-primary-text").Count.Should().Be(0);
            ...
            comp.FindAll("button.pro-inherit-text").Count.Should().Be(0);
            comp.FindAll("button.pro-primary-text").Count.Should().Be(2);
