...
using ProtonBlazor.UnitTests.TestComponents.List;
...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class ListTests : BunitTest
    {
        ...
        public async Task ListRender()
        {
            ...
            var listItem = comp.FindComponent<ProListItem<string>>();
        ...
        {
            var comp = Context.Render<ListSelectionTest>();
            var list = comp.FindComponent<ProList<string>>().Instance;
            list.SelectedValue.Should().Be(null);
            // we have seven choices, none is active
            comp.FindAll("div.pro-list-item").Count.Should().Be(9); // 7 choices, 2 groups
            comp.FindAll("div.pro-selected-item").Count.Should().Be(0);
            // click water
            await comp.FindAll("div.pro-list-item")[0].ClickAsync();
            list.SelectedValue.Should().Be("Sparkling Water");
            comp.FindAll("div.pro-selected-item").Count.Should().Be(1);
            comp.FindComponents<ProListItem<string>>()[0].Markup.Should().Contain("pro-selected-item");
            // click Pu'er, a heavily fermented Chinese tea that tastes like an old leather glove
            await comp.FindAll("div.pro-list-item")[4].ClickAsync();
            list.SelectedValue.Should().Be("Pu'er");
            comp.FindAll("div.pro-selected-item").Count.Should().Be(1);
            comp.FindComponents<ProListItem<string>>()[4].Markup.Should().Contain("pro-selected-item");
            // click Cafe Latte
            await comp.FindAll("div.pro-list-item")[8].ClickAsync();
            list.SelectedValue.Should().Be("Cafe Latte");
            comp.FindAll("div.pro-selected-item").Count.Should().Be(1);
            comp.FindComponents<ProListItem<string>>()[8].Markup.Should().Contain("pro-selected-item");
        ...
        public async Task ListToggleSelection()
        {
            ...
            list.SelectedValue.Should().Be(null);
            comp.FindAll("div.pro-selected-item").Count.Should().Be(0);
            comp.FindComponents<ProListItem<string>>()[8].Markup.Should().NotContain("pro-selected-item");
        }
        ...
        public void ListMultiSelectionInitialValues()
        {
            ...
            var GetCheckBox = (string text) => comp.FindComponents<ProListItem<string>>().FirstOrDefault(x => x.Instance.Text == text)?.FindComponent<ProCheckBox<bool?>>().Instance;
        ...
        public async Task ListMultiSelection_DisabledItems_ShouldDisplaySelectedStateAndNotBeClickable()
        {
            ...
            var appleItem = comp.FindComponents<ProListItem<string>>()
                .FirstOrDefault(x => x.Instance.Text == "Apple Juice");
            var orangeItem = comp.FindComponents<ProListItem<string>>()
                .FirstOrDefault(x => x.Instance.Text == "Orange Juice");
            await appleItem.Find("div.pro-list-item").ClickAsync();
            await orangeItem.Find("div.pro-list-item").ClickAsync();
        ...
        public async Task ListMultiSelectionBinding()
        {
            ...
            var list1 = comp.FindComponents<ProList<string>>().FirstOrDefault(x => x.Instance.Class == "list-1");
            var list2 = comp.FindComponents<ProList<string>>().FirstOrDefault(x => x.Instance.Class == "list-2");
            list1.FindComponents<ProListItem<string>>().Count.Should().Be(8);
            var GetCheckBox = (IRenderedComponent<ProList<string>> list, string text) => list.FindComponents<ProListItem<string>>()
                        .FirstOrDefault(x => x.Instance.Text == text)?.FindComponent<ProCheckBox<bool?>>().Instance;
            var Select = async (IRenderedComponent<ProList<string>> list, string text) =>
                        await list.FindComponents<ProListItem<string>>()
                        .FirstOrDefault(x => x.Instance.Text == text).Find("div.pro-list-item").ClickAsync();
        ...
        public void ListColor(Color color)
        {
            ...
            var listItemClasses = comp.Find(".pro-selected-item");
            listItemClasses.ClassList.Should().ContainInOrder(new[] { $"pro-{color.ToStringFast(true)}-text", $"pro-{color.ToStringFast(true)}-hover" });
        }
        ...
        public void ListDenseInheritance(bool dense, bool? innerListDense, int expectedDenseClassCount)
        {
            ...
            comp.FindAll("div.pro-list-item").Count.Should().Be(9); // 7 choices, 2 groups
            comp.FindAll("div.pro-list-item-dense").Count.Should().Be(expectedDenseClassCount); // 7 choices, 2 groups
        }
        ...
        public async Task ListKeyboardNavigation_NonTabbableItemDoesNotHandleKeyCommands()
        {
            ...
            await comp.WaitForAssertionAsync(() =>
            {
                ...
                var currentItems = comp.FindAll("div.pro-list-item");
        ...
        public async Task ListKeyboardNavigation_SpaceTogglesMultiSelectionWithoutTabbableCheckboxes()
        {
            ...
            foreach (var checkbox in comp.FindAll("input.pro-checkbox-input"))
            {
        ...
        {
            var comp = Context.Render<ProList<string>>(builder => builder
                .AddChildContent<ProListItem<string>>(item => item
            ...
            var customIdItem = comp.Find("div.pro-list-item[data-test='custom-marker']");
            var fallbackItem = comp.FindAll("div.pro-list-item")[1];
        ...
        public void SettingGuttersOnList_Should_OverrideGuttersOnItemsWithoutGuttersSetting(bool listGutters, bool? itemGutters, bool resultingGutters)
        {
            ...
            (comp.FindAll("div.pro-list-item-gutters").Count > 0).Should().Be(resultingGutters);
        }
    }
}
