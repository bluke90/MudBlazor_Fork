...
namespace ProtonBlazor.UnitTests.Components
{
    ...
    public class DateRangePickerTests : BunitTest
    {
        ...
        public void Default()
        {
            var comp = Context.Render<ProDateRangePicker>();
        ...
        {
            var comp = Context.Render<ProDateRangePicker>();
            var openButton = comp.Find(".pro-input-adornment button");
        ...
        public void DateRangePicker_Preset_No_Timestamp()
        {
            ...
            comp.Markup.Should().Contain("pro-range-start-selected");
            comp.Markup.Should().Contain("pro-range-end-selected");
        }
        ...
        public void DateRangePickerLabelFor()
        {
            ...
            var label = comp.Find(".pro-input-label");
        ...
        public async Task Open_SelectTheSameDateTwice_RangeStartShouldEqualsEnd()
        {
            ...
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync();
        ...
        public async Task OpenPickerWithCustomStartMonth_SetDateRange_CheckValue()
        {
            ...
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync();
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("12")).ClickAsync();
            //check result
        ...
        public async Task<IRenderedComponent<SimpleMudMudDateRangePickerTest>> OpenPicker(Action<ComponentParameterCollectionBuilder<SimpleMudMudDateRangePickerTest>> parameterBuilder = null)
        {
            ...
            // should not be open
            comp.FindAll("div.pro-picker-open").Count.Should().Be(0);
            // click to to open menu
            await comp.Find("input").ClickAsync();
            // now its open
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-picker-open").Count.Should().Be(1));
            return comp;
        }
        ...
        public async Task Open_CloseByClickingOutsidePicker_CheckClosed()
        {
            ...
            await comp.Find("div.pro-overlay").ClickAsync();
        ...
        public async Task Open_CloseBySelectingADateRange_CheckClosed()
        {
            ...
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync();
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("23")).ClickAsync();
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-picker-open").Count.Should().Be(0), TimeSpan.FromSeconds(5));
        ...
        public async Task Open_SelectEndDateLowerThanStart_CheckClosed()
        {
            ...
            comp.FindAll("div.pro-picker-open").Count.Should().Be(1);
        ...
        public async Task OpenToYear_CheckYearsShown()
        {
            ...
            // should show years
            comp.FindAll("div.pro-picker-year-container").Count.Should().Be(1);
        }
        ...
        public async Task OpenToYear_ClickYear_CheckMonthsShown()
        {
            ...
            // should show years
            comp.FindAll("div.pro-picker-year-container").Count.Should().Be(1);
            await comp.FindAll("div.pro-picker-year")[0].ClickAsync();
            comp.FindAll("div.pro-picker-month-container").Count.Should().Be(1);
        }
        ...
        public async Task Open_ClickCalendarHeader_CheckMonthsShown()
        {
            var comp = await OpenPicker();
            // should show months
            await comp.FindAll("button.pro-picker-calendar-header-transition")[0].ClickAsync();
            comp.FindAll("div.pro-picker-month-container").Count.Should().Be(1);
        }
        ...
        public async Task OpenToMonth_Select3rdMonth_Select2ndDay_CheckDateRange()
        {
            ...
            // should show months
            comp.FindAll("div.pro-picker-month-container").Count.Should().Be(1);
            await comp.FindAll("div.pro-picker-calendar-container > div.pro-picker-month-container > button.pro-picker-month")[2].ClickAsync();
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("2")).ClickAsync();
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync();
        ...
        public async Task Open_ClickCalendarHeader_Click4thMonth_Click23rdDay_CheckDateRange()
        {
            var comp = await OpenPicker();
            var picker = comp.Instance;
            await comp.Find("button.pro-picker-calendar-header-transition").ClickAsync();
            // should show months
            comp.FindAll("div.pro-picker-month-container").Count.Should().Be(1);
            await comp.FindAll("div.pro-picker-calendar-container > div.pro-picker-month-container > button.pro-picker-month")
                [3].ClickAsync();
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync();
            await comp
        ...
        public async Task OpenTo12thMonth_NavigateBack_CheckMonth()
        {
            ...
            var picker = comp.Instance;
            await comp.Find("div.pro-picker-calendar-header-switch > button:nth-child(1)").ClickAsync();
        ...
        public async Task OpenTo12thMonth_NavigateForward_CheckYear()
        {
            var comp = await OpenTo12thMonth();
            var picker = comp.Instance;
            await comp.Find("div.pro-picker-calendar-header-switch > button:nth-child(3)").ClickAsync();
        ...
        {
            var comp = await OpenPicker();
            await comp.Find("div.pro-picker-datepicker-toolbar > button.pro-button-year").ClickAsync();
            comp.FindAll("div.pro-picker-calendar-container > div.pro-picker-year-container").Count.Should().Be(1);
            await comp
                .FindAll("div.pro-picker-calendar-container > div.pro-picker-year-container > div.pro-picker-year").First(x => x.TrimmedText().Contains("2022")).ClickAsync();
            comp.FindAll("div.pro-picker-month-container").Count.Should().Be(1);
            await comp.FindAll("div.pro-picker-calendar-container > div.pro-picker-month-container > button.pro-picker-month")[1].ClickAsync();
            comp.FindAll("div.pro-picker-calendar-container > div.pro-picker-calendar-header").Count.Should().Be(2);
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("1")).ClickAsync();
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("3")).ClickAsync();
        ...
        public async Task Open_Programmatically_CheckOpen_Close_Programmatically_CheckClosed()
        {
            ...
            comp.FindAll("div.pro-picker-content").Count.Should().Be(0);
            // open programmatically
            await comp.Instance.Open();
            comp.FindAll("div.pro-picker-content").Count.Should().Be(1);
        ...
        public void SetPickerValue_CheckText()
        {
            ...
            var comp = Context.Render<ProDateRangePicker>(parameters => parameters
        ...
        public async Task SetPickerToday_CheckSelected()
        {
            ...
            comp.FindAll("button.pro-selected").Count.Should().Be(1);
        }
        ...
        public async Task IsDateDisabledFunc_DisablesCalendarDateButtons()
        {
            ...
            comp.FindAll("button.pro-picker-calendar-day").Select(button => ((IHtmlButtonElement)button).IsDisabled)
        ...
        public async Task AdditionalDateClassesFunc_ClassIsAdded()
        {
            ...
            var daysCount = comp.FindAll("button.pro-picker-calendar-day")
                                .Select(button => (IHtmlButtonElement)button)
                                .Count();

            comp.FindAll("button.pro-day")
        ...
        {
            var comp = Context.Render<ProDateRangePicker>(parameters =>
        ...
        public async Task InitializeDateRange_WithMinValue_ShouldTreatAsNull()
        {
            ...
            {
                var picker = comp.FindComponent<ProDateRangePicker>();
        ...
        public async Task DateRangePicker_RequiredValidation()
        {
            ...
            var dateRangePickerComponent = Context.Render<ProDateRangePicker>(parameters => parameters
        ...
        public async Task CheckAutoCloseDateRangePicker_DoNotCloseWhenValueIsOff()
        {
            ...
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync();
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("11")).ClickAsync();
            ...
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-popover").Count.Should().Be(1));
        }
        ...
        public async Task CheckAutoCloseDateRangePicker_CloseWhenValueIsOn()
        {
            ...
            await comp.Find("input").ClickAsync();
            // verify open
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-popover-open").Count.Should().Be(1));
            ...
            await comp.WaitForAssertionAsync(() => comp.FindAll("div.pro-popover-open").Count.Should().Be(0));
        }
        ...
        public async Task CurrentDate_ShouldBeMarked()
        {
            ...
            comp.FindAll("button.pro-current").Count.Should().Be(1);
            comp.Find("button.pro-current").TrimmedText().Should().Be(currentDay);
        ...
        public async Task DateRangePicker_CustomTimeProvider()
        {
            ...
            comp.FindAll("button.pro-current").Count.Should().Be(1);
            comp.Find("button.pro-current").TrimmedText().Should().Be("4");
            comp.Find(".pro-button-month").TrimmedText().Should().Contain("April");
            comp.Find(".pro-button-year").TrimmedText().Should().Be("2003");
        }
        ...
        public async Task SingleDayRange_Should_Render_Selected()
        {
            ...
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync(new MouseEventArgs());
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ClickAsync(new MouseEventArgs());
            ...
            //pro-selected should be applied instead of pro-range-start-selected and pro-range-end-selected
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10"))
                .ToMarkup().Should().Contain("pro-selected")
                .And.NotContain("pro-range-start-selected")
                .And.NotContain("pro-range-end-selected");
        }
        ...
        public async Task OnPointerOver_ShouldCallJavaScriptFunction()
        {
            ...
            var button = comp
                .FindAll(".pro-button-root.pro-icon-button.pro-ripple.pro-ripple-icon.pro-picker-calendar-day.pro-day")
        ...
        public void FormatFirst_Should_RenderCorrectly()
        {
            ...
            var instance = comp.FindComponent<ProDateRangePicker>().Instance;
        ...
        public async Task CheckCloseOnClearDateRangePicker(bool closeOnClear)
        {
            ...
            // Clicking day buttons to select a date range
            await comp
                .FindAll("button.pro-button").First(x => x.TrimmedText().Equals("Clear")).ClickAsync();
            ...
            if (closeOnClear)
            {
                ...
                await comp.WaitForAssertionAsync(() => comp.Markup.Should().NotContain("pro-popover-open"));
            }
            else
            {
                // Check that the component is open
                await comp.WaitForAssertionAsync(() => comp.Find("div.pro-popover").ClassList.Should().Contain("pro-popover-open"));
            }
        }
        ...
        {
            var underlinedComp = Context.Render<ProDateRangePicker>(parameters
                => parameters.Add(p => p.Underline, true));
            var notUnderlinedComp = Context.Render<ProDateRangePicker>(parameters
                => parameters.Add(p => p.Underline, false));

            underlinedComp.FindAll(".pro-input-underline").Should().HaveCount(1);
            notUnderlinedComp.FindAll(".pro-input-underline").Should().HaveCount(0);
        }
        ...
        public async Task TestDateRangeClearableWithFormat()
        {
            ...
            var picker = comp.FindComponents<ProDateRangePicker>();
            picker.Count.Should().Be(2);
            var openBtn = picker[0].FindComponents<ProIconButton>();
            ...
            IReadOnlyList<IRenderedComponent<ProIconButton>> IconButtons(int index) =>
                picker[index].FindComponents<ProIconButton>();
        ...
        public async Task DateRangePickerToolbar_DisplaysSelectedDate()
        {
            ...
            comp.FindAll("button.pro-picker-calendar-day")
                .First(x => x.TrimmedText().Equals("10"))
                .ToMarkup().Should().Contain("pro-range-start-selected");
            comp.FindAll("button.pro-picker-calendar-day")
                .First(x => x.TrimmedText().Equals("20"))
                .ToMarkup().Should().Contain("pro-range-end-selected");
            comp.Find("button.pro-button-date .pro-button-label").InnerHtml.Should().Be("Fri, 10 Jan - Mon, 20 Jan");
            comp.Find("button.pro-button-year .pro-button-label").InnerHtml.Should().Be("2025");

            //navigate to previous month
            await comp.Find(".pro-picker-nav-button-prev").ClickAsync(new MouseEventArgs());

            //toolbar should display original range
            comp.Find("button.pro-button-year .pro-button-label").InnerHtml.Should().Be("2025");
            comp.Find("button.pro-button-date .pro-button-label").InnerHtml.Should().Be("Fri, 10 Jan - Mon, 20 Jan");
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("10")).ToMarkup().Should().NotContain("pro-selected");

            //select new month
            await comp.FindAll("button.pro-button-month")[0].ClickAsync(new MouseEventArgs());
            await comp.FindAll("button.pro-picker-month").First(x => x.TrimmedText().Equals("May")).ClickAsync(new MouseEventArgs());
            ...
            //select new year
            await comp.FindAll("button.pro-button-month")[0].ClickAsync(new MouseEventArgs());
            await comp.FindAll("button.pro-picker-calendar-header-transition")[0].ClickAsync(new MouseEventArgs());
            await comp.FindAll("div.pro-picker-year").First(x => x.TrimmedText().Equals("2022")).ClickAsync(new MouseEventArgs());
        ...
        {
            var comp = await OpenPicker();
            var currentYear = int.Parse(comp.Find("button.pro-button-year .pro-button-label").InnerHtml);
            var targetYear = (currentYear - 1).ToString(CultureInfo.InvariantCulture);

            await comp.Find("button.pro-button-month").ClickAsync();
            await comp.Find("button.pro-picker-calendar-header-transition").ClickAsync();
            await comp.FindAll("div.pro-picker-year")
                .First(x => x.TrimmedText().Equals(targetYear))
                .ClickAsync();

            comp.Find("button.pro-button-year .pro-button-label").InnerHtml.Should().Be(targetYear);
        }
        ...
        public async Task DateRangePicker_HighlightSelectedMonthOnly()
        {
            ...
            comp.FindAll("button.pro-picker-month").First(x => x.TrimmedText().Equals("Jan")).ToMarkup().Should().Contain("pro-picker-month-selected");

            //select new month (March)
            await comp.FindAll("button.pro-picker-month").First(x => x.TrimmedText().Equals("Mar")).ClickAsync(new MouseEventArgs());
            await comp.FindAll("button.pro-button-month")[0].ClickAsync(new MouseEventArgs());
            ...
            //change year
            await comp.Find(".pro-picker-calendar-header-switch button[aria-label^='Previous year']").ClickAsync(new MouseEventArgs());
            await comp.Find(".pro-picker-calendar-header-switch button[aria-label^='Previous year']").ClickAsync(new MouseEventArgs());

            //confirm no month is highlighted
            comp.Find(".pro-picker-month-container").ToMarkup().Should().NotContain("pro-picker-month-selected");

            //back to present year
            await comp.Find(".pro-picker-calendar-header-switch button[aria-label^='Next year']").ClickAsync(new MouseEventArgs());
            await comp.Find(".pro-picker-calendar-header-switch button[aria-label^='Next year']").ClickAsync(new MouseEventArgs());
        ...
        public async Task DateRangePicker_HighlightSelectedYearOnly()
        {
            ...
            //2025 is highlighted
            comp.FindAll("div.pro-picker-year").First(x => x.TrimmedText().Equals("2025")).ToMarkup().Should().Contain("pro-picker-year-selected");

            //select new year
            await comp.FindAll("div.pro-picker-year").First(x => x.TrimmedText().Equals("2020")).ClickAsync(new MouseEventArgs());
            await comp.FindAll("button.pro-picker-calendar-header-transition")[0].ClickAsync(new MouseEventArgs());
        ...
        public async Task DatePicker_JumpToYear()
        {
            ...
            //Jump to 2025
            await comp.Find("button.pro-button-year").ClickAsync(new MouseEventArgs());
        ...
        public async Task DateRangePicker_MinMaxDays()
        {
            ...
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("16")).ClickAsync(new MouseEventArgs());
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("17")).ToMarkup().Should().Contain("disabled");
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("18")).ClickAsync(new MouseEventArgs());
            ...
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("16")).ClickAsync(new MouseEventArgs());
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("17")).ToMarkup().Should().Contain("disabled"); //2 days not allowed
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("18")).ToMarkup().Should().NotContain("disabled"); //3 days valid
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("19")).ToMarkup().Should().NotContain("disabled"); //4 days valid
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("20")).ToMarkup().Should().NotContain("disabled"); //5 days valid
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("21")).ToMarkup().Should().NotContain("disabled"); //6 days valid
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("22")).ToMarkup().Should().NotContain("disabled"); //7 days valid
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("23")).ToMarkup().Should().Contain("disabled"); //8 days not allowed
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("24")).ToMarkup().Should().Contain("disabled"); //9 days not allowed
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("22")).ClickAsync(new MouseEventArgs());
            ...
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("16")).ClickAsync(new MouseEventArgs()); //                              [1]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("17")).ToMarkup().Should().Contain("disabled"); //2 days not allowed           [2]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("18")).ToMarkup().Should().Contain("disabled"); //3 disabled (weekend)         [3]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("19")).ToMarkup().Should().Contain("disabled"); //4 disabled (weekend)         [4]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("20")).ToMarkup().Should().NotContain("disabled"); //5 days valid              [5]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("21")).ToMarkup().Should().NotContain("disabled"); //6 days valid              [6]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("22")).ToMarkup().Should().NotContain("disabled"); //7 days valid              [7]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("23")).ToMarkup().Should().Contain("disabled"); //8 days not allowed
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("24")).ToMarkup().Should().Contain("disabled"); //9 days not allowed
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("20")).ClickAsync(new MouseEventArgs());
            ...
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("16")).ClickAsync(new MouseEventArgs());  //                             [1]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("17")).ToMarkup().Should().Contain("disabled"); //2 days not allowed           [2]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("18")).ToMarkup().Should().Contain("disabled"); //3 disabled (weekend)         [ ]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("19")).ToMarkup().Should().Contain("disabled"); //4 disabled (weekend)         [ ]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("20")).ToMarkup().Should().NotContain("disabled"); //5 days valid              [3]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("21")).ToMarkup().Should().NotContain("disabled"); //6 days valid              [4]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("22")).ToMarkup().Should().NotContain("disabled"); //7 days valid              [5]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("23")).ToMarkup().Should().NotContain("disabled"); //8 days valid              [6]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("24")).ToMarkup().Should().NotContain("disabled"); //9 days valid              [7]
            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("20")).ClickAsync(new MouseEventArgs());
            ...
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("23")).ToMarkup().Should().NotContain("disabled"); //8 days valid              [6]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("24")).ToMarkup().Should().NotContain("disabled"); //9 days valid              [7]
            comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("25")).ToMarkup().Should().Contain("disabled"); //9 days valid                 [8]

            await comp.FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals("24")).ClickAsync(new MouseEventArgs());
        ...
        public async Task DateRangePicker_MaxSelectableDate()
        {
            ...
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals(today.Day.ToString())).ClickAsync(new MouseEventArgs());
            await comp
                .FindAll("button.pro-picker-calendar-day").First(x => x.TrimmedText().Equals(today.Day.ToString())).ClickAsync(new MouseEventArgs());
        ...
        [Test]
        public void ProDateRangePickerStaticVariant_DisabledIsNotSpecified_EnabledByDefault()
        {
        ...
        [Test]
        public void ProDateRangePickerStaticVariant_Disabled_HasMudDisabledClass()
        {
            var comp = Context.Render<DateRangePickerStaticDisabledTest>();
            var picker = comp.FindComponents<ProDateRangePicker>();
            picker.Count.Should().Be(2);

            picker[0].Markup.Should().Contain("pro-disabled");
            picker[1].Markup.Should().NotContain("pro-disabled");
        }

        [Test]
        public void DisabledMudDateRangePickerStaticVariant_ShouldHave_ProDisabledMarkup()
        {
            var comp = Context.Render<DateRangePickerStaticDisabledTest>();

            var pickers = comp.FindComponents<ProDateRangePicker>();
        ...
        public void DateRangePicker_CustomClearIcon_Should_BeRenderedInMarkup()
        {
            var comp = Context.Render<ProDateRangePicker>(parameters => parameters
                ...
                .Add(p => p.Clearable, true)
                .Add(p => p.ClearIcon, Icons.Custom.Brands.ProtonBlazor));
        ...
        private sealed class DateRangePickerImpl : ProDateRangePicker
        {
    ...
    public static class DatePickerRenderedFragmentExtensions
    {
        ...
        {
            var matchingDays = comp.FindAll("button.pro-picker-calendar-day")
                       .Where(x => !x.ClassList.Contains("pro-hidden") && x.TrimmedText().Equals(day))
                       .ToList();
