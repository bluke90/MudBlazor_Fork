...
namespace ProtonBlazor.UnitTests.Other
{
    ...
    public class CategoryAttributeTests
    {
        ...
        {
            // Currently, these classes inheriting from ProComponentBase have uncategorized properties.
            // If you want you can categorize them, and then remove from this list.
            Type[] exceptions = {
                typeof(ProDataGrid<>),  // TODO: remove it later
                ...
                typeof(ProTh),
                typeof(ProTd),
                typeof(ProTablePager),
                typeof(ProTableSortLabel<>),
                typeof(ProTableGroupRow<>),

                typeof(ProInput<>),
                typeof(ProInputControl),
                typeof(ProInputLabel),
                typeof(ProRangeInput<>),

                typeof(ProCollapse),
                typeof(ProPageContentNavigation),
                typeof(ProSnackbarElement),
                typeof(ProtonBlazor.Charts.Legend<>),

                typeof(ProRatingItem),  // TODO: remove it later; see also: https://github.com/ProtonBlazor/ProtonBlazor/discussions/3452
            };

            var isTestOK = true;

            var components = typeof(ProElement).Assembly.GetTypes()
                                                                      .Where(type => type.IsSubclassOf(typeof(ProComponentBase)))
                                                                      .Except(exceptions);
