...
public sealed class ParameterStateCodeFixProvider : CodeFixProvider
{
    ...
    private const string ProtonBlazorExtensionsNamespace = "ProtonBlazor.Extensions";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (root is CompilationUnitSyntax compilationUnit)
        {
            var hasUsingDirective = compilationUnit.Usings.Any(u =>
                u.Name?.ToString() == ProtonBlazorExtensionsNamespace);

            if (!hasUsingDirective)
            {
                // Create the namespace import using SyntaxGenerator
                var namespaceImport = generator.NamespaceImportDeclaration(ProtonBlazorExtensionsNamespace);
