...
using ProtonBlazor.Docs.Pages.Api;
...
namespace ProtonBlazor.UnitTests.Docs.Generated
{
    ...
    public partial class ApiDocsTests
    {
        ...
        public async Task AlertPage_Test()
        {
            ...
            _ = _ctx.Render<ProtonBlazor.Docs.Pages.Components.Alert.AlertPage>();
        ...
        /// </summary>
        [Test]
        public async Task ProAlert_API_Test_Example()
        {
            _navigationManager.NavigateTo("/components/ProAlert");
            var comp = _ctx.Render<Api>(parameters => parameters.Add(x => x.TypeName, "ProAlert"));
            await _ctx.Services.GetService<IRenderQueueService>().WaitUntilEmpty();
            comp.Find(".pro-breadcrumbs");
            var exampleLink = comp.FindComponents<ProLink>().FirstOrDefault(link => link.Instance.Href.StartsWith("/component"));
