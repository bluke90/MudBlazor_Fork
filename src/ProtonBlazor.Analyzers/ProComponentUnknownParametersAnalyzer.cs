...
namespace ProtonBlazor.Analyzers
{
    [DiagnosticAnalyzer(LanguageNames.CSharp)]
    public sealed class ProComponentUnknownParametersAnalyzer : DiagnosticAnalyzer
    {
        ...
        public const string DebugAnalyzerProperty = "build_property.ProDebugAnalyzer";
        ...
        private sealed class AnalyzerContext
        {
            ...
            private readonly INamedTypeSymbol? _proComponentBaseType;
            ...
            public AnalyzerContext(Compilation compilation, AllowedAttributePattern allowedAttributePattern, string allowedAttributes)
            {
                ...
                _proComponentBaseType = compilation.GetBestTypeByMetadataName("ProtonBlazor.ProComponentBase");
            }

            public bool IsValid => _componentBaseSymbol is not null && _parameterSymbol is not null && _renderTreeBuilderSymbol is not null && _proComponentBaseType is not null;
                        ...
                        if (expressionStatement.Operation is IInvocationOperation invocation)
                        {
                            ...
                            if (targetMethod.ContainingType.IsEqualTo(_renderTreeBuilderSymbol))
                            {
                                if (string.Equals(targetMethod.Name, "OpenComponent", StringComparison.Ordinal) && targetMethod.TypeArguments.Length == 1)
                                {
                                    ...
                                    if (componentType.IsOrInheritFrom(_proComponentBaseType))
                                    {
