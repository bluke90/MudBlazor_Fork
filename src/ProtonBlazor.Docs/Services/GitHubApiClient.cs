...
#nullable enable
    public class GitHubApiClient : IDisposable
    {
        ...
        public async Task<GithubContributors[]> GetContributorsAsync()
        {
            try
            {
                var result = await _http.GetFromJsonAsync<GithubContributors[]>("repos/ProtonBlazor/ProtonBlazor/contributors?per_page=100", _jsonSerializerOptions);
        ...
        public async Task<GitHubReleases[]> GetReleasesAsync()
        {
            try
            {
                var result = await _http.GetFromJsonAsync<GitHubReleases[]>("repos/ProtonBlazor/ProtonBlazor/releases?per_page=100", _jsonSerializerOptions);
