...
namespace ProtonBlazor.Docs.Utilities;

#nullable enable
public partial class MarkdownToHtml
{
    private const string GitHubBlobDevBaseUrl = "https://github.com/ProtonBlazor/ProtonBlazor/blob/dev/";
    private const string GitHubRawDevBaseUrl = "https://raw.githubusercontent.com/ProtonBlazor/ProtonBlazor/dev/";
        ...
        LinkRenderOptions linkOptions)
    {
        ...
        renderer.ObjectRenderers.ReplaceOrAdd<HtmlObjectRenderer<HeadingBlock>>(new ProHeadingRenderer(headingOptions));
        renderer.ObjectRenderers.ReplaceOrAdd<HtmlObjectRenderer<LinkInline>>(new ProLinkRenderer(linkOptions));
        renderer.ObjectRenderers.ReplaceOrAdd<HtmlObjectRenderer<ListBlock>>(new ProListRenderer(listOptions));
    ...
    private class ProListRenderer : HtmlObjectRenderer<ListBlock>
    {
        private readonly ListRenderOptions _options;

        public ProListRenderer(ListRenderOptions options)
        {
            _options = options;
        }
    ...
    private class ProHeadingRenderer : HtmlObjectRenderer<HeadingBlock>
    {
        ...
        public ProHeadingRenderer(HeadingRenderOptions options)
        {
            _options = options;
        }
        ...
        protected override void Write(HtmlRenderer renderer, HeadingBlock obj)
        {
            ...
            var className = _options.IncludeTopMargin
                ? $"pro-typography pro-typography-{heading} mt-3"
                : $"pro-typography pro-typography-{heading}";
            ...
            {
                renderer.Write("<hr class=\"pro-divider pro-divider-fullwidth\">");
            }
    ...
    private class ProLinkRenderer : HtmlObjectRenderer<LinkInline>
    {
        private readonly LinkRenderOptions _options;

        public ProLinkRenderer(LinkRenderOptions options)
        {
            _options = options;
        }
        ...
        protected override void Write(HtmlRenderer renderer, LinkInline obj)
        {
            ...
            {
                attributes.AddClass("pro-link pro-default-text pro-link-underline-hover github-user");
            }
            else
            {
                attributes.AddClass("pro-link pro-primary-text pro-link-underline-hover");
            }
    ...
    [GeneratedRegex(@"https://github\.com/ProtonBlazor/ProtonBlazor/pull/(?<id>\d{3,6})")]
    private static partial Regex PullRequestUrlRegex();

    [GeneratedRegex(@"https://github\.com/ProtonBlazor/ProtonBlazor/compare/(?<range>[^\s)]+)")]
