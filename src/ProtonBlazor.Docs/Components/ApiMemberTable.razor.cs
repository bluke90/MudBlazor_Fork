...
public partial class ApiMemberTable
{
    ...
    /// This table.
    /// </summary>
    public ProTable<DocumentedMember>? Table { get; set; }
    ...
    public TableGroupDefinition<DocumentedMember>? CurrentGroups
    {
        get
        {
            ...
            {
                return new() { Selector = (property) => (property.DeclaringType is not null && property.DeclaringType == this.Type) ? "" : $"ProGlobal.{GetDeclaringTypeName(property)}" };
            }
            else
            {
    ...
    {
        // Force a new load (otherwise ProChips won't properly update)
